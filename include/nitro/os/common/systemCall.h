...
#define SVC_CpuClear(data, destp, size, bit)                                   \
  do {                                                                         \
    ...
    SVC_CpuSet((u8 *)&(tmp), (u8 *)(destp),                                    \
               (MI_DMA_SRC_FIX | MI_DMA_##bit##BIT_BUS |                       \
                ((size) / ((bit) / 8) & 0x1fffff)));                           \
  } while (0)
...
#define SVC_CpuCopy(srcp, destp, size, bit)                                    \
  ...
  SVC_CpuSet((u8 *)(srcp), (u8 *)(destp),                                      \
             (MI_DMA_SRC_INC | MI_DMA_##bit##BIT_BUS |                         \
              ((size) / ((bit) / 8) & 0x1fffff)))
...
void SVC_UnpackBits(const void *srcp, void *destp,
...
s32 SVC_UncompressLZ16FromDevice(const void *srcp, void *destp,
                                 const void *paramp,
                                 const MIReadStreamCallbacks *callbacks);

s32 SVC_UncompressRL16FromDevice(const void *srcp, void *destp,
                                 const void *paramp,
                                 const MIReadStreamCallbacks *callbacks);

s32 SVC_UncompressHuffmanFromDevice(const void *srcp, void *destp,
                                    u8 *tableBufp,
                                    const MIReadStreamCallbacks *callbacks);
