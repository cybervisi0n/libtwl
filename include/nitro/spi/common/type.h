...
#define SPI_PXI_INDEX_MASK                                                     \
  0x000f0000 // Defines the placement of bits that show the index of consecutive
             // packets
#define SPI_PXI_INDEX_SHIFT 16 //
#define SPI_PXI_DATA_MASK                                                      \
  0x0000ffff // Defines the placement of bits that indicate the data portion of
             // consecutive packets
#define SPI_PXI_DATA_SHIFT 0                  //
#define SPI_PXI_RESULT_COMMAND_BIT 0x00008000 // Bit that indicates response
#define SPI_PXI_RESULT_COMMAND_MASK                                            \
  0x00007f00 // Defines the placement of bits for response command type
#define SPI_PXI_RESULT_COMMAND_SHIFT 8 //
#define SPI_PXI_RESULT_DATA_MASK                                               \
  0x000000ff // Defines the placement of bits for response command results
#define SPI_PXI_RESULT_DATA_SHIFT 0 //
#define SPI_PXI_CONTINUOUS_PACKET_MAX                                          \
  16 // Maximum number of consecutive packets in succession
...
#define SPI_PXI_COMMAND_TP_AUTO_SAMPLING                                       \
  0x0010 // For midway notifications during the course of auto sampling
...
#define SPI_PXI_RESULT_ILLEGAL_STATUS                                          \
  0x0003 // State that cannot receive instructions
...
#define SPI_TP_DEFAULT_STABILITY_RANGE                                         \
  20 // Output range difference that is permitted when the stability is
...
typedef enum SPITpTouch { SPI_TP_TOUCH_OFF = 0, SPI_TP_TOUCH_ON } SPITpTouch;
