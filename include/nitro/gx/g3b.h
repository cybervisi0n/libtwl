...
static void G3BS_Direct3(GXDLInfo *info, int op, u32 param0, u32 param1,
                         u32 param2);
...
void G3BS_MultTransMtx33(GXDLInfo *info, const MtxFx33 *mtx,
                         const VecFx32 *trans);
...
static void G3BS_PolygonAttr(GXDLInfo *info, int light, GXPolygonMode polyMode,
                             GXCull cullMode, int polygonID, int alpha,
                             int misc // GXPolygonAttrMisc
);
static void G3BS_TexImageParam(GXDLInfo *info, GXTexFmt texFmt, GXTexGen texGen,
                               GXTexSizeS s, GXTexSizeT t, GXTexRepeat repeat,
                               GXTexFlip flip, GXTexPlttColor0 pltt0, u32 addr);
static void G3BS_TexPlttBase(GXDLInfo *info, u32 addr, GXTexFmt texfmt);
static void G3BS_MaterialColorDiffAmb(GXDLInfo *info, GXRgb diffuse,
                                      GXRgb ambient, BOOL IsSetVtxColor);
static void G3BS_MaterialColorSpecEmi(GXDLInfo *info, GXRgb specular,
                                      GXRgb emission, BOOL IsShininess);
static void G3BS_LightVector(GXDLInfo *info, GXLightId lightID, fx16 x, fx16 y,
                             fx16 z);
...
void G3B_MultTransMtx33(GXDLInfo *info, const MtxFx33 *mtx,
                        const VecFx32 *trans);
...
void G3B_PolygonAttr(GXDLInfo *info, int light, GXPolygonMode polyMode,
                     GXCull cullMode, int polygonID, int alpha,
                     int misc // GXPolygonAttrMisc
);
void G3B_TexImageParam(GXDLInfo *info, GXTexFmt texFmt, GXTexGen texGen,
                       GXTexSizeS s, GXTexSizeT t, GXTexRepeat repeat,
                       GXTexFlip flip, GXTexPlttColor0 pltt0, u32 addr);
void G3B_TexPlttBase(GXDLInfo *info, u32 addr, GXTexFmt texfmt);
void G3B_MaterialColorDiffAmb(GXDLInfo *info, GXRgb diffuse, GXRgb ambient,
                              BOOL IsSetVtxColor);
void G3B_MaterialColorSpecEmi(GXDLInfo *info, GXRgb specular, GXRgb emission,
                              BOOL IsShininess);
...
static inline void G3BS_Direct2(GXDLInfo *info, int op, u32 param0,
                                u32 param1) {
  SDK_NULL_ASSERT(info);
  GXBS_CURR_ALIGNMENT_ASSERT(info);
  GX_VALID_OPCODE_ASSERT(op);
...
static inline void G3BS_Direct3(GXDLInfo *info, int op, u32 param0, u32 param1,
                                u32 param2) {
  SDK_NULL_ASSERT(info);
  GXBS_CURR_ALIGNMENT_ASSERT(info);
  GX_VALID_OPCODE_ASSERT(op);
...
static inline void G3BS_Nop(GXDLInfo *info) { G3BS_Direct0(info, G3OP_NOP); }
...
static inline void G3BS_PolygonAttr(GXDLInfo *info, int light,
                                    GXPolygonMode polyMode, GXCull cullMode,
  ...
  G3BS_Direct1(info, G3OP_POLYGON_ATTR,
               GX_PACK_POLYGONATTR_PARAM(light, polyMode, cullMode, polygonID,
                                         alpha, misc));
}

static inline void G3BS_TexImageParam(GXDLInfo *info, GXTexFmt texFmt,
                                      GXTexGen texGen, GXTexSizeS s,
                                      GXTexSizeT t, GXTexRepeat repeat,
                                      GXTexFlip flip, GXTexPlttColor0 pltt0,
                                      u32 addr) {
...
static inline void G3BS_MaterialColorDiffAmb(GXDLInfo *info, GXRgb diffuse,
                                             GXRgb ambient,
                                             BOOL IsSetVtxColor) {
  GXRGB_ASSERT(diffuse);
  GXRGB_ASSERT(ambient);

  G3BS_Direct1(info, G3OP_DIF_AMB,
               GX_PACK_DIFFAMB_PARAM(diffuse, ambient, IsSetVtxColor));
}

static inline void G3BS_MaterialColorSpecEmi(GXDLInfo *info, GXRgb specular,
                                             GXRgb emission, BOOL IsShininess) {
  GXRGB_ASSERT(specular);
  GXRGB_ASSERT(emission);

  G3BS_Direct1(info, G3OP_SPE_EMI,
               GX_PACK_SPECEMI_PARAM(specular, emission, IsShininess));
}

static inline void G3BS_LightVector(GXDLInfo *info, GXLightId lightID, fx16 x,
                                    fx16 y, fx16 z) {
  ...
  G3BS_Direct1(info, G3OP_LIGHT_VECTOR,
               GX_PACK_LIGHTVECTOR_PARAM(lightID, x, y, z));
}

static inline void G3BS_LightColor(GXDLInfo *info, GXLightId lightID,
                                   GXRgb rgb) {
  GX_LIGHTID_ASSERT(lightID);
  GXRGB_ASSERT(rgb);
...
static inline void G3BS_End(GXDLInfo *info) { G3BS_Direct0(info, G3OP_END); }

static inline void G3BS_SwapBuffers(GXDLInfo *info, GXSortMode am,
                                    GXBufferMode zw) {
...
static inline void G3BS_ViewPort(GXDLInfo *info, int x1, int y1, int x2,
                                 int y2) {
...
static inline void G3B_UpdateGXDLInfo(GXDLInfo *info, int n) {
  ...
  SDK_TASSERTMSG(((u32)(info->curr_cmd) - (u32)info->bottom <= info->length),
                 "Buffer overflow ! : Current DL buffer doesn't have enough "
                 "capacity for new commands\n");
}
