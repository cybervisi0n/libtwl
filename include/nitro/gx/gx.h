...
#define GX_BGMODE_ASSERT(x) SDK_MINMAX_ASSERT(x, GX_BGMODE_0, GX_BGMODE_6)

typedef enum { GX_BG0_AS_2D = 0, GX_BG0_AS_3D = 1 } GXBG0As;

#define GX_BG0_AS_ASSERT(x) SDK_MINMAX_ASSERT(x, GX_BG0_AS_2D, GX_BG0_AS_3D)
...
#define GX_DISPMODE_ASSERT(x)                                                  \
  SDK_ASSERT((x) == GX_DISPMODE_OFF || (x) == GX_DISPMODE_GRAPHICS ||          \
             (x) == GX_DISPMODE_VRAM_A || (x) == GX_DISPMODE_VRAM_B ||         \
             (x) == GX_DISPMODE_VRAM_C || (x) == GX_DISPMODE_VRAM_D ||         \
             (x) == GX_DISPMODE_MMEM)

typedef enum {
  GX_OBJVRAMMODE_CHAR_2D =
      (0 << REG_GX_DISPCNT_OBJMAP_SHIFT) | (0 << REG_GX_DISPCNT_EXOBJ_SHIFT),
...
#define GX_OBJVRAMMODE_CHAR_ASSERT(x)                                          \
  SDK_ASSERT(x == GX_OBJVRAMMODE_CHAR_2D || x == GX_OBJVRAMMODE_CHAR_1D_32K || \
...
typedef enum {
  GX_OBJVRAMMODE_BMP_2D_W128 = (0 << (REG_GX_DISPCNT_OBJMAP_SHIFT + 1)) |
                               (0 << (REG_GX_DISPCNT_EXOBJ_SHIFT + 2)),
  GX_OBJVRAMMODE_BMP_2D_W256 = (1 << (REG_GX_DISPCNT_OBJMAP_SHIFT + 1)) |
                               (0 << (REG_GX_DISPCNT_EXOBJ_SHIFT + 2)),
  GX_OBJVRAMMODE_BMP_1D_128K = (2 << (REG_GX_DISPCNT_OBJMAP_SHIFT + 1)) |
                               (0 << (REG_GX_DISPCNT_EXOBJ_SHIFT + 2)),
  GX_OBJVRAMMODE_BMP_1D_256K = (2 << (REG_GX_DISPCNT_OBJMAP_SHIFT + 1)) |
                               (1 << (REG_GX_DISPCNT_EXOBJ_SHIFT + 2))
} GXOBJVRamModeBmp;
...
#define GX_POWER_ASSERT(x)                                                     \
  SDK_TASSERTMSG((((x) & ~GX_POWER_ALL) == 0),                                 \
...
static inline GXBGScrOffset GX_GetBGScrOffset() {
  return (
      GXBGScrOffset)((reg_GX_DISPCNT & REG_GX_DISPCNT_BGSCREENOFFSET_MASK) >>
...
static inline void GX_SetBGScrOffset(GXBGScrOffset offset) {
  ...
  reg_GX_DISPCNT =
      (u32)((reg_GX_DISPCNT & ~REG_GX_DISPCNT_BGSCREENOFFSET_MASK) |
...
static inline int GX_GetVisiblePlane(void) {
  return (int)((reg_GX_DISPCNT & REG_GX_DISPCNT_DISPLAY_MASK) >>
...
static inline void GXS_SetVisiblePlane(int plane) {
  ...
  reg_GXS_DB_DISPCNT =
      (u32)((reg_GXS_DB_DISPCNT & ~REG_GXS_DB_DISPCNT_DISPLAY_MASK) |
...
static inline void GX_SetVisibleWnd(int window) {
  ...
  reg_GX_DISPCNT = (u32)((reg_GX_DISPCNT &
                          ~(REG_GX_DISPCNT_W0_MASK | REG_GX_DISPCNT_W1_MASK |
                            REG_GX_DISPCNT_OW_MASK)) |
                         (window << REG_GX_DISPCNT_W0_SHIFT));
}

static inline int GX_GetVisibleWnd(void) {
  return (
      int)((reg_GX_DISPCNT & (REG_GX_DISPCNT_W0_MASK | REG_GX_DISPCNT_W1_MASK |
                              REG_GX_DISPCNT_OW_MASK)) >>
...
static inline void GXS_SetVisibleWnd(int window) {
  ...
  reg_GXS_DB_DISPCNT =
      (u32)((reg_GXS_DB_DISPCNT &
             ~(REG_GXS_DB_DISPCNT_W0_MASK | REG_GXS_DB_DISPCNT_W1_MASK |
               REG_GXS_DB_DISPCNT_OW_MASK)) |
            (window << REG_GXS_DB_DISPCNT_W0_SHIFT));
}

static inline int GXS_GetVisibleWnd(void) {
  return (int)((reg_GXS_DB_DISPCNT &
                (REG_GXS_DB_DISPCNT_W0_MASK | REG_GXS_DB_DISPCNT_W1_MASK |
                 REG_GXS_DB_DISPCNT_OW_MASK)) >>
...
static inline void GX_SetOBJVRamModeChar(GXOBJVRamModeChar mode) {
  ...
  reg_GX_DISPCNT = (u32)(reg_GX_DISPCNT & ~(REG_GX_DISPCNT_EXOBJ_CH_MASK |
                                            REG_GX_DISPCNT_OBJMAP_CH_MASK) |
                         mode);
}

static inline GXOBJVRamModeChar GX_GetOBJVRamModeChar(void) {
  return (GXOBJVRamModeChar)(reg_GX_DISPCNT & (REG_GX_DISPCNT_EXOBJ_CH_MASK |
                                               REG_GX_DISPCNT_OBJMAP_CH_MASK));
}
...
static inline void GXS_SetOBJVRamModeChar(GXOBJVRamModeChar mode) {
  ...
  reg_GXS_DB_DISPCNT =
      (u32)(reg_GXS_DB_DISPCNT & ~(REG_GXS_DB_DISPCNT_EXOBJ_CH_MASK |
...
static inline void GX_SetOBJVRamModeBmp(GXOBJVRamModeBmp mode) {
  ...
  reg_GX_DISPCNT = (u32)(reg_GX_DISPCNT & ~(REG_GX_DISPCNT_EXOBJ_BM_MASK |
                                            REG_GX_DISPCNT_OBJMAP_BM_MASK) |
                         mode);
}

static inline GXOBJVRamModeBmp GX_GetOBJVRamModeBmp(void) {
  return (GXOBJVRamModeBmp)(reg_GX_DISPCNT & (REG_GX_DISPCNT_EXOBJ_BM_MASK |
                                              REG_GX_DISPCNT_OBJMAP_BM_MASK));
}
...
static inline void GXS_SetOBJVRamModeBmp(GXOBJVRamModeBmp mode) {
  ...
  reg_GXS_DB_DISPCNT =
      (u32)(reg_GXS_DB_DISPCNT & ~(REG_GXS_DB_DISPCNT_OBJMAP_BM_MASK) | mode);
}

static inline GXOBJVRamModeBmp GXS_GetOBJVRamModeBmp(void) {
  return (GXOBJVRamModeBmp)(reg_GXS_DB_DISPCNT &
                            REG_GXS_DB_DISPCNT_OBJMAP_BM_MASK);
}

static inline void GX_BlankScr(BOOL blank) {
  SDK_WARNING(
      (1 == 0),
      "GX_BlankScr is obsolete, please use GX_DispOn/GX_DispOff instead.");
  if (blank) {
...
static inline s32 GX_GetVCount() { return reg_GX_VCOUNT; }

static inline void GX_SetVCount(s32 count) {
  static vu32 vblank_count;

  if (count < 202 || count > 211) {
    SDK_TFATAL_ERROR("V Counter out of range(%d). it must be 202 to 211.",
                     count);
    ...
    if (current_count < 202 || current_count > 211) {
      SDK_TFATAL_ERROR(
          "Current V Counter out of range(%d). it must be 202 to 211.",
          current_count);
...
#define GX_PowerLCD(x)                                                         \
  SDK_ERR("GX_PowerLCD() is discontinued. please use PM_SetLCDPower()\n")
#define GX_PowerLCDTop(x)                                                      \
  SDK_ERR("GX_PowerLCDTop() is discontinued. please use PM_SetLCDPower()\n")
#define GX_PowerLCDBot(x)                                                      \
  SDK_ERR("GX_PowerLCDBot() is discontinued. please use PM_SetLCDPower()\n")
#define GX_POWER_LCD_TOP                                                       \
  SDK_ERR("GX_POWER_LCD_TOP is discontinued. please use PM_SetLCDPower()\n")
#define GX_POWER_LCD_BOT                                                       \
  SDK_ERR("GX_POWER_LCD_BOT is discontinued. please use PM_SetLCDPower()\n")
#define GX_POWER_LCD                                                           \
  SDK_ERR("GX_POWER_LCD is discontinued. please use PM_SetLCDPower()\n")
...
static inline GXDispSelect GX_GetDispSelect(void) {
  return (GXDispSelect)((reg_GX_POWCNT & REG_GX_POWCNT_DSEL_MASK) >>
                        REG_GX_POWCNT_DSEL_SHIFT);
}
