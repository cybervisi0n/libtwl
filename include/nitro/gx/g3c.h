...
void G3CS_MultTransMtx33(GXDLInfo *info, const MtxFx33 *mtx,
                         const VecFx32 *trans);
...
static void G3CS_PolygonAttr(GXDLInfo *info, int light, GXPolygonMode polyMode,
                             GXCull cullMode, int polygonID, int alpha,
                             int misc // GXPolygonAttrMisc
);
static void G3CS_TexImageParam(GXDLInfo *info, GXTexFmt texFmt, GXTexGen texGen,
...
static void G3CS_MaterialColorDiffAmb(GXDLInfo *info, GXRgb diffuse,
                                      GXRgb ambient, BOOL IsSetVtxColor);
static void G3CS_MaterialColorSpecEmi(GXDLInfo *info, GXRgb specular,
                                      GXRgb emission, BOOL IsShininess);
static void G3CS_LightVector(GXDLInfo *info, GXLightId lightID, fx16 x, fx16 y,
                             fx16 z);
...
void G3C_MultTransMtx33(GXDLInfo *info, const MtxFx33 *mtx,
                        const VecFx32 *trans);
...
void G3C_PolygonAttr(GXDLInfo *info, int light, GXPolygonMode polyMode,
                     GXCull cullMode, int polygonID, int alpha,
                     int misc // GXPolygonAttrMisc
);
void G3C_TexImageParam(GXDLInfo *info, GXTexFmt texFmt, GXTexGen texGen,
                       GXTexSizeS s, GXTexSizeT t, GXTexRepeat repeat,
                       GXTexFlip flip, GXTexPlttColor0 pltt0, u32 addr);
void G3C_TexPlttBase(GXDLInfo *info, u32 addr, GXTexFmt texfmt);
void G3C_MaterialColorDiffAmb(GXDLInfo *info, GXRgb diffuse, GXRgb ambient,
                              BOOL IsSetVtxColor);
void G3C_MaterialColorSpecEmi(GXDLInfo *info, GXRgb specular, GXRgb emission,
                              BOOL IsShininess);
...
static inline void G3CS_Nop(GXDLInfo *info) { G3CS_Direct0(info, G3OP_NOP); }
...
static inline void G3CS_PolygonAttr(GXDLInfo *info, int light,
  ...
  G3CS_Direct1(info, G3OP_POLYGON_ATTR,
               GX_PACK_POLYGONATTR_PARAM(light, polyMode, cullMode, polygonID,
                                         alpha, misc));
}

static inline void G3CS_TexImageParam(GXDLInfo *info, GXTexFmt texFmt,
...
static inline void G3CS_MaterialColorDiffAmb(GXDLInfo *info, GXRgb diffuse,
                                             GXRgb ambient,
                                             BOOL IsSetVtxColor) {
  GXRGB_ASSERT(diffuse);
  GXRGB_ASSERT(ambient);

  G3CS_Direct1(info, G3OP_DIF_AMB,
               GX_PACK_DIFFAMB_PARAM(diffuse, ambient, IsSetVtxColor));
}

static inline void G3CS_MaterialColorSpecEmi(GXDLInfo *info, GXRgb specular,
                                             GXRgb emission, BOOL IsShininess) {
  GXRGB_ASSERT(specular);
  GXRGB_ASSERT(emission);

  G3CS_Direct1(info, G3OP_SPE_EMI,
               GX_PACK_SPECEMI_PARAM(specular, emission, IsShininess));
}

static inline void G3CS_LightVector(GXDLInfo *info, GXLightId lightID, fx16 x,
                                    fx16 y, fx16 z) {
  ...
  G3CS_Direct1(info, G3OP_LIGHT_VECTOR,
               GX_PACK_LIGHTVECTOR_PARAM(lightID, x, y, z));
}

static inline void G3CS_LightColor(GXDLInfo *info, GXLightId lightID,
                                   GXRgb rgb) {
  GX_LIGHTID_ASSERT(lightID);
  GXRGB_ASSERT(rgb);
...
static inline void G3CS_End(GXDLInfo *info) { G3CS_Direct0(info, G3OP_END); }

static inline void G3CS_SwapBuffers(GXDLInfo *info, GXSortMode am,
                                    GXBufferMode zw) {
...
static inline void G3CS_ViewPort(GXDLInfo *info, int x1, int y1, int x2,
                                 int y2) {
