...
#define GX_VALID_OPCODE_ASSERT(x)                                              \
  SDK_ASSERT((x == 0xFF) || ((x >= 0) && (x <= 0x72)))
...
#define GX_CULL_ASSERT(x) SDK_MINMAX_ASSERT(x, GX_CULL_ALL, GX_CULL_NONE)
...
#define GX_LIGHTID_ASSERT(x) SDK_MINMAX_ASSERT(x, GX_LIGHTID_0, GX_LIGHTID_3)
...
#define GX_VTX10_MASK                                                          \
  (GX_VTX10_INT_MASK | GX_VTX10_DEC_MASK | GX_VTX10_SIGN_MASK)
...
#define GX_VECVTX10(x, y, z)                                                   \
  ((VecVtx10)(((((x) >> (FX32_DEC_SIZE - GX_VTX10_DEC_SIZE)) & GX_VTX10_MASK)  \
               << GX_VEC_VTX10_X_SHIFT) |                                      \
              ((((y) >> (FX32_DEC_SIZE - GX_VTX10_DEC_SIZE)) & GX_VTX10_MASK)  \
               << GX_VEC_VTX10_Y_SHIFT) |                                      \
              ((((z) >> (FX32_DEC_SIZE - GX_VTX10_DEC_SIZE)) & GX_VTX10_MASK)  \
               << GX_VEC_VTX10_Z_SHIFT)))
...
#define GX_PACK_LIGHTCOLOR_PARAM(lightID, rgb)                                 \
  ((u32)((rgb) | ((u32)(lightID) << REG_G3_LIGHT_COLOR_LNUM_SHIFT)))
...
#define GX_PACK_POLYGONATTR_PARAM(light, polyMode, cullMode, polygonID, alpha, \
                                  misc)                                        \
  ((u32)(((light) << REG_G3_POLYGON_ATTR_LE_SHIFT) |                           \
         ((polyMode) << REG_G3_POLYGON_ATTR_PM_SHIFT) |                        \
         ((cullMode) << REG_G3_POLYGON_ATTR_BK_SHIFT) | (misc) |               \
...
#define GX_PACK_TEXIMAGE_PARAM(texFmt, texGen, s, t, repeat, flip, pltt0,      \
                               addr)                                           \
...
static GXTexPlttBaseParam G3_MakeTexPlttBaseParam(u32 plttBaseAddr,
                                                  GXTexFmt texfmt);
...
static inline u32 G3_GetDLLength(const GXDLInfo *info) { return info->length; }
