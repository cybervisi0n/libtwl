...
#define GX_CAPTURE_DEST_ASSERT(x)                                              \
  SDK_MINMAX_ASSERT(x, GX_CAPTURE_DEST_VRAM_A_0x00000,                         \
...
typedef enum { GX_CAPTURE_SRCA_2D3D = 0, GX_CAPTURE_SRCA_3D = 1 } GXCaptureSrcA;
...
#define GX_CAPTURE_MODE_ASSERT(x)                                              \
  SDK_MINMAX_ASSERT(x, GX_CAPTURE_MODE_A, GX_CAPTURE_MODE_AB)

#if !(defined(SDK_WIN32) || defined(SDK_FROM_TOOL))

static void GX_SetCapture(GXCaptureSize sz, GXCaptureMode mode, GXCaptureSrcA a,
                          GXCaptureSrcB b, GXCaptureDest dest, int eva,
                          int evb);

static inline void GX_SetCapture(GXCaptureSize sz, GXCaptureMode mode,
                                 GXCaptureSrcA a, GXCaptureSrcB b,
                                 GXCaptureDest dest, int eva, int evb) {
  ...
  reg_GX_DISPCAPCNT =
      (REG_GX_DISPCAPCNT_E_MASK | (mode << REG_GX_DISPCAPCNT_MOD_SHIFT) |
