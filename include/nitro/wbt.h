...
#define WBT_PACKET_BITMAP_SIZE(block_size, packet_size)                        \
  (((((block_size) + ((packet_size) - WBT_PACKET_SIZE_MIN) - 1) /              \
     ((packet_size) - WBT_PACKET_SIZE_MIN)) +                                  \
    31) /                                                                      \
   sizeof(u32)) *                                                              \
      sizeof(u32)
...
static inline u16 WBT_AidToAidbitmap(int aid) { return (u16)(1 << aid); }
...
BOOL WBT_GetBlock(u16 target, u32 block_id, WBTRecvBufTable *recv_buf_table,
                  u32 recv_size, WBTPacketBitmapTable *p_bmp_table,
                  WBTCallback callback);

BOOL WBT_PutUserData(u16 target, const void *user_data, int size,
                     WBTCallback callback);
