...
typedef struct SNDLfoParam {
  ...
  u8 speed;  // At 256, one sample at a time
  u8 depth;  // At 128, multiplied by 1.0
  u8 range;
  u16 delay;
} SNDLfoParam;
...
typedef struct SNDExChannel {
  ...
  s32 env_decay; // Attenuation due to envelope release
...
struct SNDExChannel *SND_AllocExChannel(u32 chBitMask, int prio,
                                        BOOL strongRequest,
                                        SNDExChannelCallback callback,
                                        void *callbackData);
void SND_FreeExChannel(struct SNDExChannel *ch_p);

BOOL SND_StartExChannelPcm(SNDExChannel *ch_p, const struct SNDWaveParam *wave,
                           const void *data, s32 length);
