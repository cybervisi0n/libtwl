...
typedef BOOL(__stdcall *SNDReadInstDataFunc)(
    const struct SNDBankDataCallback *bank, int prgNo, int key,
...
void SND_AssignWaveArc(struct SNDBankData *bank, int index,
                       struct SNDWaveArc *waveArc);
...
BOOL SND_GetNextInstData(const struct SNDBankData *bank,
                         struct SNDInstData *inst, struct SNDInstPos *pos);

u32 SND_GetWaveDataCount(const struct SNDWaveArc *waveArc);
void SND_SetWaveDataAddress(struct SNDWaveArc *waveArc, int index,
