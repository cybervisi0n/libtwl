...
#ifndef SDK_TEG
typedef enum { SND_CAPTURE_0, SND_CAPTURE_1 } SNDCapture;
#else
typedef enum { SND_CAPTURE_1, SND_CAPTURE_0 } SNDCapture;
#endif
...
typedef enum { SND_CAPTURE_REPEAT_YES, SND_CAPTURE_REPEAT_NO } SNDCaptureRepeat;
...
static inline void SND_StartCaptureBoth(void) {
  reg_SND_SNDCAPCNT |=
      REG_SND_SNDCAPCNT_CAP0_E_MASK | REG_SND_SNDCAPCNT_CAP1_E_MASK;
}
...
void SND_SetupCapture(SNDCapture capture, SNDCaptureFormat format,
                      void *buffer_addr, int length, BOOL repeat,
