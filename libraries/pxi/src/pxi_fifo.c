...
void PXI_InitFifo(void) {
  ...
  if (!FifoCtrlInit) {
    ...
    reg_PXI_FIFO_CNT =
        (REG_PXI_FIFO_CNT_SEND_CL_MASK | REG_PXI_FIFO_CNT_RECV_RI_MASK |
...
void PXI_SetFifoRecvCallback(int fifotag, PXIFifoCallback callback) {
  ...
  SDK_WARNING(callback == NULL || FifoRecvCallbackTable[fifotag] == NULL ||
...
int PXI_SendWordByFifo(int fifotag, u32 data, BOOL err) {
  ...
  SDK_TASSERTMSG(0 <= fifotag && fifotag < PXI_MAX_FIFO_TAG,
                 "[FifoTag] out of range");
  SDK_TASSERTMSG(data < (1UL << PXI_FIFOMESSAGE_BITSZ_DATA),
                 "[data] out of range");
