...
static struct {
  TPRecvCallback
      callback;  // User callback function called when TP value obtained
...
#pragma unused(tag)
  ...
  if (command == SPI_PXI_COMMAND_TP_AUTO_SAMPLING) {
    ...
    if (tpState.callback) {
      (tpState.callback)((TPRequestCommand)command, TP_RESULT_SUCCESS,
                         (u8)tpState.index);
    }
    return;
  }
  ...
  common:
    ...
    if (tpState.callback) {
      (tpState.callback)((TPRequestCommand)command,
                         (TPRequestResult)(result & 0xff), 0);
    }
    break;
...
BOOL TP_GetUserInfo(TPCalibrateParam *calibrate) {
  ...
  if ((x1 == 0 && x2 == 0 && y1 == 0 && y2 == 0) ||
      (TP_CalcCalibrateParam(calibrate, x1, y1, dx1, dy1, x2, y2, dx2, dy2) !=
       0)) {
...
u16 TP_GetLatestIndexInAuto(void) { return tpState.index; }
...
#define IN_S16_RANGE(x) ((x) < 0x8000 && (x) >= -0x8000)
  ...
  CP_SetDiv32_32(((u32)rx_width) << TP_CALIBRATE_DOT_SCALE_SHIFT,
                 (u32)dx_width);
  ...
  CP_SetDiv32_32(((u32)ry_width) << TP_CALIBRATE_DOT_SCALE_SHIFT,
                 (u32)dy_width);
  ...
  tmp32 = (s16)((((s32)(raw_x1 + raw_x2) << TP_CALIBRATE_DOT_SCALE_SHIFT) -
                 ((s32)(dx1 + dx2) * calibrate->xDotSize)) >>
                (TP_CALIBRATE_DOT_SCALE_SHIFT -
                 TP_CALIBRATE_ORIGIN_SCALE_SHIFT + 1));
  if (!IN_S16_RANGE(tmp32)) {
    (void)OS_RestoreInterrupts(enabled);
    return 1;
  }
  ...
  tmp32 = (s16)((((s32)(raw_y1 + raw_y2) << TP_CALIBRATE_DOT_SCALE_SHIFT) -
                 ((s32)(dy1 + dy2) * calibrate->yDotSize)) >>
                (TP_CALIBRATE_DOT_SCALE_SHIFT -
                 TP_CALIBRATE_ORIGIN_SCALE_SHIFT + 1));
  if (!IN_S16_RANGE(tmp32)) {
    return 1;
...
void TP_GetCalibratedPoint(TPData *disp, const TPData *raw) {
  ...
  enum { MAX_X = GX_LCD_SIZE_X - 1, MAX_Y = GX_LCD_SIZE_Y - 1 };
  ...
  disp->x =
      (u16)((((u64)(raw->x << TP_CALIBRATE_ORIGIN_SCALE_SHIFT) - cal->x0) *
             cal->xDotSizeInv) >>
            (TP_CALIBRATE_DOT_INV_SCALE_SHIFT +
  ...
  disp->y =
      (u16)((((u64)(raw->y << TP_CALIBRATE_ORIGIN_SCALE_SHIFT) - cal->y0) *
             cal->yDotSizeInv) >>
            (TP_CALIBRATE_DOT_INV_SCALE_SHIFT +
             TP_CALIBRATE_ORIGIN_SCALE_SHIFT));
...
void TP_GetUnCalibratedPoint(u16 *raw_x, u16 *raw_y, u16 dx, u16 dy) {
  ...
  *raw_x = (u16)((((s32)(dx * cal->xDotSize) >>
                   TP_CALIBRATE_DOT2ORIGIN_SCALE_SHIFT) +
                  (s32)cal->x0) >>
                 TP_CALIBRATE_ORIGIN_SCALE_SHIFT);
  *raw_y = (u16)((((s32)(dy * cal->yDotSize) >>
                   TP_CALIBRATE_DOT2ORIGIN_SCALE_SHIFT) +
                  (s32)cal->y0) >>
                 TP_CALIBRATE_ORIGIN_SCALE_SHIFT);
}
