...
#define WBT_COMMAND_REQ(type, pair_type, argument)                             \
  {(u16)WBT_CMD_REQ_##type, (u16)WBT_CMD_##pair_type,                          \
   sizeof(WBTPacketHeaderFormat) + argument, TRUE, FALSE}
#define WBT_COMMAND_RES(type, pair_type, argument)                             \
  {(u16)WBT_CMD_RES_##type, (u16)WBT_CMD_##pair_type,                          \
   sizeof(WBTPacketHeaderFormat) + argument, FALSE, TRUE}
#define WBT_COMMAND_MSG(type) {(u16)WBT_CMD_##type, 0, 0, FALSE, FALSE}
static const struct {
  ...
  u32 is_res : 1;    /* TRUE if WBT_CMD_RES_* */
    ...
    WBT_COMMAND_REQ(GET_BLOCK, RES_GET_BLOCK,
                    sizeof(WBTPacketRequestGetBlockFormat)),
    WBT_COMMAND_RES(GET_BLOCK, REQ_GET_BLOCK,
                    sizeof(WBTPacketResponseGetBlockFormat)),
    WBT_COMMAND_REQ(GET_BLOCKINFO, RES_GET_BLOCKINFO,
                    sizeof(WBTPacketRequestGetBlockFormat)),
    WBT_COMMAND_RES(GET_BLOCKINFO, REQ_GET_BLOCKINFO,
                    sizeof(WBTPacketResponseGetBlockFormat)),
    WBT_COMMAND_REQ(GET_BLOCK_DONE, RES_GET_BLOCK_DONE,
                    sizeof(WBTPacketRequestGetBlockDoneFormat)),
    WBT_COMMAND_RES(GET_BLOCK_DONE, REQ_GET_BLOCK_DONE,
                    sizeof(WBTPacketResponseGetBlockDoneFormat)),
    WBT_COMMAND_REQ(USER_DATA, RES_USER_DATA,
                    sizeof(WBTPacketRequestUserDataFormat)),
...
SDK_INLINE int div32(int a) { return (a >> 5); }

SDK_INLINE int mod32(int a) { return (a & 0x1f); }

PLATFORM_ATTRIBUTE_INLINE void WBTi_CopySafeMemory(const void *src, void *dst,
                                                   u32 len) {
  if (!src) {
...
static void WBTi_InitBitmap(WBTContext *work, WBTPacketBitmap *pkt_bmp,
                            s32 length, u32 *bits, u8 *buffer, int packet) {
  SDK_ASSERT(packet > 0);
...
static BOOL WBTi_MergeBitmapIndex(WBTPacketBitmap *pkt_bmp, int index,
                                  u32 packet, const void *src) {
  BOOL retval = FALSE;
...
static void WBTi_NotifySystemCallback(WBTContext *work, WBTCommandType event,
...
PLATFORM_ATTRIBUTE_INLINE
WBTPacketFormat *WBTi_MakeCommandHeader(void *dst, u8 cmd, int target,
                                        u8 counter) {
...
static int WBTi_TryCreateResponse(WBTContext *work, int aid, void *buf,
  ...
  if (command == WBT_CMD_REQ_SYNC) {
    WBTPacketFormat *format = WBTi_MakeCommandHeader(
        buf, WBT_CMD_RES_SYNC, (1 << aid), token->token_peer_cmd_counter);

    MI_StoreLE16(&format->res_sync.block_total,
                 (u16)WBT_GetRegisteredCount(work));
    MI_StoreLE16(&format->res_sync.peer_packet, (u16)work->my_data_packet_size);
    MI_StoreLE16(&format->res_sync.own_packet,
                 (u16)work->peer_data_packet_size);
    ret_size =
        (int /* Temporary */)sizeof(format->header) + sizeof(format->res_sync);
    done = TRUE;
    ...
    ret_size = (int /* Temporary */)sizeof(format->header) +
               sizeof(format->res_getblock_done);
    done = TRUE;
    WBT_DEBUG_OUTPUT1("send BlockDone to %d id = %d\n", aid, id);
  } else if (command == WBT_CMD_REQ_USER_DATA) {
    WBTPacketFormat *format = WBTi_MakeCommandHeader(
        buf, WBT_CMD_RES_USER_DATA, (1 << aid), token->token_peer_cmd_counter);
    ret_size = (int /* Temporary */)sizeof(format->header);
    done = TRUE;
    ...
    if (list) {
      ...
      if (id >= WBT_NUM_MAX_BLOCK_INFO_ID) {
        ...
        if (id == work->last_block_id) {
          const int index_total =
              (list->data_info.block_size + work->my_data_packet_size - 1) /
              work->my_data_packet_size;
          int i;

          for (i = 0; (i < 3) && ((index == work->last_seq_no_1) ||
                                  (index == work->last_seq_no_2));
               ++i) {
...

      {
        ...
        if (list->data_ptr) {
          ...
          WBT_DEBUG_OUTPUT1(
              "send BlockData to %d id = %d seq no = %d pktsize %d\n", aid, id,
              index, packet);
          sendable = TRUE;
        ...
        if (sendable) {
          ...
          u16 target =
              (u16)((WBT_GetAid(work) == WBT_AID_PARENT) ? 0xFFFE : 0x0001);
          WBTPacketFormat *format =
          ...
          ret_size = (int /* Temporary */)(
              sizeof(format->header) + sizeof(format->res_getblock) + packet);
          done = TRUE;
        }
      }
    }
    ...
    if (foroce_blockinfo || data_ptr) {
      ...
      WBTPacketFormat *format =
          WBTi_MakeCommandHeader(buf, WBT_CMD_RES_GET_BLOCKINFO, target,
                                 token->token_peer_cmd_counter);
      MI_StoreLE32(&format->res_getblock.id, id);
...
static int WBTi_CheckRequest(WBTContext *work, void *send_buf, int size) {
  ...
  if (current) {
    ...
    case WBT_CMD_REQ_SYNC: {
      WBTPacketFormat *format =
          WBTi_MakeCommandHeader(send_buf, WBT_CMD_REQ_SYNC,
                                 current->target_bmp, current->my_cmd_counter);

      MI_StoreLE16(&format->req_sync.peer_packet,
                   (u16)work->my_data_packet_size);
      MI_StoreLE16(&format->req_sync.own_packet,
                   (u16)work->peer_data_packet_size);
      send_size = (int /* Temporary */)sizeof(format->header) +
                  sizeof(format->req_sync);
      ...
      for (aid = 0; aid < 16; ++aid) {
        ...
        if ((current->target_bmp & (1 << aid)) != 0) {
          ...
          if (next_seq_no == -1) {
            ...
            WBTPacketFormat *format = WBTi_MakeCommandHeader(
                send_buf, current->command, current->target_bmp,
            ...
            WBT_DEBUG_OUTPUT1("send ReqBlock id=%d seq=%d\n",
                              current->get.block_id, next_seq_no);
          }
          if (send_size) {
            break;
          }
    ...
    case WBT_CMD_REQ_USER_DATA: {
      WBTPacketFormat *format =
          WBTi_MakeCommandHeader(send_buf, WBT_CMD_REQ_USER_DATA,
                                 current->target_bmp, current->my_cmd_counter);
    ...
    do {
      ...
      if ((work->req_bitmap & (1 << aid)) != 0) {
        ...
        if (WBT_CMD_REQ_GET_BLOCK == token->token_command) {
          ...
          retval = WBTi_TryCreateResponse(work, aid, buffer, length,
                                          WBT_CMD_REQ_GET_BLOCK, FALSE);
        }
        if (retval) {
    ...
    do {
      ...
      if ((work->req_bitmap & (1 << aid)) != 0) {
        ...
        if (WBT_CMD_REQ_GET_BLOCKINFO == token->token_command) {
          retval = WBTi_TryCreateResponse(work, aid, buffer, length,
                                          WBT_CMD_REQ_GET_BLOCKINFO, TRUE);
        }
        if (retval) {
          work->last_target_aid = aid;
        }
...
int WBT_CallPacketSendHook(WBTContext *work, void *buffer, int length,
                           BOOL is_parent) {
  int retval = 0;
...

  {
    ...
    static const WBTCommandType tbl[] = {
        WBT_CMD_REQ_USER_DATA,      WBT_CMD_REQ_SYNC, WBT_CMD_REQ_GET_BLOCKINFO,
        WBT_CMD_REQ_GET_BLOCK_DONE, WBT_CMD_REQ_NONE,
    };
        ...
        if ((mask & (1 << aid)) != 0) {
          ...
          for (i = 0; !retval && (tbl[i] != WBT_CMD_REQ_NONE); ++i) {
            ...
            if (tbl[i] == token->token_command) {
              retval = WBTi_TryCreateResponse(work, aid, buffer, length, tbl[i],
                                              FALSE);
            }
          }
          if (retval) {
  ...
  if (!retval) {
    ...
    if (!retval) {
      ...
      WBTPacketFormat *format = WBTi_MakeCommandHeader(
          buffer, WBT_CMD_REQ_WAIT, (u16)mask, (WBTCommandCounter)0);
...
static void WBTi_NotifyCompletionCallback(WBTContext *work,
                                          WBTCommandType event, int aid) {
...
void WBT_CallPacketRecvHook(WBTContext *work, int aid, const void *buffer,
                            int length) {
  WBTRecvToken *token = &work->peer_param[aid].recv_token;
  ...
  if (buffer && (length >= sizeof(WBTPacketHeaderFormat))) {
    ...
    if ((WBT_GetAid(work) != -1) && ((bitmap & (1 << WBT_GetAid(work))) != 0)) {
      ...
      if (command >= WBT_COMMAND_MAX) {
        ...
        if (command == WBT_CMD_REQ_WAIT) {
          ...
          cb->peer_packet_size =
              (s16)MI_LoadLE16(&format->req_sync.peer_packet);
          cb->my_packet_size = (s16)MI_LoadLE16(&format->req_sync.own_packet);
          cb->num_of_list = 0; /* In the old specifications, this is the only
                                  member not included in a request */
          ...
          token->token_block_seq_no =
              (s32)MI_LoadLE32(&format->req_getblock.index);
          work->req_bitmap |= (1 << aid);
          WBT_DEBUG_OUTPUT1("get req Block from %d id = %d seq no = %d\n", aid,
                            token->token_block_id, token->token_block_seq_no);
        }
    ...
    if ((work->req_bitmap & (1 << aid)) != 0) {
      ...
      if ((command != WBT_CMD_REQ_GET_BLOCK) &&
          (command != WBT_CMD_REQ_GET_BLOCKINFO)) {
        WBTi_NotifySystemCallback(work, (WBTCommandType)command, aid,
                                  WBT_RESULT_SUCCESS);
      }
    }
  }
...
void WBT_InitContext(WBTContext *work, void *userdata,
...
int WBT_GetBitmapLength(const WBTContext *work, int length) {
  ...
  SDK_ASSERT(packet > 0);
  return (int)(sizeof(u32) *
               MATH_ROUNDUP(((length + packet - 1) / packet), 32));
}

void WBT_GetDownloadProgress(const WBTContext *work, u32 id, int aid,
                             int *count, int *total) {
  const WBTCommand *current = WBT_GetCurrentCommand(work);
  if ((current != NULL) && (current->command == WBT_CMD_REQ_GET_BLOCK) &&
      (current->get.block_id == id)) {
...
BOOL WBT_RegisterBlockInfo(WBTContext *work, WBTBlockInfoList *list, u32 id,
                           const void *userinfo, const void *buffer,
                           int length) {
  PLATFORM_ENTER_CRITICALSECTION();
  {
    WBTBlockInfoList **pp;
    ...
    *pp = list;
    ...
    list->block_type =
        (u16)(buffer ? WBT_BLOCK_LIST_TYPE_COMMON : WBT_BLOCK_LIST_TYPE_USER);
  }
  PLATFORM_LEAVE_CRITICALSECTION();
