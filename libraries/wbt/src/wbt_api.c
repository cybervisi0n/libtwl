...
void WBT_InitParent(int send_packet_size, int recv_packet_size,
  ...
  if (!wbt_initialize_flag) {
    ...
    WBT_AddCommandPool(wbti_command_work, cmd_q,
                       sizeof(cmd_q) / sizeof(*cmd_q));
...
BOOL WBT_GetCurrentDownloadProgress(u32 block_id, int aid, int *current_count,
                                    int *total_count) {
  WBT_GetDownloadProgress(wbti_command_work, block_id, aid, current_count,
                          total_count);
  return (*total_count != 0);
}

BOOL WBT_SetPacketSize(int send_packet_size, int recv_packet_size) {
  return WBT_SetPacketLength(wbti_command_work, send_packet_size,
                             recv_packet_size);
}
...
BOOL WBT_RegisterBlock(WBTBlockInfoList *block_info_list, WBTBlockId block_id,
  ...
  return WBT_RegisterBlockInfo(wbti_command_work, block_info_list, block_id,
                               user_id, data_ptr, data_size);
}
...
BOOL WBT_GetBlockInfo(WBTAidBitmap target, int block_info_no,
                      WBTBlockInfoTable *block_info_table,
                      WBTCallback callback) {
  WBTCommandList *list = WBT_AllocCommandList(wbti_command_work);
  if (list) {
    list->command.callback = callback;
    WBT_CreateCommandINFO(wbti_command_work, list, block_info_no,
                          block_info_table);
    WBT_PostCommand(wbti_command_work, list, target, NULL);
  }
  return (list != NULL);
...
BOOL WBT_GetBlock(WBTAidBitmap target, WBTBlockId block_id,
                  WBTRecvBufTable *recv_buf_table, u32 recv_size,
                  WBTPacketBitmapTable *p_bmp_table, WBTCallback callback) {
  WBTCommandList *list = WBT_AllocCommandList(wbti_command_work);
  if (list) {
    list->command.callback = callback;
    WBT_CreateCommandGET(wbti_command_work, list, block_id, recv_size,
                         recv_buf_table, p_bmp_table);
    WBT_PostCommand(wbti_command_work, list, target, NULL);
  }
  return (list != NULL);
}

BOOL WBT_PutUserData(WBTAidBitmap target, const void *user_data, int size,
                     WBTCallback callback) {
  WBTCommandList *list = WBT_AllocCommandList(wbti_command_work);
  if (list) {
    list->command.callback = callback;
