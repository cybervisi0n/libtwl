...
static s32 sDayOfYear[12] = {0,   31,  59,  90,  120, 151,
                             181, 212, 243, 273, 304, 334};

static inline BOOL RTCi_IsLeapYear(u32 year) { return !((year & 0x03)); }

s32 RTC_ConvertDateToDay(const RTCDate *date) {
  s32 dayNum;

  if (date->year >= 100 || (date->month < 1) || (date->month > 12) ||
...
RTCWeek RTC_GetDayOfWeek(RTCDate *date) {
  ...
  cent = year / 100;
  year %= 100;
  return (RTCWeek)(((26 * month - 2) / 10 + day + year + year / 4 + cent / 4 +
                    5 * cent) %
                   7);
}
