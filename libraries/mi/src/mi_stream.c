...
s32 MIi_InitReadStreamFromMemory(const u8 *srcp, void *destp,
                                 const void *paramp);
...
static MIReadStreamCallbacks readMemoryStreamCallbacks = {
    MIi_InitReadStreamFromMemory, MIi_TerminateReadStreamFromMemory,
    MIi_ReadByteStreamFromMemory, MIi_ReadShortStreamFromMemory,
...
u8 MIi_ReadByteStreamFromMemory(const u8 *srcp) { return *srcp; }

u16 MIi_ReadShortStreamFromMemory(const u8 *srcp) { return *(u16 *)srcp; }

u32 MIi_ReadWordStreamFromMemory(const u8 *srcp) { return *(u32 *)srcp; }
