...
#define PushCommand(c, a0, a1, a2, a3)                                         \
...
static void PushCommand_impl(SNDCommandID command, u32 arg0, u32 arg1, u32 arg2,
                             u32 arg3);

void SND_StartSeq(int playerNo, const void *base, u32 offset,
...
void SND_PrepareSeq(int playerNo, const void *base, u32 offset,
                    const struct SNDBankData *bank) {
  SDK_MINMAX_ASSERT(playerNo, SND_PLAYER_MIN, SND_PLAYER_MAX);
  SDK_NULL_ASSERT(base);
  SDK_NULL_ASSERT(bank);
...
void SND_SetPlayerTempoRatio(int playerNo, int ratio) {
  ...
  SNDi_SetPlayerParam(playerNo, GetOffset(SNDPlayer, tempo_ratio), (u32)ratio,
                      GetSize(SNDPlayer, tempo_ratio));
}
...
void SND_SetPlayerVolume(int playerNo, int volume) {
  ...
  SNDi_SetPlayerParam(playerNo, GetOffset(SNDPlayer, extFader), (u32)volume,
                      GetSize(SNDPlayer, extFader));
}
...
void SND_SetPlayerChannelPriority(int playerNo, int prio) {
  ...
  SNDi_SetPlayerParam(playerNo, GetOffset(SNDPlayer, prio), (u32)prio,
                      GetSize(SNDPlayer, prio));
}
...
void SND_SetTrackVolume(int playerNo, u32 trackBitMask, int volume) {
  ...
  SNDi_SetTrackParam(playerNo, trackBitMask, GetOffset(SNDTrack, extFader),
                     (u32)volume, GetSize(SNDTrack, extFader));
}
...
void SND_SetTrackPitch(int playerNo, u32 trackBitMask, int pitch) {
  ...
  SNDi_SetTrackParam(playerNo, trackBitMask, GetOffset(SNDTrack, ext_pitch),
                     (u32)pitch, GetSize(SNDTrack, ext_pitch));
}
...
void SND_SetTrackPan(int playerNo, u32 trackBitMask, int pan) {
  ...
  SNDi_SetTrackParam(playerNo, trackBitMask, GetOffset(SNDTrack, ext_pan),
                     (u32)pan, GetSize(SNDTrack, ext_pan));
}
...
void SND_SetTrackPanRange(int playerNo, u32 trackBitMask, int panRange) {
  ...
  SNDi_SetTrackParam(playerNo, trackBitMask, GetOffset(SNDTrack, pan_range),
                     (u32)panRange, GetSize(SNDTrack, pan_range));
}
...
void SND_SetTrackModDepth(int playerNo, u32 trackBitMask, int depth) {
  ...
  SNDi_SetTrackParam(playerNo, trackBitMask, GetOffset(SNDTrack, mod.depth),
                     (u32)depth, GetSize(SNDTrack, mod.depth));
}
...
void SND_SetTrackModSpeed(int playerNo, u32 trackBitMask, int speed) {
  ...
  SNDi_SetTrackParam(playerNo, trackBitMask, GetOffset(SNDTrack, mod.speed),
                     (u32)speed, GetSize(SNDTrack, mod.speed));
}

void SND_SetTrackAllocatableChannel(int playerNo, u32 trackBitMask,
                                    u32 chBitMask) {
  SDK_MINMAX_ASSERT(playerNo, SND_PLAYER_MIN, SND_PLAYER_MAX);
  SDK_MINMAX_ASSERT(chBitMask, 0, 0xffff);

  PushCommand(SND_COMMAND_ALLOCATABLE_CHANNEL, playerNo, trackBitMask,
              chBitMask, 0);
}

void SND_StartTimer(u32 chBitMask, u32 capBitMask, u32 alarmBitMask,
                    u32 flags) {
  PushCommand(SND_COMMAND_START_TIMER, chBitMask, capBitMask, alarmBitMask,
              flags);
}
...
void SND_StopTimer(u32 chBitMask, u32 capBitMask, u32 alarmBitMask, u32 flags) {
  ...
  for (alarmNo = 0; alarmNo < SND_ALARM_NUM && mask != 0;
       alarmNo++, mask >>= 1) {
    if ((mask & 0x01) == 0)
      continue;
  ...
  PushCommand(SND_COMMAND_STOP_TIMER, chBitMask, capBitMask, alarmBitMask,
              flags);
}

void SND_SetupCapture(SNDCapture capture, SNDCaptureFormat format,
                      void *buffer_p, u32 length, BOOL loopFlag,
                      SNDCaptureIn in, SNDCaptureOut out) {
  PushCommand(
      SND_COMMAND_SETUP_CAPTURE, buffer_p, length,
      capture << 31 | format << 30 | loopFlag << 29 | in << 28 | out << 27, 0);
}

void SND_SetupAlarm(int alarmNo, u32 tick, u32 period, SNDAlarmHandler handler,
                    void *arg) {
...
void SND_SetChannelVolume(u32 chBitMask, int volume,
...
void SND_SetupChannelPcm(int chNo, SNDWaveFormat format, const void *dataaddr,
                         SNDChannelLoop loop, int loopStart, int loopLen,
                         int volume, SNDChannelDataShift shift, int timer,
                         int pan) {
  PushCommand(SND_COMMAND_SETUP_CHANNEL_PCM, timer << 16 | chNo, dataaddr,
              volume << 24 | shift << 22 | loopLen,
              loop << 26 | format << 24 | pan << 16 | loopStart);
}

void SND_SetupChannelPsg(int chNo, SNDDuty duty, int volume,
                         SNDChannelDataShift shift, int timer, int pan) {
  PushCommand(SND_COMMAND_SETUP_CHANNEL_PSG, chNo, shift << 8 | volume,
              timer << 8 | pan, duty);
}

void SND_SetupChannelNoise(int chNo, int volume, SNDChannelDataShift shift,
                           int timer, int pan) {
  PushCommand(SND_COMMAND_SETUP_CHANNEL_NOISE, chNo, shift << 8 | volume,
              timer << 8 | pan, 0);
}
...
void SND_SetOutputSelector(SNDOutput left, SNDOutput right,
                           SNDChannelOut channel1, SNDChannelOut channel3) {
...
void SNDi_SetTrackParam(int playerNo, u32 trackBitMask, u32 offset, u32 data,
                        int size) {
  PushCommand(SND_COMMAND_TRACK_PARAM, size << 24 | playerNo, trackBitMask,
              offset, data);
}
...
static void PushCommand_impl(SNDCommandID id, u32 arg0, u32 arg1, u32 arg2,
                             u32 arg3) {
