...
static u8 MI_ReadByte(const void *address) { return *(u8 *)address; }
...
void SND_AssignWaveArc(SNDBankData *bank, int index, SNDWaveArc *waveArc) {
  ...
  SDK_TASSERTMSG(((u32)bank & 0x1f) == 0,
                 "bank address must be aligned 32 bytes boundary.");
  SDK_TASSERTMSG(((u32)waveArc & 0x1f) == 0,
                 "waveArc address must be aligned 32 bytes boundary.");
#endif
  ...
  if (bank->waveArcLink[index].waveArc != NULL) {
    ...
    if (&bank->waveArcLink[index] ==
        bank->waveArcLink[index].waveArc->topLink) {
...
BOOL SND_ReadInstData(const SNDBankData *bank, int prgNo, int key,
                      SNDInstData *inst) {
  u32 instOffset;
  ...
  case SND_INST_KEY_SPLIT: {
    int index = 0;
    const SNDKeySplit *keySplit =
        (const SNDKeySplit *)((u8 *)bank + instOffset);
...
BOOL SND_WriteInstData(SNDBankData *bank, int prgNo, int key,
...
BOOL SND_GetNextInstData(const SNDBankData *bank, SNDInstData *inst,
                         SNDInstPos *pos) {
  SDK_NULL_ASSERT(bank);
  SDK_NULL_ASSERT(inst);
  SDK_NULL_ASSERT(pos);
  ...
  for (; pos->prgNo < bank->instCount; pos->prgNo++, pos->index = 0) {
    ...
    case SND_INST_DIRECTPCM:
    case SND_INST_NULL: {
      const SNDInstParam *param =
          (const SNDInstParam *)((u8 *)bank + instOffset);
      inst->param = *param;
      pos->prgNo++;
      return TRUE;
    }
...
void SND_SetWaveDataAddress(SNDWaveArc *waveArc, int index,
...
const SNDWaveData *SND_GetWaveDataAddress(const SNDWaveArc *waveArc,
                                          int index) {
...
BOOL SND_NoteOn(SNDExChannel *ch_p, int key, int velocity, s32 length,
  ...
  case SND_INST_DIRECTPCM:
    if (inst->type == SND_INST_PCM) {
      ...
      wave_data = (const SNDWaveData *)(inst->param.wave[1] << 16 |
                                        inst->param.wave[0]);
    }

    if (wave_data != NULL) {
      result = SND_StartExChannelPcm(ch_p, &wave_data->param,
                                     wave_data->samples, length);
    } else {
      result = FALSE;
    }
    break;
