...
__inline
#endif
    const void *ConvAddr(const void *p, const void *baseFrom,
                         const void *baseTo) {
  if (p == NULL)
    return NULL;
...
BOOL SND_ReadChannelInfo(const SNDDriverInfo *driverInfo, int chNo,
  ...
  if (chInfo->lockFlag) {
    ...
    {
      const u8 volume = (u8)((chCtrl & REG_SND_SOUND0CNT_VOLUME_MASK) >>
                             REG_SND_SOUND0CNT_VOLUME_SHIFT);
    ...
    chInfo->pan = (u8)((chCtrl & REG_SND_SOUND0CNT_PAN_MASK) >>
                       REG_SND_SOUND0CNT_PAN_SHIFT);
  } else {
...
BOOL SND_ReadTrackInfo(const SNDDriverInfo *driverInfo, int playerNo,
                       int trackNo, SNDTrackInfo *trackInfo) {
  const SNDPlayer *player;
  ...
  {
    const SNDExChannel *chp = (const SNDExChannel *)ConvAddr(
        track->channel_list, driverInfo->workAddress, &driverInfo->work);
    ...
    while (chp != NULL) {
      ...
      chp = (const SNDExChannel *)ConvAddr(
          chp->nextLink, driverInfo->workAddress, &driverInfo->work);
    }
  }
