...
#define UNPACK_COMMAND(arg, shift, bit)                                        \
  (((arg) >> (shift)) & ((1 << (bit)) - 1))

#if defined(SDK_ARM9) || defined(SDK_PORT)

static SNDCommand
    sCommandArray[SND_COMMAND_NUM] ATTRIBUTE_ALIGN(32); /* Command array */
...
static void SetChannelVolume(u32 chBitMask, int volume,
                             SNDChannelDataShift shift);
static void SetChannelPan(u32 chBitMask, int pan);
static void StartTimer(u32 chBitMask, u32 capBitMask, u32 alarmBitMask,
                       u32 flags);
static void StopTimer(u32 chBitMask, u32 capBitMask, u32 alarmBitMask,
                      u32 flags);
...
#ifdef SDK_ARM7
  OS_InitMessageQueue(&sCommandMesgQueue, sCommandMesgBuffer,
...
BOOL SND_FlushCommand(u32 flags) {
  ...
  if (PXI_SendWordByFifo(PXI_FIFO_TAG_SOUND, (u32)sReserveList, FALSE) < 0) {
    ...
    while (sWaitingCommandListCount >= SND_PXI_FIFO_MESSAGE_BUFSIZE ||
           PXI_SendWordByFifo(PXI_FIFO_TAG_SOUND, (u32)sReserveList, FALSE) <
               0) {
  ...
  while (OS_ReceiveMessage(&sCommandMesgQueue, &message, OS_MESSAGE_NOBLOCK)) {
    ...
    while (command_p != NULL) {
      ...
      case SND_COMMAND_START_SEQ:
        SND_StartSeq((int)command.arg[0], (const void *)command.arg[1],
      ...
      case SND_COMMAND_PREPARE_SEQ:
        SND_PrepareSeq((int)command.arg[0], (const void *)command.arg[1],
                       command.arg[2], (struct SNDBankData *)command.arg[3]);
        break;
      ...
      case SND_COMMAND_PLAYER_PARAM:
        SNDi_SetPlayerParam((int)command.arg[0], command.arg[1], command.arg[2],
                            (int)command.arg[3]);
        break;

      case SND_COMMAND_TRACK_PARAM:
        SNDi_SetTrackParam((int)UNPACK_COMMAND(command.arg[0], 0, 24),
                           command.arg[1], command.arg[2], command.arg[3],
                           (int)UNPACK_COMMAND(command.arg[0], 24, 8));
        break;

      case SND_COMMAND_MUTE_TRACK:
        SND_SetTrackMute((int)command.arg[0], command.arg[1],
                         (SNDSeqMute)command.arg[2]);
        break;

      case SND_COMMAND_ALLOCATABLE_CHANNEL:
        SND_SetTrackAllocatableChannel((int)command.arg[0], command.arg[1],
                                       command.arg[2]);
        break;

      case SND_COMMAND_PLAYER_LOCAL_VAR:
        SND_SetPlayerLocalVariable((int)command.arg[0], (int)command.arg[1],
                                   (s16)command.arg[2]);
        break;
      ...
      case SND_COMMAND_START_TIMER:
        StartTimer(command.arg[0], command.arg[1], command.arg[2],
                   command.arg[3]);
        break;

      case SND_COMMAND_STOP_TIMER:
        StopTimer(command.arg[0], command.arg[1], command.arg[2],
                  command.arg[3]);
        break;
      ...
      case SND_COMMAND_SETUP_CAPTURE:
        SND_SetupCapture(
            ...
            (void *)command.arg[0], (int)command.arg[1],
      ...
      case SND_COMMAND_SETUP_ALARM:
        SND_SetupAlarm((int)command.arg[0], (OSTick)command.arg[1],
                       (OSTick)command.arg[2], (int)command.arg[3]);
        break;
      ...
      case SND_COMMAND_CHANNEL_VOLUME:
        SetChannelVolume((u32)command.arg[0], (int)command.arg[1],
      ...
      case SND_COMMAND_SETUP_CHANNEL_PCM:
        SND_SetupChannelPcm(
            ...
            (SNDChannelDataShift)UNPACK_COMMAND(command.arg[2], 22,
                                                2),      /* SHIFT */
      ...
      case SND_COMMAND_SETUP_CHANNEL_PSG:
        SND_SetupChannelPsg(
            ...
            (SNDChannelDataShift)UNPACK_COMMAND(command.arg[1], 8,
                                                2),     /* SHIFT */
      ...
      case SND_COMMAND_OUTPUT_SELECTOR:
        SND_SetOutputSelector(
            (SNDOutput)command.arg[0], (SNDOutput)command.arg[1],
      ...
      case SND_COMMAND_INVALIDATE_SEQ:
        SND_InvalidateSeq((const void *)command.arg[0],
                          (const void *)command.arg[1]);
        break;

      case SND_COMMAND_INVALIDATE_BANK:
        SND_InvalidateBank((const void *)command.arg[0],
                           (const void *)command.arg[1]);
        break;

      case SND_COMMAND_INVALIDATE_WAVE:
        SND_InvalidateWave((const void *)command.arg[0],
                           (const void *)command.arg[1]);
        break;
...
#pragma unused(result)
  ...
  if (data >= HW_MAIN_MEM) {
    ...
    result =
        OS_SendMessage(&sCommandMesgQueue, (OSMessage)data, OS_MESSAGE_NOBLOCK);
...
static void RequestCommandProc(void) {
  while (PXI_SendWordByFifo(PXI_FIFO_TAG_SOUND, SND_MSG_REQUEST_COMMAND_PROC,
                            FALSE) < 0) {
  }
}
