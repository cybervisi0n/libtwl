  ...
  case WBT_CMD_SYSTEM_CALLBACK:
...

    {
      ...
      const WFSMessageFormat *const msg =
          (const WFSMessageFormat *)uc->user_data.data;
...

      {
        ...
        WFS_DEBUG_OUTPUT(("WBT-MSG(LOCK):recv [offset=0x%08X, length=0x%08X] "
                          "(AID=%d, packet=%d)",
                          offset, length, aid, packet));

        if (work->is_changing ||
            (packet !=
             WBT_GetParentPacketLength(work->wbt) + WBT_PACKET_SIZE_MIN)) {
          ...
          work->recv_msg[aid].arg1 =
              MI_HToLE32((u32)(WFS_LOCKED_BLOCK_INDEX + index));
        }
...

      {
        ...
        PLATFORM_LEAVE_CRITICALSECTION();
        WFS_DEBUG_OUTPUT(
            ("WBT-MSG(UNLOCK):recv [id=0x%08X] (AID=%d)", id, aid));
      } break;
      }
    } break;
...
void WFS_CallServerConnectHook(WFSServerContext *context,
...
void WFS_CallServerDisconnectHook(WFSServerContext *context,
                                  const WFSPeerInfo *peer) {
...
void WFS_CallServerPacketSendHook(WFSServerContext *context,
  ...
  if (!context->msg_busy) {
    ...
    if (context->is_changing && !context->use_bitmap) {
      ...
      (void)WBT_SetPacketLength(context->wbt, context->new_packet,
                                WBT_PACKET_SIZE_MIN);

      if (context->deny_bitmap) {
        WFS_DEBUG_OUTPUT(
            ("WBT-MSG(LOCK):deny [packet-length renewal] (BITMAP=%d)",
             context->deny_bitmap));
        (void)WFS_SendMessageLOCK_ACK(context->wbt, WFSi_WBTCallback,
      ...
      if (msg) {
        ...
        case WFS_MSG_LOCK_REQ:
          ...
          break;
        case WFS_MSG_UNLOCK_REQ:
          (void)WFS_SendMessageUNLOCK_ACK(context->wbt, WFSi_WBTCallback,
                                          bitmap, MI_LEToH32(msg->arg1));
...
void WFS_CallServerPacketRecvHook(WFSServerContext *context,
...
void WFS_InitServer(WFSServerContext *context, void *userdata,
...
BOOL WFS_RegisterServerTable(WFSServerContext *context, MIDevice *device,
                             u32 fatbase, u32 overlay) {
  BOOL retval = FALSE;
  ...
  else if (WFS_LoadTable(context->table, context->allocator, device, fatbase,
                         overlay)) {

    (void)WBT_RegisterBlockInfo(
        context->wbt, context->table_info, WFS_TABLE_BLOCK_INDEX, NULL,
        context->table->buffer, (int)context->table->length);
    retval = TRUE;
  }
  return retval;
}
