...
typedef struct WFSServerThread {
  ...
  FSFile file[1];
  MICache cache[1];
  u8 cache_buf[MI_CACHE_BUFFER_WORKSIZE(WFS_FILE_CACHE_SIZE,
                                        WFS_CACHE_PAGE_TOTAL)];
...
static int WFSi_ReadRomCallback(void *userdata, void *buffer, u32 offset,
                                u32 length) {
...
static int WFSi_WriteRomCallback(void *userdata, const void *buffer, u32 offset,
                                 u32 length) {
  (void)userdata;
  (void)buffer;
  (void)offset;
...
static void WFSi_ServerThreadProc(void *arg) {
  ...
  for (;;) {
    BOOL busy = FALSE;
    (void)OS_ReceiveMessage(thread->msg_queue, (OSMessage *)&busy,
                            OS_MESSAGE_BLOCK);
    if (!busy) {
      break;
    }
...
static void WFSi_ThreadHook(void *work, void *argument) {
  ...
  else if (!MI_ReadCache(thread->cache, segment->buffer, segment->offset,
                         segment->length)) {

    segment->buffer = NULL; /* == "could not prepare immediately" */

    (void)OS_SendMessage(thread->msg_queue, (OSMessage)TRUE,
                         OS_MESSAGE_NOBLOCK);
  }
}

BOOL WFS_ExecuteRomServerThread(WFSServerContext *context, FSFile *file,
                                BOOL sharedFS) {
  BOOL retval = FALSE;

  WFSServerThread *thread =
      MI_CallAlloc(context->allocator, sizeof(WFSServerThread), 32);
  if (!thread) {
    ...
    MI_InitDevice(thread->device, thread, WFSi_ReadRomCallback,
                  WFSi_WriteRomCallback);
    MI_InitCache(thread->cache, WFS_FILE_CACHE_SIZE, thread->cache_buf,
                 sizeof(thread->cache_buf));
