...
static void WFSi_ReallocBitmap(WFSClientContext *context, int length) {
  ...
  if (context->max_file_size < length) {
    ...
    context->recv_pkt_bmp_buf = (u32 *)MI_CallAlloc(
        context->allocator, sizeof(u32) * newsize, sizeof(u32));
    if (context->recv_pkt_bmp_buf == NULL) {
      OS_TPanic("cannot allocate bitmap buffer %d BYTEs!",
                sizeof(u32) * newsize);
    }
    context->recv_buf_packet_bmp_table.packet_bitmap[0] =
        context->recv_pkt_bmp_buf;
  }
...
static void WFSi_ReceiveTableSequence(void *userdata, WBTCommand *callback) {
  ...
  if ((callback == NULL) || (callback->result == WBT_RESULT_SUCCESS)) {
    ...
    if (callback == NULL) {
      ...
      post = WBT_PostCommandINFO(wbt, bitmap, WFSi_ReceiveTableSequence, 0,
                                 &context->block_info_table);
    }
      ...
      context->table->buffer =
          (u8 *)MI_CallAlloc(context->allocator, (u32)length, 1);
      ...
      WFS_DEBUG_OUTPUT(
          ("WBT-GET(0x%08X):done [ready for mount]", WFS_TABLE_BLOCK_INDEX));
  ...
  if ((callback == NULL) || (callback->result == WBT_RESULT_SUCCESS)) {
    ...
    if (callback == NULL) {
      ...
      post = WFS_SendMessageLOCK_REQ(wbt, WFSi_ReadRomSequence, bitmap,
                                     context->request_region.offset +
                                         context->table->origin,
      ...
      const WFSMessageFormat *const msg =
          (const WFSMessageFormat *)callback->user_data.data;
      ...
      if ((callback->event == WBT_CMD_REQ_USER_DATA) &&
        ...
        if (!accepted) {
          ...
          post = WBT_PostCommandGET(
              wbt, bitmap, WFSi_ReadRomSequence, context->block_id,
              context->request_region.length, &context->recv_buf_table,
              &context->recv_buf_packet_bmp_table);
        }
      }
        ...
        WFS_DEBUG_OUTPUT(
            ("WBT-MSG(UNLOCK,0x%08X):done [read-operation completed]",
             context->block_id));
...
static void WFSi_WBTSystemCallback(void *userdata, WBTCommand *callback) {
  ...
  if ((callback->event == WBT_CMD_REQ_USER_DATA) && (context->request_buffer)) {
    WFSi_ReadRomSequence(context, callback);
  }
}

void WFS_CallClientConnectHook(WFSClientContext *context,
                               const WFSPeerInfo *peer) {
  (void)context;
  (void)peer;
}

void WFS_CallClientDisconnectHook(WFSClientContext *context,
                                  const WFSPeerInfo *peer) {
  (void)context;
  (void)peer;
}

void WFS_CallClientPacketSendHook(WFSClientContext *context,
                                  WFSPacketBuffer *packet) {
  packet->length = WBT_CallPacketSendHook(context->wbt, packet->buffer,
                                          packet->length, FALSE);
}

void WFS_CallClientPacketRecvHook(WFSClientContext *context,
                                  const WFSPacketBuffer *packet) {
  int aid = (int)MATH_CTZ((u32)packet->bitmap);
  WBT_CallPacketRecvHook(context->wbt, aid, packet->buffer, packet->length);
}

void WFS_InitClient(WFSClientContext *context, void *userdata,
                    WFSEventCallback callback, MIAllocator *allocator) {
  int i;
...
void WFS_EndClient(WFSClientContext *context) {
  ...
  if (context->request_callback) {
    (*context->request_callback)(context->request_argument, FALSE,
                                 context->request_argument);
  }
...
void WFS_RequestClientRead(WFSClientContext *context, void *buffer, u32 offset,
                           u32 length,
                           WFSRequestClientReadDoneCallback callback,
                           void *arg) {
...
void WFS_GetClientReadProgress(WFSClientContext *context, int *current,
                               int *total) {
