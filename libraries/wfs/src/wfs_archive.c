...
static FSResult WFSi_ArchiveReadCallback(FSArchive *archive, void *buffer,
                                         u32 offset, u32 length) {
  FSResult result = FS_RESULT_ERROR;
  WFSClientContext *const context =
      (WFSClientContext *)FS_GetArchiveBase(archive);
...
static void WFSi_ArchiveReadDoneCallback(WFSClientContext *context,
                                         BOOL succeeded, void *arg) {
...
static FSResult WFSi_EmptyArchiveProc(FSFile *file, FSCommandType command) {
  ...
  if ((command != FS_COMMAND_CLOSEFILE) &&
      ((command < FS_COMMAND_STATUS_BEGIN) ||
       (command >= FS_COMMAND_STATUS_END))) {
    static BOOL once = FALSE;
    if (!once) {
      once = TRUE;
      OS_TWarning(
          "WFS-Client has been finalized. (all the commands will fail)\n");
    }
...
static void WFSi_SetThreadForEndClient(FSArchive *archive) {
  ...
  OS_InitMessageQueue(&WFSi_MesgQueue, &WFSi_MesgArray[0],
                      WFSi_MESG_QUEUE_DEPTH);
  OS_CreateThread(&WFSi_SubThread, WFSi_SubThreadForEndClient, (void *)0,
                  WFSi_Stack + WFSi_STACK_SIZE / sizeof(u64), WFSi_STACK_SIZE,
                  WFSi_STACK_PRIORITY);
