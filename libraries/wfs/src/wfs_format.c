...
BOOL WFS_LoadTable(WFSTableFormat *archive, MIAllocator *allocator,
...

  {
    ...
    archive->region[WFS_TABLE_REGION_FAT].offset =
        MI_LoadLE32(&header->fat.offset);
    archive->region[WFS_TABLE_REGION_FAT].length =
        MI_LoadLE32(&header->fat.length);
    archive->region[WFS_TABLE_REGION_FNT].offset =
        MI_LoadLE32(&header->fnt.offset);
    archive->region[WFS_TABLE_REGION_FNT].length =
        MI_LoadLE32(&header->fnt.length);
  }

  {
    ...
    archive->region[WFS_TABLE_REGION_OV9].offset =
        MI_LoadLE32(&header->main_ovt.offset) + diff;
    archive->region[WFS_TABLE_REGION_OV9].length =
        MI_LoadLE32(&header->main_ovt.length);
    archive->region[WFS_TABLE_REGION_OV7].offset =
        MI_LoadLE32(&header->sub_ovt.offset) + diff;
    archive->region[WFS_TABLE_REGION_OV7].length =
        MI_LoadLE32(&header->sub_ovt.length);
    ...
    if (archive->buffer) {

      {
        ...
        for (i = 0; i < WFS_TABLE_REGION_MAX; ++i) {
          ...
          dst += sizeof(u32);
          (void)MI_ReadDevice(
              device, dst, archive->origin + archive->region[i].offset, length);
          dst += length;
      ...
      if (mixed) {
        u32 count =
            archive->region[WFS_TABLE_REGION_FAT].length / sizeof(WFSFATFormat);
        ...
        for (i = WFS_TABLE_REGION_OV9; i < WFS_TABLE_REGION_MAX; ++i) {
          WFSOVLFormat *ovl = (WFSOVLFormat *)&archive
                                  ->buffer[mem_region[i].offset + sizeof(u32)];
          ...
          for (j = 0; j < num; ++j) {
            if (count >= total) {
              ...
              (void)MI_ReadDevice(device, &entry,
                                  archive->origin + origin_b_fat +
                                      ovl[j].file_id * sizeof(WFSFATFormat),
                                  sizeof(entry));
