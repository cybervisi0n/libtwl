...
void G2x_SetBGyAffine_(u32 addr, const MtxFx22 *mtx, int centerX, int centerY,
  ...
  *((vu32 *)addr + 0) =
      (u32)((u16)(s16)(mtx->_00 >> 4) | (u16)(s16)(mtx->_01 >> 4) << 16);

  *((vu32 *)addr + 1) =
      (u32)((u16)(s16)(mtx->_10 >> 4) | (u16)(s16)(mtx->_11 >> 4) << 16);
...
void G2x_SetBlendAlpha_(u32 addr, int plane1, int plane2, int ev1, int ev2) {
  ...
  *((vu32 *)addr + 0) = (u32)((G2_BLENDTYPE_ALPHA | plane1 | plane2 << 8) |
                              ((ev1 | ev2 << 8) << 16));
}
...
void G2x_ChangeBlendBrightness_(u32 addr, int brightness) {
  ...
  if (brightness < 0) {
    if (G2_BLENDTYPE_FADEIN == (tmp & REG_G2_BLDCNT_EFFECT_MASK)) {
      ...
      *((vu16 *)addr + 0) =
          (u16)((tmp & ~REG_G2_BLDCNT_EFFECT_MASK) | G2_BLENDTYPE_FADEOUT);
    }
    ...
    *((vu16 *)addr + 2) = (u16)-brightness;
    ...
    if (G2_BLENDTYPE_FADEOUT == (tmp & REG_G2_BLDCNT_EFFECT_MASK)) {
      ...
      *((vu16 *)addr + 0) =
          (u16)((tmp & ~REG_G2_BLDCNT_EFFECT_MASK) | G2_BLENDTYPE_FADEIN);
    }
