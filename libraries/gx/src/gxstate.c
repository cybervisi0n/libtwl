...
#define SDK_CONFLICT_ASSERT(a, b, name1, name2)                                \
  SDK_TASSERTMSG(!(a & b),                                                     \
                 "VRAM Bank conflicts between " #name1 " and " #name2          \
                 " (0x%02x, 0x%02x)\n",                                        \
                 a, b)

#define SDK_CONFLICT_CHECK(val, bank)                                          \
  if (val & bank)                                                              \
    goto SDK_VRAMCNT_ERROR;                                                    \
  else                                                                         \
    val |= bank
...
SDK_VRAMCNT_ERROR:
  ...
  SDK_CONFLICT_ASSERT(p->bgExtPltt, p->sub_objExtPltt, BGExtPltt,
                      SubOBJExtPltt);

  SDK_CONFLICT_ASSERT(p->objExtPltt, p->sub_bg, OBJExtPltt, SubBG);
  SDK_CONFLICT_ASSERT(p->objExtPltt, p->sub_obj, OBJExtPltt, SubOBJ);
  SDK_CONFLICT_ASSERT(p->objExtPltt, p->sub_bgExtPltt, OBJExtPltt,
                      SubBGExtPltt);
  SDK_CONFLICT_ASSERT(p->objExtPltt, p->sub_objExtPltt, OBJExtPltt,
                      SubOBJExtPltt);
  ...
  SDK_CONFLICT_ASSERT(p->sub_bgExtPltt, p->sub_objExtPltt, SubBGExtPltt,
                      SubOBJExtPltt);
}
...
#define SDK_REGION_ASSERT(name, r1, r2, d1, d2)                                \
  SDK_TASSERTMSG(((r1) <= (d1)) && ((r2) >= (d2)),                             \
                 ...
                 "access occured (0x%08x -> 0x%08x).",                         \
                 r1, r2, d1, d2)

#define SDK_REGION_ASSERT_EX(name, r1, r2, r3, r4, d1, d2)                     \
  SDK_TASSERTMSG((((r1) <= (d1)) && ((r2) >= (d2))) ||                         \
                     (((r3) <= (d1)) && ((r4) >= (d2))),                       \
                 "Region allocated to " #name                                  \
                 " is (0x%08x -> 0x%08x, 0x%08x -> 0x%08x),\n"                 \
                 "access occured (0x%08x -> 0x%08x).",                         \
                 r1, r2, r3, r4, d1, d2)
...
void GX_RegionCheck_OBJ_(u32 first, u32 last) {
  ...
  case GX_VRAM_OBJ_16_G:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM, HW_OBJ_VRAM + HW_VRAM_F_SIZE, first,
                      last);
    break;
  case GX_VRAM_OBJ_32_FG:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM,
                      HW_OBJ_VRAM + HW_VRAM_F_SIZE + HW_VRAM_G_SIZE, first,
                      last);
    break;
  case GX_VRAM_OBJ_64_E:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM, HW_OBJ_VRAM + HW_VRAM_E_SIZE, first,
                      last);
    break;
  case GX_VRAM_OBJ_80_EF:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM,
                      HW_OBJ_VRAM + HW_VRAM_E_SIZE + HW_VRAM_F_SIZE, first,
                      last);
    break;
  case GX_VRAM_OBJ_80_EG:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM,
                      HW_OBJ_VRAM + HW_VRAM_E_SIZE + HW_VRAM_G_SIZE, first,
                      last);
    break;
  case GX_VRAM_OBJ_96_EFG:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM,
                      HW_OBJ_VRAM + HW_VRAM_E_SIZE + HW_VRAM_F_SIZE +
                          HW_VRAM_G_SIZE,
                      first, last);
    break;
  ...
  case GX_VRAM_OBJ_128_B:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM, HW_OBJ_VRAM + HW_VRAM_A_SIZE, first,
                      last);
    break;
  case GX_VRAM_OBJ_256_AB:
    SDK_REGION_ASSERT(OBJ, HW_OBJ_VRAM,
                      HW_OBJ_VRAM + HW_VRAM_A_SIZE + HW_VRAM_B_SIZE, first,
                      last);
    break;
  default:
...
void GX_RegionCheck_BG_(u32 first, u32 last) {
  ...
  case GX_VRAM_BG_512_ABCD:
    abcd_size =
        HW_VRAM_A_SIZE + HW_VRAM_B_SIZE + HW_VRAM_C_SIZE + HW_VRAM_D_SIZE;
    break;
  }
  ...
  if (vram_abcd && vram_efg) {
    ...
    SDK_REGION_ASSERT_EX(BG, HW_BG_VRAM, HW_BG_VRAM + efg_size,
                         HW_BG_VRAM + HW_VRAM_A_SIZE,
                         HW_BG_VRAM + HW_VRAM_A_SIZE + abcd_size, first, last);
  } else if (vram_abcd) {
...
void GX_RegionCheck_SubOBJ_(u32 first, u32 last) {
  ...
  case GX_VRAM_SUB_OBJ_128_D:
    SDK_REGION_ASSERT(SubOBJ, HW_DB_OBJ_VRAM, HW_DB_OBJ_VRAM + HW_VRAM_D_SIZE,
                      first, last);
    break;
  case GX_VRAM_SUB_OBJ_16_I:
    SDK_REGION_ASSERT(SubOBJ, HW_DB_OBJ_VRAM, HW_DB_OBJ_VRAM + HW_VRAM_I_SIZE,
                      first, last);
    break;
  }
}
...
void GX_RegionCheck_SubBG_(u32 first, u32 last) {
  ...
  case GX_VRAM_SUB_BG_128_C:
    SDK_REGION_ASSERT(SubBG, HW_DB_BG_VRAM, HW_DB_BG_VRAM + HW_VRAM_C_SIZE,
                      first, last);
    break;
  case GX_VRAM_SUB_BG_32_H:
    SDK_REGION_ASSERT(SubBG, HW_DB_BG_VRAM, HW_DB_BG_VRAM + HW_VRAM_H_SIZE,
                      first, last);
    break;
  case GX_VRAM_SUB_BG_48_HI:
    SDK_REGION_ASSERT(SubBG, HW_DB_BG_VRAM,
                      HW_DB_BG_VRAM + HW_VRAM_H_SIZE + HW_VRAM_I_SIZE, first,
                      last);
    break;
  default:
...
#pragma unused(first)
  ...
  case GX_VRAM_TEXPLTT_0_G:
    SDK_TASSERTMSG(
        last <= 0x04000,
        "Texture pallete 0x04000-0x18000 not available(0x%5x->0x%5x)", first,
        last);
    break;
  ...
  case GX_VRAM_TEXPLTT_01_FG:
    SDK_TASSERTMSG(
        last <= 0x08000,
        "Texture pallete 0x08000-0x18000 not available(0x%5x->0x%5x)", first,
        last);
    break;
  ...
  case GX_VRAM_TEXPLTT_0123_E:
    SDK_TASSERTMSG(
        last <= 0x10000,
        "Texture pallete 0x10000-0x18000 not available(0x%5x->0x%5x)", first,
        last);
    break;
  ...
  case GX_VRAM_TEXPLTT_01234_EF:
    SDK_TASSERTMSG(
        last <= 0x14000,
        "Texture pallete 0x14000-0x18000 not available(0x%5x->0x%5x)", first,
        last);
    break;

  case GX_VRAM_TEXPLTT_012345_EFG:
    SDK_TASSERTMSG(last <= 0x18000,
                   "Illegal Address/size specified(0x%5x->0x%5x)", first, last);
    break;
...
#pragma unused(first)
  ...
  case GX_VRAM_TEX_0_D:
    SDK_TASSERTMSG(last <= 0x20000,
                   "Texture slot 0x20000-0x80000 not available(0x%5x->0x%5x)",
                   first, last);
    break;
  ...
  case GX_VRAM_TEX_01_BD:
    SDK_TASSERTMSG(last <= 0x40000,
                   "Texture slot 0x40000-0x80000 not available(0x%5x->0x%5x)",
                   first, last);
    break;
  ...
  case GX_VRAM_TEX_012_ACD:
    SDK_TASSERTMSG(last <= 0x60000,
                   "Texture slot 0x60000-0x80000 not available(0x%5x->0x%5x)",
                   first, last);
    break;

  case GX_VRAM_TEX_0123_ABCD:
    SDK_TASSERTMSG(last <= 0x80000,
                   "Illegal Address/size specified(0x%5x->0x%5x)", first, last);
    break;
