...
void GX_SetVCountEqVal(s32 val) {
  ...
  reg_GX_DISPSTAT =
      (u16)((reg_GX_DISPSTAT &
             (REG_GX_DISPSTAT_VBLK_MASK | REG_GX_DISPSTAT_HBLK_MASK |
              REG_GX_DISPSTAT_LYC_MASK | REG_GX_DISPSTAT_VBI_MASK |
              REG_GX_DISPSTAT_HBI_MASK | REG_GX_DISPSTAT_VQI_MASK)) |
...
BOOL GX_IsDispOn(void) { return sIsDispOn; }
...
void GX_DispOff(void) {
  ...
  sIsDispOn = FALSE;
  sDispMode =
      (u16)((tmp & REG_GX_DISPCNT_MODE_MASK) >> REG_GX_DISPCNT_MODE_SHIFT);
...
void GX_DispOn(void) {
  ...
  if (sDispMode != (u16)GX_DISPMODE_OFF) {
    ...
    reg_GX_DISPCNT = ((reg_GX_DISPCNT |
                       (GX_DISPMODE_GRAPHICS << REG_GX_DISPCNT_MODE_SHIFT)));
  }
}

void GX_SetGraphicsMode(GXDispMode dispMode, GXBGMode bgMode,
                        GXBG0As bg0_2d3d) {
  ...
  cnt &= ~(REG_GX_DISPCNT_BGMODE_MASK | REG_GX_DISPCNT_BG02D3D_MASK |
           REG_GX_DISPCNT_MODE_MASK | REG_GX_DISPCNT_VRAM_MASK);

  reg_GX_DISPCNT = (u32)(cnt | (dispMode << REG_GX_DISPCNT_MODE_SHIFT) |
                         (bgMode << REG_GX_DISPCNT_BGMODE_SHIFT) |
                         (bg0_2d3d << REG_GX_DISPCNT_BG02D3D_SHIFT));
