...
static u32
    fifoBuf[WM_BUF_MSG_NUM][WM_FIFO_BUF_SIZE / sizeof(u32)] ATTRIBUTE_ALIGN(32);
...
#define WM_SYSTEM_BUF_SIZE_FOR_SCAN                                            \
  (WM_ARM9WM_BUF_SIZE_FOR_SCAN + WM_ARM7WM_BUF_SIZE +                          \
   WM_STATUS_BUF_SIZE_FOR_SCAN + WM_FIFO_BUF_SIZE + WM_FIFO_BUF_SIZE)
  ...
  wm9buf->fifo9to7 =
      (u32 *)((u32)(wm9buf->status) + WM_STATUS_BUF_SIZE_FOR_SCAN);
...
static WMErrCode WmInitCore(void *wmSysBuf, u16 dmaNo, u32 bufSize) {
  ...
  if (OS_IsOnVram(wmSysBuf) ||
      (OS_IsRunOnTwl() && OS_IsOnExtendedMainMemory(wmSysBuf)))
#else
  if (OS_IsOnVram(wmSysBuf))
#endif
  {
    WM_WARNING(
        "Parameter \"wmSysBuf\" must be alocated less than 0x04000000.\n");
    (void)OS_RestoreInterrupts(e);
    return WM_ERRCODE_INVALID_PARAM;
  }
...
WMArm9Buf *WMi_GetSystemWork(void) { return wm9buf; }
...
WMErrCode WMi_CheckIdle(void) {
  ...
  if ((wm9buf->status->state == WM_STATE_READY) ||
      (wm9buf->status->state == WM_STATE_STOP)) {
...
#pragma unused(tag)
  ...
  if (callback_p->apiid >= WM_NUM_OF_CALLBACK) {
    if (callback_p->apiid == WM_APIID_INDICATION) {
      ...
      if (w9b->portCallbackTable[cb_Port->port] != NULL) {
        ...
        DC_InvalidateRange(
            cb_Port->recvBuf,
            w9b->status
                ->mp_recvBufSize); // TODO: Too much is being invalidated here
        (w9b->portCallbackTable[cb_Port->port])((void *)cb_Port);
      }
#ifdef WM_DEBUG
      else {
        WM_DPRINTF(
            "Warning: no callback function on port %d { %04x %04x ... }\n",
    ...
    if (((callback_p->apiid == WM_APIID_DISABLE ||
          callback_p->apiid == WM_APIID_END) &&
         callback_p->errcode == WM_ERRCODE_SUCCESS) ||
        ((callback_p->apiid == WM_APIID_ENABLE ||
          callback_p->apiid == WM_APIID_INITIALIZE) &&
         callback_p->errcode != WM_ERRCODE_SUCCESS)) {
      ...
      if (state == WM_STATECODE_CONNECTED ||
        ...
        if (state == WM_STATECODE_CONNECTED) {
          ...
          if (w9b->connectedAidBitmap & (1 << aid)) {
            WM_DPRINTF(
                "Warning: someone is connecting to connected aid: %d (%04x)",
          ...
          w9b->connectedAidBitmap |= (1 << aid);
        } else // WM_STATECODE_DISCONNECTED ||
               // WM_STATECODE_DISCONNECTED_FROM_MYSELF
        {
#ifdef WM_DEBUG
          if (!(w9b->connectedAidBitmap & (1 << aid))) {
            WM_DPRINTF("Warning: someone is disconnecting to disconnected aid: "
                       "%d (%04x)",
                       aid, w9b->connectedAidBitmap);
          }
#endif
...
void WMi_DebugPrintSendQueue(WMPortSendQueue *queue) {
  ...
  DC_InvalidateRange(
      wm9buf->status,
      WM_STATUS_BUF_SIZE); // Invalidates the ARM7 status region cache
  queueData = status->sendQueueData;

  OS_TPrintf("head = %d, tail = %d, ", queue->head, queue->tail);
  if (queue->tail != WM_SEND_QUEUE_END) {
    OS_TPrintf("%s", (queueData[queue->tail].next == WM_SEND_QUEUE_END)
                         ? "valid"
                         : "invalid");
  }
  OS_TPrintf("\n");
  for (index = queue->head; index != WM_SEND_QUEUE_END;
    ...
    OS_TPrintf("queueData[%d] -> %d { port=%d, destBitmap=%x, size=%d } \n",
               index, data->next, data->port, data->destBitmap, data->size);
  }
}
...
void WMi_DebugPrintAllSendQueue(void) {
  ...
  OS_TPrintf(" head: %d, tail: %d\n", status->readyQueue[2].head,
             status->readyQueue[2].tail);
#endif
}
...
BOOL WMi_CheckMpPacketTimeRequired(u16 parentSize, u16 childSize, u8 childs) {
  ...
  if (mp_time > WM_MAX_MP_PACKET_TIME) {
    OS_TWarning("It is required %dus to transfer each MP packet.\nThat should "
                "not exceed %dus.\n",
...
void WMi_DeleteSleepCallback(void) { PM_DeletePreSleepCallback(&sleepCbInfo); }
