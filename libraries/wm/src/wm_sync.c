...
int WM_GetMPSendBufferSize(void) {
  ...
  DC_InvalidateRange(&(p->status->mp_flag),
...
int WM_GetMPReceiveBufferSize(void) {
  ...
  if (isParent == TRUE) {
    ...
    return (int)((sizeof(WMmpRecvHeader) - sizeof(WMmpRecvData) +
                  ((sizeof(WMmpRecvData) + maxReceiveSize - 2 + 2 /*MACBUG*/) *
                   maxEntry) +
                  31) &
                 ~0x1f) *
           2;
...
WMMpRecvData *WM_ReadMPData(const WMMpRecvHeader *header, u16 aid) {
  int i;
  WMErrCode result;
  WMMpRecvData *recvdata_p[WM_NUM_MAX_CHILD]; // Array of pointers to data
                                              // starting position for all child
