...
WMErrCode WMi_StartMP(WMCallbackFunc callback, u16 *recvBuf, u16 recvBufSize,
                      u16 *sendBuf, u16 sendBufSize, WMMPTmpParam *tmpParam) {
  WMErrCode result;
  ...
  DC_InvalidateRange(&(status->mp_flag),
                     4); // Invalidates the ARM7 status region cache
...
WMErrCode WM_StartMPEx(WMCallbackFunc callback, u16 *recvBuf, u16 recvBufSize,
  ...
  tmpParam.mask = WM_MP_TMP_PARAM_MIN_FREQUENCY | WM_MP_TMP_PARAM_FREQUENCY |
                  WM_MP_TMP_PARAM_DEFAULT_RETRY_COUNT |
                  WM_MP_TMP_PARAM_MIN_POLL_BMP_MODE |
  ...
  return WMi_StartMP(callback, recvBuf, recvBufSize, sendBuf, sendBufSize,
                     &tmpParam);
}
...
WMErrCode WM_SetMPDataToPortEx(WMCallbackFunc callback, void *arg,
  ...
  if ((void *)sendData == (void *)status->mp_sendBuf) {
    WM_WARNING("Parameter \"sendData\" must not be equal to the WM_StartMP "
               "argument \"sendBuf\".\n");
    return WM_ERRCODE_INVALID_PARAM;
  }
  ...
  if (isParent && status->mp_current_minPollBmpMode) {
    ...
    (void)WMi_CheckMpPacketTimeRequired(
        status->pparam.parentMaxSize, status->pparam.childMaxSize,
  ...
  result = WMi_SendCommand(WM_APIID_SET_MP_DATA, 7, (u32)sendData,
                           (u32)sendDataSize, (u32)destBitmap, (u32)port,
                           (u32)prio, (u32)callback, (u32)arg);
  WM_CHECK_RESULT(result);
