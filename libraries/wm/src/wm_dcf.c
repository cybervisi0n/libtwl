...
WMErrCode WM_StartDCF(WMCallbackFunc callback, WMDcfRecvBuf *recvBuf,
  ...
  result =
      WMi_SendCommand(WM_APIID_START_DCF, 2, (u32)recvBuf, (u32)recvBufSize);
  WM_CHECK_RESULT(result);
...
WMErrCode WM_SetDCFData(WMCallbackFunc callback, const u8 *destAdr,
  ...
  result = WMi_SendCommand(WM_APIID_SET_DCF_DATA, 4, (u32)wMac[0], (u32)wMac[1],
                           (u32)sendData, (u32)sendDataSize);
  WM_CHECK_RESULT(result);
