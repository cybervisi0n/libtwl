...
WMErrCode WMi_EnableEx(WMCallbackFunc callback, u32 miscFlags) {
...

  {
    ...
    result = WMi_SendCommand(WM_APIID_ENABLE, 4, (u32)(p->WM7),
                             (u32)(p->status), (u32)(p->fifo7to9), miscFlags);
    WM_CHECK_RESULT(result);
  }
...
WMErrCode WMi_InitializeEx(void *wmSysBuf, WMCallbackFunc callback, u16 dmaNo,
...

  {
    ...
    result = WMi_SendCommand(WM_APIID_INITIALIZE, 4, (u32)(p->WM7),
                             (u32)(p->status), (u32)(p->fifo7to9), miscFlags);
    WM_CHECK_RESULT(result);
  }
...
WMErrCode WM_SetParentParameter(WMCallbackFunc callback,
...
#ifdef SDK_TWL // Return an error if the TWL System Settings are configured not
               // to use wireless.
  if (OS_IsRunOnTwl()) {
  ...
  if (pparaBuf->userGameInfoLength > 0) {
    ...
    if ((u32)(pparaBuf->userGameInfo) & 0x01f) {
      ...
      WM_WARNING(
          "Parameter \"pparaBuf->userGameInfo\" is not 32-byte aligned.\n");
    }
  }
  ...
  if ((pparaBuf->parentMaxSize +
      ...
      (pparaBuf->childMaxSize +
           (pparaBuf->KS_Flag ? WM_SIZE_KS_CHILD_DATA + WM_SIZE_MP_CHILD_PADDING
                              : 0) >
       WM_SIZE_MP_DATA_MAX)) {
...
WMErrCode WM_StartScanEx(WMCallbackFunc callback, const WMScanExParam *param) {
  ...
  if (param->scanBufSize > WM_SIZE_SCAN_EX_BUF) {
    WM_WARNING("Parameter \"param->scanBufSize\" must be less than or equal to "
               "WM_SIZE_SCAN_EX_BUF.\n");
    return WM_ERRCODE_INVALID_PARAM;
  }
  if ((u32)(param->scanBuf) & 0x01f) {
  ...
  if (param->ssidLength > WM_SIZE_SSID) {
    WM_WARNING("Parameter \"param->ssidLength\" must be less than or equal to "
               "WM_SIZE_SSID.\n");
    return WM_ERRCODE_INVALID_PARAM;
  }
  if (param->scanType != WM_SCANTYPE_ACTIVE &&
      param->scanType != WM_SCANTYPE_PASSIVE &&
      param->scanType != WM_SCANTYPE_ACTIVE_CUSTOM &&
      param->scanType != WM_SCANTYPE_PASSIVE_CUSTOM) {
    WM_WARNING("Parameter \"param->scanType\" must be WM_SCANTYPE_PASSIVE or "
               "WM_SCANTYPE_ACTIVE.\n");
    return WM_ERRCODE_INVALID_PARAM;
  }
  if ((param->scanType == WM_SCANTYPE_ACTIVE_CUSTOM ||
       param->scanType == WM_SCANTYPE_PASSIVE_CUSTOM) &&
      param->ssidMatchLength > WM_SIZE_SSID) {
    WM_WARNING("Parameter \"param->ssidMatchLength\" must be less than or "
               "equal to WM_SIZE_SSID.\n");
    return WM_ERRCODE_INVALID_PARAM;
  }
...
WMErrCode WM_StartConnectEx(WMCallbackFunc callback, const WMBssDesc *pInfo,
  ...
  if ((authMode != WM_AUTHMODE_OPEN_SYSTEM) &&
      (authMode != WM_AUTHMODE_SHARED_KEY)) {
    WM_WARNING("Parameter \"authMode\" must be WM_AUTHMODE_OPEN_SYSTEM or "
               "WM_AUTHMODE_SHARED_KEY.\n");
  }
...
WMErrCode WM_Disconnect(WMCallbackFunc callback, u16 aid) {
  ...
  result =
      WMi_CheckStateEx(5, WM_STATE_PARENT, WM_STATE_MP_PARENT, WM_STATE_CHILD,
                       WM_STATE_MP_CHILD, WM_STATE_DCF_CHILD);
  WM_CHECK_RESULT(result);

  if ( // p->status->state cache has been discarded
      (p->status->state == WM_STATE_PARENT) ||
      (p->status->state == WM_STATE_MP_PARENT)) {
