...
static void WmDataSharingReceiveData(WMDataSharingInfo *dsInfo, u16 aid,
                                     u16 *data);
static void WmDataSharingSendDataSet(WMDataSharingInfo *dsInfo, BOOL delayed);
static u16 *WmGetSharedDataAddress(WMDataSharingInfo *dsInfo, u32 aidBitmap,
                                   u16 *receiveBuf, u32 aid);
...
WMErrCode WM_StartDataSharing(WMDataSharingInfo *dsInfo, u16 port,
  ...
  aidBitmap |= (1 << aid); // Add the self's own portion
...

  {
    ...
    if (dsInfo->dataSetLength > WM_DS_DATA_SIZE) {
      ...
      dsInfo->aidBitmap = 0;
      WM_WARNING("Total size of sharing data must be less than or equal to %d "
                 "bytes.\n",
                 WM_DS_DATA_SIZE);
      return WM_ERRCODE_INVALID_PARAM;
    }
  ...
  if (aid == 0) {
    ...
    for (i = 0; i < WM_DS_DATASET_NUM; i++) {
      dsInfo->ds[i].aidBitmap =
          (u16)(dsInfo->aidBitmap & (connectedAIDs | 0x0001));
    }

    (void)WM_SetPortCallback(port, WmDataSharingReceiveCallback_Parent,
                             (void *)dsInfo);
    ...
    for (i = 0; i < ((dsInfo->doubleMode == TRUE) ? 2 : 1); i++) {
      ...
      res = WM_SetMPDataToPortEx(WmDataSharingSetDataCallback, dsInfo,
                                 (u16 *)&dsInfo->ds[i], dsInfo->dataSetLength,
                                 (u16)(dsInfo->aidBitmap & connectedAIDs),
    ...
    (void)WM_SetPortCallback(port, WmDataSharingReceiveCallback_Child,
                             (void *)dsInfo);
  }
...
WMErrCode WM_StepDataSharing(WMDataSharingInfo *dsInfo, const u16 *sendData,
  ...
  if (aid == 0) {
    ...
    if (state == WM_DS_STATE_RETRY_SEND) {
      ...
      if (res == WM_ERRCODE_NO_CHILD) {
        WM_DLOGF_DATASHARING("sent ds   : write: %d, read: %d, send: %d\n",
                             dsInfo->writeIndex, dsInfo->readIndex,
                             dsInfo->sendIndex);
    ...
    if (dsInfo->readIndex != dsInfo->sendIndex) {
      ...
      dsInfo->ds[dsInfo->readIndex].aidBitmap |=
          0x0001; // The lowest bit in the aidBitmap in the transmission data is
                  // a delay flag.
      MI_CpuCopy16(&dsInfo->ds[dsInfo->readIndex], receiveData,
                   sizeof(WMDataSet));
    ...
    if (sendFlag) {
      ...
      WM_DLOGF_DATASHARING("send data : write: %d, read: %d, send: %d",
                           dsInfo->writeIndex, dsInfo->readIndex,
                           dsInfo->sendIndex);

      WmDataSharingReceiveData(
          dsInfo, 0, (u16 *)sendData); // Remove const because chained rewrite
                                       // past here is bothersome.
    ...
    if (sendFlag) {
      ...
      u16 *buf = (u16 *)(((u8 *)&dsInfo->ds[dsInfo->sendIndex]) +
                         32); // 32-byte alignment is required
      ...
      res = WM_SetMPDataToPortEx(WmDataSharingSetDataCallback, dsInfo, buf,
                                 dsInfo->dataLength, dsInfo->aidBitmap,
                                 dsInfo->port, WM_PRIORITY_HIGH);
      dsInfo->sendIndex = WmDataSharingGetNextIndex(dsInfo->sendIndex);
...
static void WmDataSharingSetDataCallback(void *callback) {
  ...
  if ((p->portCallbackTable[cb_Port->port] !=
           WmDataSharingReceiveCallback_Parent &&
       p->portCallbackTable[cb_Port->port] !=
  ...
  if (cb_Port->errcode == WM_ERRCODE_SUCCESS) {
    ...
    if (aid == 0) {
      ...
      WM_DLOGF_DATASHARING("sent data : write: %d, read: %d, send: %d",
                           dsInfo->writeIndex, dsInfo->readIndex,
                           dsInfo->sendIndex);
    }
  } else {
...
static void WmDataSharingReceiveData(WMDataSharingInfo *dsInfo, u16 aid,
  ...
  WM_DLOGF_DATASHARING("recv data%d: write: %d, read: %d, send: %d", aid,
                       dsInfo->writeIndex, dsInfo->readIndex,
                       dsInfo->sendIndex);
  ...
  if (dsInfo->aidBitmap & aidBit) {
    ...
    buf = WmGetSharedDataAddress(dsInfo, dsInfo->aidBitmap,
                                 dsInfo->ds[writeIndex].data, aid);
    if (data != NULL) {
...
void WmDataSharingSendDataSet(WMDataSharingInfo *dsInfo, BOOL delayed) {
  ...
  if (dsInfo->ds[dsInfo->writeIndex].aidBitmap == 0) {
    ...
    WM_DLOGF_DATASHARING("send ds   : write: %d, read: %d, send: %d",
                         dsInfo->writeIndex, dsInfo->readIndex,
                         dsInfo->sendIndex);
    ...
    dsInfo->ds[resetWI].aidBitmap =
        (u16)(dsInfo->aidBitmap & (connectedAIDs | 0x0001));
    dsInfo->writeIndex = newWI;
    dsInfo->ds[oldWI].aidBitmap =
        dsInfo
            ->aidBitmap; // Enter the original value in aidBitmap before sending
    if (delayed == TRUE) {
...
u16 *WmGetSharedDataAddress(WMDataSharingInfo *dsInfo, u32 aidBitmap,
                            u16 *receiveBuf, u32 aid) {
  u32 mask;
  u32 count;
  u32 offset;
