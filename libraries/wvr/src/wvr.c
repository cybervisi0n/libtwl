...
WVRResult WVR_StartUpAsync(GXVRamARM7 vram, WVRCallbackFunc callback,
                           void *arg) {
  OSIntrMode e;
  ...
  case GX_VRAM_ARM7_128_C:
    ...
    reg_GX_VRAMCNT_C =
        ((2 << REG_GX_VRAMCNT_C_MST_SHIFT) | (0 << REG_GX_VRAMCNT_C_OFS_SHIFT) |
  ...
  case GX_VRAM_ARM7_128_D:
    ...
    reg_GX_VRAMCNT_D =
        ((2 << REG_GX_VRAMCNT_D_MST_SHIFT) | (0 << REG_GX_VRAMCNT_D_OFS_SHIFT) |
  ...
  case GX_VRAM_ARM7_256_CD:
    ...
    reg_GX_VRAMCNT_C =
        ((2 << REG_GX_VRAMCNT_C_MST_SHIFT) | (0 << REG_GX_VRAMCNT_C_OFS_SHIFT) |
         (1 << REG_GX_VRAMCNT_C_E_SHIFT));
    reg_GX_VRAMCNT_D =
        ((2 << REG_GX_VRAMCNT_D_MST_SHIFT) | (1 << REG_GX_VRAMCNT_D_OFS_SHIFT) |
         (1 << REG_GX_VRAMCNT_D_E_SHIFT));
    break;
  default:
    (void)OS_RestoreInterrupts(e);
  ...
  if (0 >
      PXI_SendWordByFifo(PXI_FIFO_TAG_WVR, WVR_PXI_COMMAND_STARTUP, FALSE)) {
...
WVRResult WVR_TerminateAsync(WVRCallbackFunc callback, void *arg) {
  ...
  if (0 >
      PXI_SendWordByFifo(PXI_FIFO_TAG_WVR, WVR_PXI_COMMAND_TERMINATE, FALSE)) {
    wvrCallback = NULL;
    (void)OS_RestoreInterrupts(e);
    return WVR_RESULT_FIFO_ERROR;
  }
