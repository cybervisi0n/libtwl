...
void SND_PrepareSeq7(int playerNo, const void *seqBase, u32 seqOffset, SNDBankData *bank_p)
{
    SNDPlayer *player_p;
    SNDTrack *track_p;
    int     trackNo;
...
void SND_StartSeq7(int playerNo, const void *seqBase, u32 seqOffset, SNDBankData *bank_p)
{
    SND_PrepareSeq7(playerNo, seqBase, seqOffset, bank_p);
    SND_StartPreparedSeq7(playerNo);
}
...
void SND_SetTrackMute7(int playerNo, u32 trackBitMask, SNDSeqMute mute)
{
    ...
    for (trackNo = 0; trackNo < SND_TRACK_NUM_PER_PLAYER && trackBitMask != 0;
         trackNo++, trackBitMask >>= 1)
    {
        if ((trackBitMask & 0x01) == 0)
            continue;
...
{
#ifdef SDK_PORT
    seqCache.base = (const u32 *)((u64)ptr & 0xfffffffffffffffc);
#else
#ifndef _MSC_VER
...
static void UpdateTrackChannel(SNDTrack *track_p, SNDPlayer *player_p, BOOL doRelease)
{
    SNDExChannel *ch_p;
    s32     decay
        = SND_CalcDecibelSquare(track_p->volume)
        + SND_CalcDecibelSquare(track_p->volume2) + SND_CalcDecibelSquare(player_p->volume);
    ...
    while (track_p->wait == 0 && !track_p->note_finish_wait)
    {
        ...
        else
        {
            ...
            case 0xe0:
                {
                    s16     arg =
                        (s16)ReadArg(track_p, player_p, useArgType ? argType : SEQ_ARG_S16);
            ...
            case 0xf0:
                ...
                case SND_MML_ALLOC_TRACK:
                    SDK_ASSERTMSG(FALSE, "SND: must use alloctrack in startup code");
                    break;
...
static vs16 *GetVariablePtr(SNDPlayer *player_p, int varNo)
{
    ...
    SDK_WARNING(SNDi_SharedWork != NULL,
                "SND: Cannot use sequence variable because shared work don't set");
