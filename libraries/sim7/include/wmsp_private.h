...
int     WMSP_PutSendQueue(u32 childBitmap, u16 priority, u16 port, u32 destBitmap,
                          const u16 *sendData, u16 sendDataSize, WMCallbackFunc callback,
                          void *arg);
...
WlMlmePowerManagementCfm *WMSP_WL_MlmePowerManagement(u16 *buf, u16 pwrMgtMode, u16 wakeUp,
                                                      u16 recieveDtims);
WlMlmeScanCfm *WMSP_WL_MlmeScan(u16 *buf, u32 bufSize, u16 *bssid, u16 ssidLength, u8 *ssid,
                                u16 scanType, u8 *channelList, u16 maxChannelTime);
WlMlmeJoinCfm *WMSP_WL_MlmeJoin(u16 *buf, u16 timeOut, WlBssDesc *bssDesc);
WlMlmeAuthenticateCfm *WMSP_WL_MlmeAuthenticate(u16 *buf, u16 *peerMacAdrs, u16 algorithm,
                                                u16 timeOut);
WlMlmeDeAuthenticateCfm *WMSP_WL_MlmeDeAuthenticate(u16 *buf, u16 *peerMacAdrs, u16 reasonCode);
WlMlmeAssociateCfm *WMSP_WL_MlmeAssociate(u16 *buf, u16 *peerMacAdrs, u16 listenInterval,
                                          u16 timeOut);
WlMlmeReAssociateCfm *WMSP_WL_MlmeReAssociate(u16 *buf, u16 *newApMacAdrs, u16 listenInterval,
                                              u16 timeOut);
WlMlmeDisAssociateCfm *WMSP_WL_MlmeDisAssociate(u16 *buf, u16 *peerMacAdrs, u16 reasonCode);
WlMlmeStartCfm *WMSP_WL_MlmeStart(u16 *buf, u16 ssidLength, u8 *ssid, u16 beaconPeriod,
                                  u16 dtimPeriod, u16 channel, u16 basicRateSet, u16 supportRateSet,
                                  u16 gameInfoLength, WMGameInfo *gameInfo);
WlMlmeMeasureChannelCfm *WMSP_WL_MlmeMeasureChannel(u16 *buf, u16 ccaMode, u16 edThreshold,
                                                    u16 measureTime, u8 *channelList);
...
WlMaMpCfm *WMSP_WL_MaMp(u16 *buf, u16 resume, u16 retryLimit, u16 txop, u16 pollBitmap, u16 tmptt,
                        u16 currTsf, u16 dataLength, u16 wmHeader, u16 *datap);
...
WlParamSetCfm *WMSP_WL_ParamSetCCAModeEDThreshold(u16 *buf, u16 ccaMode, u16 edThreshold);
#endif
WlParamSetCfm *WMSP_WL_ParamSetLifeTime(u16 *buf, u16 tableNumber, u16 camLifeTime,
                                        u16 frameLifeTime);
...
static inline int WMSP_GetMPOneReceiveBufferSizeParent(int recvSize, int nChildlen)
{
    return (int)((sizeof(WMmpRecvHeader) - sizeof(WMmpRecvData) +
                  ((sizeof(WMmpRecvData) + recvSize - 2 + 2) * nChildlen) + 31) & ~0x1f);
}
