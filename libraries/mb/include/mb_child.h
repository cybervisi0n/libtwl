...
#define MB_ARM7_STATIC_RECV_BUFFER_SIZE                                        \
  (MB_ARM7_STATIC_RECV_BUFFER_END - MB_LOAD_AREA_HI)
#define MB_ARM7_STATIC_LOAD_AREA_HI                                            \
  (MB_LOAD_AREA_HI + MB_ARM7_STATIC_RECV_BUFFER_SIZE)
#define MB_ARM7_STATIC_LOAD_WRAM_MAX_SIZE                                      \
  (0x18000 - 0x1000) // 0x18000 = HW_WRAM_SIZE + HW_PRV_WRAM_SIZE

#define MB_BSSDESC_ADDRESS (HW_MAIN_MEM + 0x003fe800)

#define MB_DOWNLOAD_FILEINFO_ADDRESS                                           \
  (MB_BSSDESC_ADDRESS + (((MB_BSSDESC_SIZE) + (32) - 1) & ~((32) - 1)))

#define MB_CARD_ROM_HEADER_ADDRESS                                             \
  (MB_DOWNLOAD_FILEINFO_ADDRESS +                                              \
   (((MB_DOWNLOAD_FILEINFO_SIZE) + (32) - 1) & ~((32) - 1)))
...
int MB_CommDownloadRequest(
    int index); // Designate the parent device for which to request a connection
                // with the list number of MbBeaconRecvStatus
