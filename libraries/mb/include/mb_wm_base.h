...
#define MB_CALLBACK_API_ERROR                                                  \
  255 // Error on the value returned from an API call
...
typedef struct {
  ...
  const WMBssDesc *pInfo;  // Pointer to information for the connected parent
                           // (used by the child)
  u16 mode;                // MB_MODE_***
  u16 endReq;
  ...
  u16 contSend; // Flag indicating execution of continuous transmission (1:
                // Continuous transmission, 0: Per-frame communication)
  u8 reserved1[2];
