...
SDK_COMPILER_ASSERT(ROM_HEADER_SIZE_FULL + MB_AUTHCODE_SIZE +
                        sizeof(MBiCacheList) + sizeof(u32) * 4 +
                        MB_CACHE_PAGE_SIZE * 3 <=
...
static void MBi_MakeDownloadFileInfo(MbDownloadFileInfoHeader *mbdlFileInfop,
                                     const void *buf);
static void MBi_SetSegmentInfo(const RomHeader *mbRomHeaderp,
                               const MbSegmentType *loadSegListp,
                               MbSegmentInfo *seg_infop,
                               u32 *child_recv_buff_addr);
static void MBi_ReadSegmentHeader(const MBiSegmentHeaderInfo *p, u32 top,
                                  u32 bottom, BOOL clean);
static BOOL IsAbleToLoad(u8 segment_no, u32 address, u32 size);

u32 MB_GetSegmentLength(FSFile *file) {
  enum { ROM_HEADER_SIZE_SMALL = 0x60 };
...
BOOL MB_ReadSegment(FSFile *file, void *buf, u32 len) {
  ...
  if (!file) {
    ...
    if (!CARD_IsExecutableOnTWL(
            (const CARDRomHeader *)CARD_GetOwnRomHeader())) {
  ...
  if (len >= ROM_HEADER_SIZE_FULL + 4) {
    ...
    if (rest >= MB_AUTHCODE_SIZE) {
      if (is_own_binary) {
        MI_CpuCopy8(CARDi_GetOwnSignature(), dst,
    ...
    if (rest >= sizeof(MBiCacheList)) {
      ...
      MBi_AttachCacheBuffer(p_cache, 0, ROM_HEADER_SIZE_FULL, p_rom,
                            MB_CACHE_STATE_LOCKED);
    ...
    if (rest >= sizeof(u32) * 4) {
      ...
      p_map[1] = FS_GetFileImageTop(file) + bak_pos +
                 p_rom->main_rom_offset; /* ARM9 static */
      p_map[2] = FS_GetFileImageTop(file) + bak_pos +
                 p_rom->sub_rom_offset; /* ARM7 static */
    ...
    if (rest >= p_rom->main_size + p_rom->sub_size) {
      ...
      MBi_AttachCacheBuffer(p_cache, p_map[1], p_rom->main_size, dst,
                            MB_CACHE_STATE_LOCKED);
      ...
      MBi_AttachCacheBuffer(p_cache, p_map[2], p_rom->sub_size, dst,
                            MB_CACHE_STATE_LOCKED);
      ...
      (void)FS_SeekFile(file, (int)mainTop, FS_SEEK_SET);
      (void)FS_ReadFile(file, mainBuf, (int)mainSize);
      MBi_AttachCacheBuffer(p_cache, offset, MB_CACHE_PAGE_SIZE, dst,
                            MB_CACHE_STATE_LOCKED);
    ...
    if (is_own_binary) {
      ...
      if (ret) {
        ...
        info.mem_src =
            (u32)((u32)p_rom->main_ram_address - p_rom->main_rom_offset);
        info.mem_dst =
            (u32)((u32)p_cache->list[1].ptr - p_rom->main_rom_offset);
        info.len = len;

        MBi_ReadSegmentHeader(&info, MB_SEGMENT_HEADER_MIN,
                              MB_SEGMENT_HEADER_MAX, TRUE);

        for (; p_header->length != 0; ++p_header) {
          MBi_ReadSegmentHeader(&info, p_header->offset,
                                (u32)(p_header->offset + p_header->length),
                                FALSE);
        }
        {
          ...
          dst += ((u32)&_start_AutoloadDoneCallback -
                  (u32)p_rom->main_ram_address);
...
static void MBi_ReadSegmentHeader(const MBiSegmentHeaderInfo *p, u32 top,
  ...
  if (top < bottom) {
    if (clean) { /* 0 Initialize */
      MI_CpuClear8((void *)(p->mem_dst + top), (u32)(bottom - top));
    } else { /* Transfer */
      MI_CpuCopy8((const void *)(p->mem_src + top), (void *)(p->mem_dst + top),
                  (u32)(bottom - top));
    }
  }
}
...
BOOL MB_RegisterFile(const MBGameRegistry *game_reg, const void *buf) {
  ...
  MI_CpuCopy8(game_reg->userParam, ((MBDownloadFileInfo *)mdfi)->reserved,
              HW_DOWNLOAD_PARAMETER_SIZE);

  if (FALSE ==
      MBi_MakeBlockInfoTable(&pPwork->fileinfo[fileID].blockinfo_table, mdfi)) {
    OS_TWarning("Cannot make block information!\n");
    (void)OS_RestoreInterrupts(enabled); /* Remove interrupt prohibition */
    return FALSE;
  }

  MBi_MakeGameInfo(&pPwork->fileinfo[fileID].game_info, game_reg,
                   &pPwork->common.user);
  ...
  pPwork->fileinfo[fileID].cache_list =
      (MBiCacheList *)((u32)buf + ROM_HEADER_SIZE_FULL + MB_AUTHCODE_SIZE);
  pPwork->fileinfo[fileID].card_mapping =
      (u32 *)((u32)buf + ROM_HEADER_SIZE_FULL + MB_AUTHCODE_SIZE +
              sizeof(MBiCacheList));

  if (pPwork->fileinfo[fileID].cache_list->list[3].state !=
      MB_CACHE_STATE_EMPTY) {
...
void *MB_UnregisterFile(const MBGameRegistry *game_reg) {
  ...
  if (fileID != pPwork->fileinfo[fileID].game_info.fileNo) {
    OS_TWarning("Registerd File ID does not correspond with File ID in "
                "Registry List.\n");
    (void)OS_RestoreInterrupts(enabled); /* Remove interrupt prohibition */
    return NULL;
  }
...
static void MBi_MakeDownloadFileInfo(MbDownloadFileInfoHeader *mbdlFileInfop,
  ...
  MB_DEBUG_OUTPUT(
      "\t<segment list>         recv_adr load_adr     size  rom_adr (base)\n");

  for (seg_num = 0; seg_num < MB_DL_SEGMENT_NUM; seg_num++) {

    MB_DEBUG_OUTPUT("\t SEG%2d : ", seg_num);
    MBi_SetSegmentInfo(mbRomHeaderp, loadSegListp, seg_infop,
                       &child_recv_buff_addr);
    seg_infop++;
    loadSegListp++;
  }
...
static void MBi_SetSegmentInfo(const RomHeader *mbRomHeaderp,
  ...
  case MB_SEG_ARM7STATIC: {
    ...
    if (((u32)romRegp->offset >= MB_LOAD_AREA_LO) &&
      ...
      if (load_last_addr <=
          MB_ARM7_STATIC_LOAD_AREA_HI) // Set as is if relocation is not
                                       // necessary.
      {
      } else if ((load_last_addr < MB_BSSDESC_ADDRESS) &&
                 (romRegp->length <=
                  MB_ARM7_STATIC_RECV_BUFFER_SIZE)) { // If there is a need to
                                                      // relocate, behave like
                                                      // so
        reload_flag = TRUE;
      } else {
        error_flag = TRUE;
        ...
        if (((u32)romRegp->offset >= HW_WRAM) &&
            ((u32)romRegp->offset <
             HW_WRAM + MB_ARM7_STATIC_LOAD_WRAM_MAX_SIZE)) {

      if (load_last_addr <=
          (HW_WRAM + MB_ARM7_STATIC_LOAD_WRAM_MAX_SIZE)) { // To be relocated
        reload_flag = TRUE;
      } else {
        error_flag = TRUE;
      }
    } else { // An error when in regions other than this
      error_flag = TRUE;
    }
  ...
  MB_DEBUG_OUTPUT(" %8x %8x %8x %8x\n", seg_infop->recv_addr,
                  seg_infop->load_addr, seg_infop->size,
                  (*loadSegListp == MB_SEG_ROMHEADER) ? 0
                                                      : *((u32 *)romRegp - 2));
}
...
BOOL MBi_MakeBlockInfoTable(MB_BlockInfoTable *table,
  ...
  for (i = 0; i < MB_DL_SEGMENT_NUM; ++i) {
    MbSegmentInfo *si = MBi_GetSegmentInfo(mdfi, i);
    u16 next_block_head =
        (u16)(seg_headB[i] + (u16)((si->size + mbc->block_size_max - 1) /
                                   mbc->block_size_max));
...
BOOL MBi_get_blockinfo(MB_BlockInfo *bi, MB_BlockInfoTable *table, u32 block,
  ...
  MB_DEBUG_OUTPUT(
      ...
      seg_block, bi->offset, bi->child_address, bi->size);
  ...
  case MB_SEG_ARM7STATIC:
    ...
    if (address >= MB_LOAD_AREA_LO && address < MB_BSSDESC_ADDRESS) {
...

      {
        ...
        return TRUE; // OK
      } else if (end_adr < MB_BSSDESC_ADDRESS &&
                 size <= MB_ARM7_STATIC_RECV_BUFFER_SIZE) {
        ...
        if (address >= HW_WRAM &&
            address < HW_WRAM + MB_ARM7_STATIC_LOAD_WRAM_MAX_SIZE) {
      u32 end_adr = address + size;
