...
static void MBi_CommBeaconRecvCallback(MbBeaconMsg msg,
                                       MBGameInfoRecvList *gInfop, int index);
...
int MB_CommDownloadRequest(int index) {
  ...
  const WMBssDesc *bssDescp;       // Pointer to the parent information of the
  ...
  pCwork->connectTargetNo = index; // Saves the list number of the parent to
                                   // which to attempt a connection
  pCwork->fileid = fileNo;         // Register the requested file ID in advance
  pCwork->last_recv_seq_no =
      -1; // Initialize the last received block sequence number

  MI_CpuCopy16(bssDescp, &pCwork->bssDescbuf, WM_BSS_DESC_SIZE);

  ret = MBi_CommConnectToParent(
      (const WMBssDesc *)&pCwork->bssDescbuf); // First, establish a connection
...
static int MBi_CommRequestFile(u8 file_no, u32 ggid) {
  ...
  errcode = MBi_BlockHeaderEnd(MB_COMM_CHILD_HEADER_SIZE, 0xFFFF,
                               pCwork->common.sendbuf);
  return errcode;
}
...
BOOL MB_CommStartDownload(void) {
  ...
  if (pCwork && pCwork->c_comm_state == MB_COMM_CSTATE_DLINFO_ACCEPTED &&
...
void MBi_CommChildCallback(u16 type, void *arg) {
  ...
  case MB_CALLBACK_PARENT_FOUND: {
    ...
    (void)MB_RecvGameInfoBeacon(MBi_CommBeaconRecvCallback, *linkLevel,
                                bssDescp);
  ...
  case MB_CALLBACK_END_COMPLETE:
    ...
    if (pCwork->c_comm_state == MB_COMM_CSTATE_BOOTREQ_ACCEPTED &&
        pCwork->boot_end_flag == 1) {

      MI_CpuCopy16(&pCwork->dl_fileinfo, (void *)MB_DOWNLOAD_FILEINFO_ADDRESS,
                   sizeof(MBDownloadFileInfo));
      MI_CpuCopy16(&pCwork->bssDescbuf, (void *)MB_BSSDESC_ADDRESS,
                   MB_BSSDESC_SIZE);
      MBi_CommChangeChildState(MB_COMM_CSTATE_BOOT_READY,
                               NULL); // Boot preparation completed
    }
  ...
  case MB_CALLBACK_CONNECT_FAILED:
    ...
    MB_DeleteRecvGameInfo(
        pCwork->connectTargetNo); // Delete the game information of a parent
...
static void MBi_CommChildRecvData(void *arg) {
  ...
  case MB_COMM_TYPE_PARENT_MEMBER_FULL: // Message from parent regarding
  ...
  case MB_COMM_TYPE_PARENT_DL_FILEINFO:
    ...
    if (p_child->c_comm_state == MB_COMM_CSTATE_REQ_ENABLE) {
      ...
      MB_DEBUG_OUTPUT(
          "Download File Info has received (Total block num = %d)\n",
          p_child->total_block);
      if (!MBi_MakeBlockInfoTable(
              &p_child->blockinfo_table,
      ...
      MBi_CommChangeChildState(MB_COMM_CSTATE_DLINFO_ACCEPTED,
                               (void *)&p_child->dl_fileinfo);
    }
  ...
  case MB_COMM_TYPE_PARENT_DATA:
    ...
    if (p_child->c_comm_state == MB_COMM_CSTATE_RECV_PROCEED) {
      ...
      if (hd.seqno < 0 || hd.seqno >= block_num ||
        ...
        OS_TWarning(
            "The illegal block No.[%d] has been received! (maxnum %d)\n",
            hd.seqno, block_num);
      ...
      if (!MBi_IsAbleToRecv(bi.segment_no, bi.child_address, bi.size)) {
        ...
        OS_TWarning(
            "The receive address of Block No.%d is illegal. [%08x - %08x]\n",
      ...
      if (checkRecvFlag(hd.seqno) == FALSE) {
        MB_DEBUG_OUTPUT(
            "DATA : BLOCK(%d)/REMAIN(%d), Recv address[%x] size[%x]\n",
    ...
    CheckRemainBlock:
      ...
      if (0 == countRemainingblocks()) {
        MBi_CommChangeChildState(MB_COMM_CSTATE_RECV_COMPLETE,
                                 NULL); // Receive complete
      }
    }
    break;
...
static int MBi_CommChildSendData(void) {
  ...
  default:
    ...
    errcode = MBi_BlockHeaderEnd(MB_COMM_CHILD_HEADER_SIZE, pollbmp,
                                 pCwork->common.sendbuf);
    break;
  ...
  case MB_COMM_CSTATE_REQ_ENABLE: {
    ...
    errcode = MBi_CommRequestFile(
        pCwork->fileid, mbrsp->list[pCwork->connectTargetNo].gameInfo.ggid);
...
void MB_ReadMultiBootParentBssDesc(WMBssDesc *p_desc, u16 parent_max_size,
  ...
  p_desc->gameInfo.ggid = (u32)(*(const u16 *)(&p_desc->ssid[0]) |
                                (*(const u16 *)(&p_desc->ssid[2]) << 16));
...
static void clearRecvRegion(void) {
  ...
  MI_CpuClearFast((void *)MB_ARM7_STATIC_RECV_BUFFER,
                  MB_ARM7_STATIC_RECV_BUFFER_SIZE);

  MI_CpuClear16((void *)MB_BSSDESC_ADDRESS, MB_DOWNLOAD_FILEINFO_ADDRESS +
                                                MB_DOWNLOAD_FILEINFO_SIZE -
                                                MB_BSSDESC_ADDRESS);
