...
SDK_COMPILER_ASSERT(32 + sizeof(MBiParam) + 32 + sizeof(MB_CommPWork) + 32 +
                        WM_SYSTEM_BUF_SIZE <=
                    MB_SYSTEM_BUF_SIZE);
SDK_COMPILER_ASSERT(32 + sizeof(MBiParam) + 32 + sizeof(MB_CommCWork) + 32 +
                        WM_SYSTEM_BUF_SIZE <=
...
#define WM_CalcChildSendBufSize(_pInfo_)                                       \
  (u16)(WM_SIZE_MP_CHILD_SEND_BUFFER(                                          \
      ((WMGameInfo *)(&(((WMBssDesc *)(_pInfo_))->gameInfo)))->childMaxSize,   \
      FALSE))
#define WM_CalcChildRecvBufSize(_pInfo_)                                       \
  (u16)(WM_SIZE_MP_CHILD_RECEIVE_BUFFER(                                       \
      ((WMGameInfo *)(&(((WMBssDesc *)(_pInfo_))->gameInfo)))->parentMaxSize,  \
      FALSE))

#define WM_CalcParentSendBufSize(_libParam_)                                   \
  (u16)(WM_SIZE_MP_PARENT_SEND_BUFFER(                                         \
      ((MBiParam *)(_libParam_))->parentParam.parentMaxSize, FALSE))
#define WM_CalcParentRecvBufSize(_libParam_)                                   \
  (u16)(WM_SIZE_MP_PARENT_RECEIVE_BUFFER(                                      \
      ((MBiParam *)(_libParam_))->parentParam.childMaxSize, WM_NUM_MAX_CHILD,  \
      FALSE))
...
static BOOL changeScanChannel(WMscanExParam *p) {
  ...
  for (i = 0, channel = p->channelList; i < 16;
       i++, channel = (u16)((channel == 0) ? 1 : channel << 1)) {
...
void MB_SetPowerSaveMode(BOOL enable) { mbi_power_save_mode = enable; }
...
static BOOL MBi_IsSendEnabled(void) {
  return (p_mbi_param->mpStarted == 1) && (p_mbi_param->mpBusy == 0) &&
...
static void MBi_OnInitializeDone(void) {
  ...
  errcode = WM_SetLifeTime(MBi_ParentCallback, mbi_life_table_no, mbi_life_cam,
...
static void MBi_ParentCallback(void *arg) {
  ...
  case WM_APIID_SET_LIFETIME: {
    ...
    errcode =
        WM_SetParentParameter(MBi_ParentCallback, &p_mbi_param->parentParam);
    ...
    case WM_STATECODE_CHILD_CONNECTED:
      ...
      if ((p_mbi_param->mpStarted == 0) && (!mbc->start_mp_busy)) {
        ...
        errcode = WM_StartMPEx(
            MBi_ParentCallback, (u16 *)p_mbi_param->recvBuf,
            p_mbi_param->recvBufSize, (u16 *)p_mbi_param->sendBuf,
            p_mbi_param->sendBufSize, (u16)(p_mbi_param->contSend ? 0 : 1),
            0,               /* defaultRetryCount */
  ...
  case WM_APIID_START_MP: {
    ...
    case WM_STATECODE_MPEND_IND:
      ...
      p_mbi_param->callback(MB_CALLBACK_MP_PARENT_RECV,
                            (void *)(callback->recvBuf));
      break;
  ...
  case WM_APIID_SET_MP_DATA: {
    ...
    if (pPwork->useWvrFlag) {
      MbWaitForWvr(13000); // Make ARM9 wait for approximately 3 lines during an
                           // IRQ interrupt.
    }
    ...
    if (buf->errcode == WM_ERRCODE_SUCCESS) {
      ...
      if (p_mbi_param->endReq == 0) {
        p_mbi_param->callback(MB_CALLBACK_MP_SEND_ENABLE,
                              NULL); // Allow the next transmission.
      }
...
static void MBi_ChildCallback(void *arg) {
  ...
  case WM_APIID_INITIALIZE: {
    ...
    errcode = WM_SetLifeTime(MBi_ChildCallback, mbi_life_table_no, mbi_life_cam,
                             mbi_life_frame, mbi_life_mp);
    MBi_CheckWmErrcode(WM_APIID_SET_LIFETIME, errcode);
  } break;
  ...
  case WM_APIID_START_SCAN_EX: {
...

    {
      ...
      for (j = 0; j < tmpBuf->bssDescCount; j++) {
        ...
        if (isEnableReject == TRUE &&
            ((buffer->gameInfo.ggid & rejectGgidMask) ==
             (rejectGgid & rejectGgidMask))) {
          numInvalidBeacon++;
        ...
        for (i = 0; i < p_param->found_parent_count; ++i) {
          ...
          if (WM_IsBssidEqual(buffer->bssid, p[i].scan_data.macAddress)) {
            ...
            DC_InvalidateRange(p_param->parent_info[i].parentInfo,
                               WM_BSS_DESC_SIZE);
            MI_DmaCopy16(WM_DMA_NO, buffer, p_param->parent_info[i].parentInfo,
                         WM_BSS_DESC_SIZE);
            p_param->last_found_parent_no = i;
            p_param->callback(MB_CALLBACK_PARENT_FOUND, &tmpBuf->linkLevel[j]);
            goto scan_end; // In the meantime, do not receive multiple MB
                           // beacons
          }
        }
        ...
        if (i < MB_NUM_PARENT_INFORMATIONS) {
          ...
          MI_CpuCopy16(&buffer->gameInfo, &p[i].scan_data.gameInfo,
                       WM_SIZE_GAMEINFO);
      ...
      if (tmpBuf->bssDescCount == numInvalidBeacon) {
        p_param->callback(MB_CALLBACK_PARENT_NOT_FOUND,
                          arg); // This callback to ParentInfoLifeTimeCount
      }
  ...
  case WM_APIID_START_CONNECT: {
    ...
    case WM_STATECODE_CONNECTED:
      ...
      errcode = WM_StartMPEx(
          MBi_ChildCallback, (u16 *)p_param->recvBuf, p_param->recvBufSize,
          (u16 *)p_param->sendBuf, p_param->sendBufSize,
          (u16)(p_param->contSend ? 0 : 1), 0, /* defaultRetryCount */
          FALSE, FALSE, 1,                     /* fixFreqMode */
          TRUE                                 /* ignoreFatalError */
      );
      MBi_CheckWmErrcode(WM_APIID_START_MP, errcode);
...
static BOOL MBi_IsCommSizeValid(u16 sendSize, u16 recvSize, u16 entry_num) {
  ...
  if (recvSize > MB_COMM_P_RECVLEN_MAX || recvSize < MB_COMM_P_RECVLEN_MIN) {
    OS_TWarning("MB Parent receive buffer size per child is out of the "
                "range.[%3d - %3d Bytes]\n",
...
BOOL MB_SetParentCommParam(u16 sendSize, u16 maxChildren) {
  ...
  if (FALSE ==
      MBi_IsCommSizeValid(sendSize, MB_COMM_P_RECVLEN_DEFAULT, maxChildren)) {
...
u16 MB_GetTgid(void) { return p_mbi_param->parentParam.tgid; }
...
static int MBi_StartParentCore(int channel) {
  ...
  for (i = 0; i < MB_MAX_CHILD; i++) {
    ...
    pPwork->fileid_of_child[i] =
        -1; /* Initialize the requested FileID from the child. */
  }
  pPwork->file_num = 0;
...
void MB_End(void) {
  ...
  if (mbc->is_started_ex) {
    OS_TPanic("MB_End called after MB_StartParentFromIdle! (please call "
              "MB_EndToIdle)");
  }
...
int MBi_GetLastFountParent(void) { return p_mbi_param->last_found_parent_no; }
...
void MBi_SetMaxScanTime(u16 time) { mbiScanParam.maxChannelTime = time; }

static int MBi_SetMPData(WMCallbackFunc callback, const u16 *sendData,
                         u16 sendDataSize, u16 tmptt, u16 pollbmp) {
#pragma unused(tmptt)
  int errcode;
  errcode = WM_SetMPDataToPort(callback, sendData, sendDataSize, pollbmp,
                               WM_PORT_BT, WM_PRIORITY_LOW);
...
int MBi_SendMP(const void *buf, int len, int pollbmp) {
  ...
  case MB_MODE_PARENT:
    errcode = MBi_SetMPData(
        p_mbi_param->callback_ptr, (u16 *)buf,
        size, // * If MBi_ParentCallback is used, ParentCallback will be linked
              // in mb_child as well, so callback_ptr should be used instead.
...
int MBi_GetSendBufSize(void) { return (int)p_mbi_param->sendBufSize; }

int MBi_GetRecvBufSize(void) { return (int)p_mbi_param->recvBufSize; }
...
u32 MBi_GetGgid(void) { return p_mbi_param->parentParam.ggid; }

u16 MBi_GetTgid(void) { return (p_mbi_param->parentParam.tgid); }

u8 MBi_GetAttribute(void) {
  return (
      (u8)(((p_mbi_param->parentParam.entryFlag) ? WM_ATTR_FLAG_ENTRY
                                                 : 0) | // entryFlag lowest bit
           ((p_mbi_param->parentParam.multiBootFlag)
                ? WM_ATTR_FLAG_MB
                : 0) | // multiBootFlag second bit
           ((p_mbi_param->parentParam.KS_Flag) ? WM_ATTR_FLAG_KS
                                               : 0) | // KS_Flag third bit
           ((p_mbi_param->parentParam.CS_Flag) ? WM_ATTR_FLAG_CS
                                               : 0) // CS_Flag fourth bit
           ));
}
...
int MBi_GetScanChannel(void) { return mbiScanParam.channelList; }

u16 MBi_GetAid(void) { return p_mbi_param->my_aid; }
