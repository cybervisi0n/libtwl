...
static u8 *MBi_ReceiveRequestDataPiece(const MBCommChildBlockHeader *hdr,
                                       u16 child);
...
u8 *MBi_SetRecvBufferFromChild(const u8 *recvbuf, MBCommChildBlockHeader *hdr,
                               u16 child_id) {
  u8 *ptr = (u8 *)recvbuf;
  ...
  *pbuf++ = hdr->type;
  ...
  case MB_COMM_TYPE_CHILD_FILEREQ:
    *pbuf++ = hdr->req_data.piece;
    ...
    MI_CpuCopy8((void *)(hdr->req_data.data), (void *)pbuf,
                (u32)req_data_piece.size);
  ...
  case MB_COMM_TYPE_CHILD_CONTINUE:
    ...
    *pbuf++ = (u8)((0xff00 & hdr->data.req) >> 8); // Hi
    MI_CpuCopy8((void *)(hdr->data.reserved), (void *)pbuf,
                (u32)req_data_piece.size);
    pbuf += req_data_piece.size;
    break;
...
u8 MBi_SendRequestDataPiece(u8 *pData, const MBCommRequestData *pReq) {
  ...
  MI_CpuCopy8((void *)&ptr[req_data_piece_idx * req_data_piece.size], pData,
              (u32)req_data_piece.size);
...
u8 *MBi_SetRecvBufferFromParent(MBCommParentBlockHeader *hdr,
                                const u8 *recvbuf) {
