...
typedef struct {
  u32 sendBuf[WM_SIZE_CHILD_SEND_BUFFER(MB_COMM_PARENT_RECV_MAX, FALSE) /
              sizeof(u32)] ATTRIBUTE_ALIGN(32); // Send buffer  0x400 byte
  u32 recvBuf[WM_SIZE_CHILD_RECEIVE_BUFFER(MB_COMM_PARENT_SEND_MAX, FALSE) /
              sizeof(u32)] ATTRIBUTE_ALIGN(32); // Receive buffer 0xF00 byte
  u32 sendDataBuf[WM_SIZE_CHILD_SEND_BUFFER(MB_COMM_PARENT_RECV_MAX, FALSE) /
                  sizeof(u32)] ATTRIBUTE_ALIGN(32); // Buffer for creating send
                                                    // data 0x400 byte
  WMBssDesc
      bssDescBuf ATTRIBUTE_ALIGN(32); // Buffer for scanning parents. Needs to
                                      // be 32-byte aligned   0xC0 byte
  WMScanParam scanParam ATTRIBUTE_ALIGN(
      32); // Scan parameter. Needs to be 32-byte aligned  0x20 byte
  MBWMWork wmWork ATTRIBUTE_ALIGN(
      32); // Work buffer for MP communication. Needs to be 32-byte aligned
  ...
  BOOL endFlag;                       // MB termination process request
  u32 targetGgid;                     // GGID of binary to request
  u16 targetFileNo;                   // FileNo of binary to request
  u16 c_comm_state;                   // Child status
  WMCallbackFunc verboseScanCallback; // Callback that alerts the user of the
                                      // the raw WMScan notification
  MBFakeCompareGGIDCallbackFunc
      compareGGIDCallback; // User-defined GGID comparison callback
  u8 _padding2[8];
} MBFakeWork;
...
static void MBFi_CommBeaconRecvCallback(MbBeaconMsg msg,
...
u32 MB_FakeGetWorkSize(void) { return sizeof(MBFakeWork); }
...
static void MBFi_ScanCallback(u16 state, MBGameInfoRecvList *gInfop,
                              int index) {
...
static void MBFi_CommChildRecvData(WMPortRecvCallback *cb) {
  ...
  case MB_COMM_TYPE_PARENT_DATA:
    ...
    if (mbf_work->c_comm_state == MB_COMM_CSTATE_RECV_PROCEED) {
      MBFi_CommChangeChildState(MB_COMM_CSTATE_RECV_COMPLETE,
...
static void MBFi_CommChildSendData(void) {
  ...
  case MB_COMM_CSTATE_REQ_ENABLE: {
    ...
    errcode = MBi_MPSendToParent(MB_COMM_CHILD_HEADER_SIZE, pollbmp,
                                 mbf_work->sendDataBuf);
  } break;
...
BOOL MB_FakeReadParentBssDesc(u16 index, WMBssDesc *pBssDesc,
                              u16 parent_max_size, u16 child_max_size,
                              BOOL ks_flag, BOOL cs_flag) {
  BOOL result;
  ...
  pBssDesc->gameInfo.attribute =
      (u8)((ks_flag ? WM_ATTR_FLAG_KS : 0) | (cs_flag ? WM_ATTR_FLAG_CS : 0) |
           WM_ATTR_FLAG_ENTRY);
  return TRUE;
}
