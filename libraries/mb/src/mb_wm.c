...
void MBi_WMStartConnect(WMBssDesc *bssDesc) {
  ...
  wmWork->sendBufSize =
      (u16)WM_SIZE_MP_CHILD_SEND_BUFFER(bssDesc->gameInfo.childMaxSize, FALSE);
  wmWork->recvBufSize = (u16)WM_SIZE_MP_CHILD_RECEIVE_BUFFER(
      bssDesc->gameInfo.parentMaxSize, FALSE);
...
void MBi_ChildStartMP(u16 *sendBuf, u16 *recvBuf) {
  ...
  result = WM_StartMPEx(ChildStateOutStartMP, recvBuf, wmWork->recvBufSize,
                        sendBuf, wmWork->sendBufSize, 1, // mpFreq
                        0,                               // defaultRetryCount
...
static BOOL IsSendEnabled(void) {
  return (wmWork->mpStarted == 1) && (wmWork->mpBusy == 0) &&
         (wmWork->endReq == 0);
}
...
WMErrCode MBi_MPSendToParent(u32 body_len, u16 pollbmp, u32 *sendbuf) {
  ...
  result =
      WM_SetMPDataToPort(StateOutMPSendToParent, (u16 *)sendbuf, (u16)body_len,
