...
static void MBi_CommChangeParentStateCallbackOnly(u16 child, int state,
                                                  void *arg);
...
const MBUserInfo *MB_CommGetChildUser(u16 child_aid) {
  ...
  if (pPwork && IsChildAidValid(child_aid)) {
    MI_CpuCopy8(&pPwork->childUser[child_aid - 1], &pPwork->childUserBuf,
                sizeof(MBUserInfo));
    (void)OS_RestoreInterrupts(enabled); /* Remove interrupt prohibition */
...
u16 MB_GetGameEntryBitmap(const MBGameRegistry *game_reg) {
  ...
  for (i = 0; i < MB_MAX_FILE; i++) {
    if ((pPwork->fileinfo[i].active) &&
        ((u32)pPwork->fileinfo[i].game_reg == (u32)game_reg)) {
...
void MBi_CommParentCallback(u16 type, void *arg) {
  ...
  case MB_CALLBACK_CHILD_DISCONNECTED: {
    ...
    if (pPwork->p_comm_state[sparg->aid - 1] == MB_COMM_PSTATE_BOOT_REQUEST) {
      MBi_CommChangeParentState(sparg->aid, MB_COMM_PSTATE_BOOT_STARTABLE,
                                NULL);
    }
...
static void MBi_CommParentRecvDataPerChild(void *arg, u16 child) {
  ...
  p_data = MBi_SetRecvBufferFromChild((u8 *)&((WMMpRecvData *)arg)->cdata[0],
                                      &hd, child);
    ...
    if (p_data != NULL) {
      ...
      if (state == MB_COMM_PSTATE_CONNECTED) {
        ...
        MB_DEBUG_OUTPUT("Child [%2d] MB_IPL_VERSION : %04x\n", child,
                        req_data.version);
        MI_CpuCopy8(&req_data.userinfo, &pPwork->childUser[child - 1],
                    sizeof(MBUserInfo));
        pPwork->childUser[child - 1].playerNo = child;

        MBi_CommChangeParentState(child, MB_COMM_PSTATE_REQUESTED,
                                  &req_data.userinfo);
      }
      ...
      if (state == MB_COMM_PSTATE_REQUESTED) {
        ...
        if (fileid >= MB_MAX_FILE || pPwork->fileinfo[fileid].active == 0 ||
            pPwork->childggid[child - 1] !=
                pPwork->fileinfo[fileid].game_reg->ggid) {
  ...
  case MB_COMM_TYPE_CHILD_STOPREQ:
    if (state == MB_COMM_PSTATE_SEND_PROCEED) {
      ...
      MBi_CommChangeParentState(child, MB_COMM_PSTATE_SEND_COMPLETE,
                                NULL); // Send completed
...
static int MBi_CommParentSendMsg(u8 type, u16 pollbmp) {
  ...
  return MBi_BlockHeaderEnd(MB_COMM_PARENT_HEADER_SIZE, pollbmp,
    ...
    if (pPwork->fileinfo[i].active) {
      if (file_req_num[i] > 0) {
        ...
        if (send_candidate_fid == -1 ||
            file_req_num[i] > file_req_num[send_candidate_fid]) {
...
static void MBi_ReloadCache(MBiTaskInfo *p_task) {
  ...
  if (OS_IsRunOnTwl() && (arc == FS_FindArchive("rom", 3))) {
    const CARDRomHeaderTWL *header =
        (const CARDRomHeaderTWL *)CARD_GetOwnRomHeaderTWL();

    const u32 boundary_ntr =
        header->digest_area_ntr.offset + header->digest_area_ntr.length;
    const u32 boundary_ltd =
        header->digest_area_ltd.offset + header->digest_area_ltd.length;
    if ((p_info->src < boundary_ntr) && (p_info->src + length > boundary_ntr)) {
      length = boundary_ntr - p_info->src;
    } else if ((p_info->src < boundary_ltd) &&
               (p_info->src + length > boundary_ltd)) {
  ...
  if (FS_OpenFileDirect(file, arc, p_info->src, p_info->src + length,
                        (u32)~0)) {
...
static int MBi_CommParentSendBlock(void) {
  ...
  if (!MBi_get_blockinfo(
          &bi, &pPwork->fileinfo[pPwork->cur_fileid].blockinfo_table,
...

  {
    ...
    u32 card_addr =
        (u32)(bi.offset -
              pPwork->fileinfo[pPwork->cur_fileid]
                  .blockinfo_table.seg_src_offset[bi.segment_no] +
...
static int MBi_CommParentSendData(void) {
  ...
  {
    errcode =
        MBi_CommParentSendMsg(MB_COMM_TYPE_PARENT_SENDSTART, bmp.connected);
  } else if (bmp.mem_full) // Send member exceeded message
  {
    errcode =
        MBi_CommParentSendMsg(MB_COMM_TYPE_PARENT_MEMBER_FULL, bmp.mem_full);
...
static inline u16 max(u16 a, u16 b) { return (u16)((a > b) ? a : b); }
