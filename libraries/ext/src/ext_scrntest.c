...
static GXVRamLCDC vramForCapture_ =
    GX_VRAM_LCDC_C; // default VRAM for cap. is VRAM_C
...
void EXT_TestScreenShot(u32 testFrame, u32 checkSum) {
  ...
  SDK_TASSERTMSG((testFrame > 0) && (testFrame < (u32)COUNTER_MAX_),
...
void EXT_TestTickCounter() { frameCounter++; }

void EXT_TestResetCounter() { frameCounter = 0; }

void EXT_TestSetVRAMForScreenShot(GXVRamLCDC vram) {

  SDK_TASSERTMSG(vram == GX_VRAM_LCDC_A || vram == GX_VRAM_LCDC_B ||
                     vram == GX_VRAM_LCDC_C || vram == GX_VRAM_LCDC_D,
                 "Currentry VRAM_A B C D are supported for capturing.");
...
static void startCapture_() {
  ...
  GX_SetCapture(GX_CAPTURE_SIZE_256x192, getCaptureMode_(dispMode),
                GX_CAPTURE_SRCA_2D3D, getCaptureSrcB_(dispMode),
...
static GXDispMode getDispMode_() {
  const GXDispMode ret =
      (GXDispMode)((reg_GX_DISPCNT & REG_GX_DISPCNT_MODE_MASK) >>
                   REG_GX_DISPCNT_MODE_SHIFT);
