...
#ifdef SDK_ARM9
#ifdef SDK_TS
#define MIi_DMA_CLEAR_DATA_BUF                                                 \
  REG_DMA0_CLR_DATA_ADDR // ARM9-TS:  defined ARM9/ioreg_MI.h
#else
#define MIi_DMA_CLEAR_DATA_BUF                                                 \
  HW_DMA_CLEAR_DATA_BUF // ARM9-TEG: defined common/mmap_shared.h
#endif
#else // ifdef SDK_ARM9
#define MIi_DMA_CLEAR_DATA_BUF                                                 \
  HW_PRV_WRAM_DMA_CLEAR_DATA_BUF // ARM7:  defined ARM7/mmap_wram.h
...
static inline void MIi_DmaSetParams_wait(u32 dmaNo, u32 src, u32 dest,
                                         u32 ctrl) {
  OSIntrMode enabled = OS_DisableInterrupts();
  vu32 *p = (vu32 *)((u32)REG_DMA0SAD_ADDR + dmaNo * 12);
  *p = (vu32)src;
...
static inline void MIi_DmaSetParams_noInt(u32 dmaNo, u32 src, u32 dest,
                                          u32 ctrl) {
  vu32 *p = (vu32 *)((u32)REG_DMA0SAD_ADDR + dmaNo * 12);
  *p = (vu32)src;
  *(p + 1) = (vu32)dest;
...
static inline void MIi_DmaSetParams_wait_noInt(u32 dmaNo, u32 src, u32 dest,
                                               u32 ctrl) {
  vu32 *p = (vu32 *)((u32)REG_DMA0SAD_ADDR + dmaNo * 12);
  *p = (vu32)src;
  *(p + 1) = (vu32)dest;
...
static inline void MIi_DmaSetParams_src32(u32 dmaNo, u32 data, u32 dest,
                                          u32 ctrl) {
  OSIntrMode enabled = OS_DisableInterrupts();
...
static inline void MIi_DmaSetParams_src16(u32 dmaNo, u16 data, u32 dest,
                                          u32 ctrl) {
  OSIntrMode enabled = OS_DisableInterrupts();
...
static inline void MIi_DmaSetParams_wait_src32(u32 dmaNo, u32 data, u32 dest,
                                               u32 ctrl) {
  OSIntrMode enabled = OS_DisableInterrupts();
...
static inline void MIi_DmaSetParams_wait_src16(u32 dmaNo, u16 data, u32 dest,
                                               u32 ctrl) {
  OSIntrMode enabled = OS_DisableInterrupts();
...
#define MIi_ASSERT_SIZE(dmaNo, size)                                           \
  SDK_TASSERTMSG((size) <= 0x1fffff, "size too large");
#else // SDK_ARM7
#define MIi_ASSERT_SIZE(dmaNo, size)                                           \
  do {                                                                         \
    if (dmaNo <= 2) {                                                          \
...
#define MIi_DUMMY_CNT                                                          \
  (MI_DMA_ENABLE | MI_DMA_SRC_FIX | MI_DMA_DEST_FIX | MI_DMA_16BIT_BUS | 1)
