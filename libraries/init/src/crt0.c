...
#define SDK_VERSION_ID                                                         \
  ((u32)SDK_VERSION_MAJOR << 24 | (u32)SDK_VERSION_MINOR << 16 |               \
      ...
      @wait_vcount_0 : ldrh r0,
                       ...
                       #0 bne @wait_vcount_0
                           ...
                           bl init_cp15
                               ...
                               mov r0,
                       #HW_PSR_SVC_MODE msr cpsr_c,
                       r0 ldr r0,
                       = INITi_HW_DTCM add r0,
                       r0,
                       #0x3fc0 mov sp,
                       r0

                           mov r0,
                       #HW_PSR_IRQ_MODE msr cpsr_c,
                       r0 ldr r0,
                       = INITi_HW_DTCM add r0,
                       r0,
                       #0x3fc0 sub r0,
                       r0,
                       #HW_SVC_STACK_SIZE sub sp,
                       r0,
                       ...
                       mov r0,
                       #0 ldr r1,
                       = INITi_HW_DTCM mov r2,
                       #HW_DTCM_SIZE bl INITi_CpuClear32

                           mov r0,
                       #0 ldr r1,
                       = HW_PLTT mov r2,
                       #HW_PLTT_SIZE bl INITi_CpuClear32

                           mov r0,
                       #0x0200 ldr r1,
                       = HW_OAM mov r2,
                       #HW_OAM_SIZE bl INITi_CpuClear32

                           ldr r1,
                       = _start_ModuleParams ldr r0,
                       [ r1, #20 ] // r0 = bottom of compressed data
                       bl MIi_UncompressBackward bl do_autoload

                           ldr r0,
                       = _start_ModuleParams ldr r1,
                       [ r0, #12 ] // BSS segment start
                       ldr r2,
                       [ r0, #16 ] // BSS segment end
                       mov r3,
                       r1 // For next step (flush bss)
                           mov r0,
                       #0 @1
      : cmp r1,
        r2 strcc r0,
        ...
        add r1,
        r1,
        #HW_CACHE_LINE_SIZE cmp r1,
        r2 blt @cacheflush

            ldr r1,
        = HW_COMPONENT_PARAM str r0,
        [ r1, #0 ]

        ldr r1,
        = INITi_HW_DTCM add r1,
        r1,
        #0x3fc0 add r1,
        r1,
        ...
        bl _fp_init bl NitroStartUp bl __call_static_initializers
#endif
...
#define index r7
#define len r12
  cmp bottom, #0 beq @exit stmfd sp !, {r4 - r7} ldmdb bottom,
...
#define tmp r7
  ...
  ldr ptable, = _start_ModuleParams ldr infop,
      [ ptable, #0 ] // r1 = start pointer to autoload_info
      ldr infop_end,
      [ ptable, #4 ] // r2 = end pointer to autoload_info
      ldr src,
      [ ptable, #8 ] // r3 = autoload block

      @2 : cmp infop,
      infop_end // Does it get to the end?
               beq @skipout
      ...
      tmp // dest_end
               mov dest,
      dest_begin // dest working pointer
           @1 : cmp dest,
      dest_end ldrmi tmp,
      [src],
      #4 // [dest++] <- [src++]
      ...
      1 // DCache
              add dest,
      dest,
      #HW_CACHE_LINE_SIZE cmp dest,
      dest_end blt @cacheflush
...
static asm void init_cp15(void){
    ...
    c0,
    0 ldr r1,
    = HW_C1_ICACHE_ENABLE | HW_C1_DCACHE_ENABLE | HW_C1_ITCM_ENABLE |
      HW_C1_DTCM_ENABLE | HW_C1_ITCM_LOAD_MODE | HW_C1_DTCM_LOAD_MODE |
      HW_C1_LD_INTERWORK_DISABLE | HW_C1_PROTECT_UNIT_ENABLE bic r0,
    r0,
    r1 mcr p15,
    0,
    r0,
                    ...
                    SET_PROTECTION_B(c2, HW_MAIN_MEM_IM_SHARED, 4KB)
                        ...
                        SET_PROTECTION_A(c3, HW_CTRDG_ROM, 128MB)
                            SET_PROTECTION_B(c3, HW_CTRDG_ROM, 128MB)
                                ...
                                ldr r0,
    = SDK_AUTOLOAD_DTCM_START orr r0,
    r0,
    #HW_C6_PR_16KB orr r0,
    r0,
    #HW_C6_PR_ENABLE SET_PROTECTION_B(c4, HW_DTCM, 16KB)
...
#pragma message(ERROR : Size unmatch HW_MAIN_MEM_SHARED_SIZE)
    ...
    mov r0,
    #REGION_BIT(0, 1, 0, 1, 0, 0, 1, 0) mcr p15,
    0,
    r0,
    c2,
    c0,
    ...
    mov r0,
    #REGION_BIT(0, 1, 0, 1, 0, 0, 0, 0) mcr p15,
    0,
    r0,
    c3,
    c0,
    ...
    c0,
    0 ldr r1,
    = HW_C1_ICACHE_ENABLE | HW_C1_DCACHE_ENABLE | HW_C1_CACHE_ROUND_ROBIN |
      HW_C1_ITCM_ENABLE | HW_C1_DTCM_ENABLE | HW_C1_SB1_BITSET |
      HW_C1_EXCEPT_VEC_UPPER | HW_C1_PROTECT_UNIT_ENABLE orr r0,
    r0,
    r1 mcr p15,
    0,
    r0,
