...
u8 CTRDGi_ReadSrFunc(
    u8 *adr); // The polling function is referenced separately from 512K and 1M
...
void (*CTRDGi_WriteAgbFlashSectorAsync)(u16 secNo, u8 *src,
...
vu16 ctrdgi_timeoutFlag; // The polling function is referenced separately from
                         // 512K and 1M
...
u8 CTRDGi_ReadSrFunc(u8 *adr) { return *adr; }
  ...
  while (1) {
    ...
    if (ctrdgi_timeoutFlag) {
      if (CTRDGi_ReadSrFunc(adr) == lastData) {
        ...
        result = (u16)(CTRDG_BACKUP_RESULT_ERROR | CTRDG_BACKUP_RESULT_TIMEOUT |
                       phase);
        break;
      }
    }
    ...
    if (poll == lastData) {
      ...
      if ((poll & 0x20) != 0) { // Q5(TimeOutFlag)
        if (CTRDGi_ReadSrFunc(adr) == lastData) {
          ...
          *(vu8 *)CTRDG_BACKUP_COM_ADR1 = 0xf0; // Command reset
          result = (u16)(CTRDG_BACKUP_RESULT_ERROR |
                         CTRDG_BACKUP_RESULT_Q5TIMEOUT | phase);
          break;
        }
      }
    }
...
u32 CTRDGi_ReadAgbFlashCore(CTRDGTaskInfo *arg) {
  ...
  src = (u8 *)(CTRDG_AGB_FLASH_ADR + (sec_num << AgbFlash->sector.shift) +
               offset);
...
void CTRDG_WriteAgbFlashSectorAsync(u16 secNo, u8 *src,
                                    CTRDG_TASK_FUNC callback) {
