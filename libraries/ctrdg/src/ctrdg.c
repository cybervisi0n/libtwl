...
void CTRDGi_InitCommon(void) {
  SVC_CpuClear(0, &CTRDGi_Work, sizeof(CTRDGi_Work),
               32); // use SVC_* intentionally.
...
BOOL CTRDGi_IsNumberIDAtInit(u8 numberID) {
  ...
  if (cip->moduleID.raw != 0xffff && cip->moduleID.raw != 0x0000) {
    if (cip->moduleID.numberID == numberID) {
      ...
      if (cip->exLsiID[0] == numberID || cip->exLsiID[1] == numberID ||
          cip->exLsiID[2] == numberID) {
        retval = TRUE;
      }
    }
  }
...
BOOL CTRDG_IsExisting(void) {
...

  {
    ...
    if ((isRomCode == CTRDG_IS_ROM_CODE &&
         cip->moduleID.raw != chp->moduleID) // memory loaded
        || (isRomCode != CTRDG_IS_ROM_CODE &&
            cip->moduleID.raw !=
                *CTRDGi_GetModuleIDImageAddr()) // memory not loaded
        || ((cip->gameCode != chp->gameCode) &&
            cip->isAgbCartridge)) // AGB cartridge comparison
    {
...
void CTRDGi_SendtoPxi(u32 data) {
  while (PXI_SendWordByFifo(PXI_FIFO_TAG_CTRDG, data, FALSE) !=
         PXI_FIFO_SUCCESS) {
    SVC_WaitByLoop(1);
  }
}
...
BOOL CTRDG_CpuCopy8(const void *src, void *dest, u32 size) {
  ...
  if (HW_CTRDG_ROM <= (u32)dest && (u32)dest < HW_CTRDG_RAM_END) {
    return CTRDGi_CopyCommon(0 /*dummy */, (const void *)dest, (void *)src,
                             size, CTRDGi_FORWARD_CPU8);
  } else {
    return CTRDGi_CopyCommon(0 /*dummy */, src, dest, size,
                             CTRDGi_FORWARD_CPU8);
  }
}
...
BOOL CTRDG_Read8(const u8 *address, u8 *rdata) {
  ...
  SDK_ASSERT(CTRDGi_IsInitialized());
#endif
  return CTRDGi_AccessCommon((void *)address, 0 /*dummy */, rdata,
                             CTRDGi_ACCESS_READ8);
}
...
BOOL CTRDG_Write16(u16 *address, u16 data) {
  ...
  SDK_ASSERT(CTRDGi_IsInitialized());
#endif
  return CTRDGi_AccessCommon(address, data, 0 /*dummy */,
                             CTRDGi_ACCESS_WRITE16);
}
...
void CTRDG_CheckEnabled(void) {
  ...
  if (!CTRDG_IsOptionCartridge() && !CTRDG_IsEnabled()) {
    OS_TPanic("cartridge permission denied. (you must call CTRDG_Enable() "
              "under the guideline.)");
  }
#endif
}
