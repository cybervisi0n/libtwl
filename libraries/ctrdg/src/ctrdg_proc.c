...
#ifndef SDK_TWLLTD
BOOL CTRDGi_IsInitialized(void) { return isInitialized; }
#endif
...
void CTRDGi_InitModuleInfo(void) {
...

  {
    ...
    MI_DmaCopy16(1, (void *)(HW_CTRDG_ROM + 0x80), &((u8 *)&headerBuf)[0x80],
                 sizeof(headerBuf) - 0x80);
  ...
  CTRDGi_SendtoPxi(CTRDG_PXI_COMMAND_INIT_MODULE_INFO |
                   (((u32)&headerBuf - HW_MAIN_MEM) >> 5)
                       << CTRDG_PXI_COMMAND_PARAM_SHIFT);
...
void CTRDG_CheckPulledOut(void) {
  ...
  if (isCartridgePullOut) {
    ...
    CTRDGi_PulledOutCallback(PXI_FIFO_TAG_CTRDG, CTRDG_PXI_COMMAND_PULLED_OUT,
                             FALSE);
  }
}
...
void CTRDG_SetPhiClock(CTRDGPhiClock clock) {
  u32 data =
      ((u32)clock << CTRDG_PXI_COMMAND_PARAM_SHIFT) | CTRDG_PXI_COMMAND_SET_PHI;
  ...
  CTRDGi_Lock = TRUE;
  while (PXI_SendWordByFifo(PXI_FIFO_TAG_CTRDG_PHI, data, FALSE) !=
         PXI_FIFO_SUCCESS) {
    SVC_WaitByLoop(1);
  }
