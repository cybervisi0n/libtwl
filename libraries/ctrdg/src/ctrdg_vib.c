...
typedef struct {
  ...
  u32 repeat_num;    /* Number of times to repeat pulse set. When 0, repeats
                        endlessly. */
  u32 current_count; /* Indicates the number of times that the pulse set was
                        repeated. */
  VIBCartridgePulloutCallback
      cartridge_pullout_callback; /* Cartridge removal callback */
} VIBiPulseInfo;
...
void VIB_StartPulse(const VIBPulseState *state) {
  ...
  pulse_vib.rest_tick =
      VIBi_PulseTimeToTicks(state->rest_time) - VIBi_INTR_DELAY_TICK;
...

  {
    ...
    for (i = 0; i < VIB_PULSE_NUM_MAX; i++) {
      pulse_vib.on_tick[i] =
          VIBi_PulseTimeToTicks(state->on_time[i]) - VIBi_INTR_DELAY_TICK;
...
BOOL VIB_IsExecuting(void) { return pulse_vib.is_enable; }
...
BOOL VIB_IsCartridgeEnabled(void) { return CTRDG_IsBitID(VIBi_BITID); }
...
static void VIBi_SleepCallback(void *) { VIB_StopPulse(); }
