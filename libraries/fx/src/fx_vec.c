...
fx32 VEC_DotProduct(const VecFx32 *a, const VecFx32 *b) {
  SDK_NULL_ASSERT(a);
  SDK_NULL_ASSERT(b);
  return (fx32)(((fx64)a->x * b->x + (fx64)a->y * b->y + (fx64)a->z * b->z +
                 (1 << (FX64_SHIFT - 1))) >>
                FX64_SHIFT);
}
...
fx32 VEC_Fx16DotProduct(const VecFx16 *a, const VecFx16 *b) {
  ...
  tmp1 = (fx64)(a->x * b->x) + (a->y * b->y);
  tmp2 = (fx64)(a->z * b->z) + (1 << (FX64_SHIFT - 1));
  return (fx32)((tmp1 + tmp2) >> FX64_SHIFT);
}
...
void VEC_CrossProduct(const VecFx32 *a, const VecFx32 *b, VecFx32 *axb) {
  ...
  x = ((fx64)a->y * b->z - (fx64)a->z * b->y + (1 << (FX64_SHIFT - 1))) >>
      FX64_SHIFT;
  y = ((fx64)a->z * b->x - (fx64)a->x * b->z + (1 << (FX64_SHIFT - 1))) >>
      FX64_SHIFT;
  z = ((fx64)a->x * b->y - (fx64)a->y * b->x + (1 << (FX64_SHIFT - 1))) >>
      FX64_SHIFT;

  SDK_WARNING(
      x <= FX32_MAX && x >= FX32_MIN && y <= FX32_MAX && y >= FX32_MIN &&
          z <= FX32_MAX && z >= FX32_MIN,
      "VEC_CrossProduct: Overflow (0x%x, 0x%x, 0x%x) x (0x%x, 0x%x, 0x%x).",
...
void VEC_Fx16CrossProduct(const VecFx16 *a, const VecFx16 *b, VecFx16 *axb) {
  ...
  SDK_WARNING(
      x <= FX16_MAX && x >= FX16_MIN && y <= FX16_MAX && y >= FX16_MIN &&
...
fx32 VEC_Mag(const VecFx32 *pSrc) {
  u64 t; // always positive number
  fx32 rval;

  SDK_NULL_ASSERT(pSrc);
  SDK_ASSERT(!CP_IsSqrtBusy());

  t = (u64)((fx64)pSrc->x * pSrc->x);
  t += (u64)((fx64)pSrc->y * pSrc->y);
  t += (u64)((fx64)pSrc->z * pSrc->z);

  SDK_WARNING(t <= FX64_MAX >> 1, "VEC_Mag: Overflow (0x%x, 0x%x, 0x%x).",
              pSrc->x, pSrc->y, pSrc->z);
...
void VEC_Normalize(const VecFx32 *pSrc, VecFx32 *pDst) {
  ...
  SDK_WARNING(1LL << (FX32_SHIFT + FX32_SHIFT + FX64C_SHIFT) >= (u64)t,
              "*pSrc is too large!");
  ...
  pDst->x = (fx32)((t * pSrc->x + (1LL << (32 + FX32_SHIFT))) >>
                   (32 + FX32_SHIFT + 1));
  pDst->y = (fx32)((t * pSrc->y + (1LL << (32 + FX32_SHIFT))) >>
                   (32 + FX32_SHIFT + 1));
  pDst->z = (fx32)((t * pSrc->z + (1LL << (32 + FX32_SHIFT))) >>
                   (32 + FX32_SHIFT + 1));
}
...
void VEC_Fx16Normalize(const VecFx16 *pSrc, VecFx16 *pDst) {
  ...
  t = t * sqrt; // in s18.45 format
  pDst->x = (fx16)((t * pSrc->x + (1LL << (32 + FX32_SHIFT))) >>
                   (32 + FX32_SHIFT + 1));
  pDst->y = (fx16)((t * pSrc->y + (1LL << (32 + FX32_SHIFT))) >>
                   (32 + FX32_SHIFT + 1));
  pDst->z = (fx16)((t * pSrc->z + (1LL << (32 + FX32_SHIFT))) >>
                   (32 + FX32_SHIFT + 1));
}
...
void VEC_MultSubtract(fx32 a, const VecFx32 *v1, const VecFx32 *v2,
                      VecFx32 *pDest) {
  SDK_NULL_ASSERT(v1);
  SDK_NULL_ASSERT(v2);
  SDK_NULL_ASSERT(pDest);
...
fx32 VEC_Distance(const VecFx32 *v1, const VecFx32 *v2) {
  u64 tmp; // always positive number
  fx32 diff;
  ...
  SDK_WARNING(tmp <= FX64_MAX >> 1,
              "VEC_Distance: Overflow (0x%x, 0x%x, 0x%x), (0x%x, 0x%x, 0x%x).",
              v1->x, v1->y, v1->z, v2->x, v2->y, v2->z);

  tmp <<= 2; // for better precision
  CP_SetSqrt64(tmp);
