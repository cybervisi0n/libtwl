...
void FX_Init() {
  ...
  for (idx = 0, i = 0; i <= 4096 / 4;
       idx += (FX64C_TWOPI_65536 * (65536 / 4096)), ++i) {
  ...
  for (i = 1; i <= SDK_FX_INIT_PI_2; ++i) {
    FX_SinCosTable_[i * 2 + 1] =
        FX_SinCosTable_[SDK_FX_INIT_PI_2 +
                        i * 2]; // cos: cos(x) = sin(pi/2 + x)
    FX_SinCosTable_[SDK_FX_INIT_2PI - i * 2 + 1] =
        FX_SinCosTable_[i * 2 + 1]; // cos: cos(x) = cos(-x)
  }
#else
