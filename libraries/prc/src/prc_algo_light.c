...
u32 PRC_GetRecognitionBufferSizeEx_Light(int maxPointCount, int maxStrokeCount,
...
int PRC_GetRecognizedEntriesEx_Light(PRCPrototypeEntry **resultEntries,
...

  {
    ...
    border =
        FX32_ONE; // Cutoff level: If this value is set to FX32_ONE/2, then
                  // those with a score below 0.5 will not be evaluated at all.
    ...
    for (iPattern = 0; iPattern < protoDB->patternCount; iPattern++, proto++) {
      ...
      if (proto->entry->correction != 0) {
        ...
        if (proto->entry->correction < -FX32_ONE) {
          ...
          patternBorder =
              FX_Mul(patternBorder, FX32_ONE + proto->entry->correction / 2);
      ...
      for (iStroke = 0; iStroke < inputData->strokeCount; iStroke++) {
        ...
        strokeRatio =
            protoData->strokeRatios[iStroke]; // If here do inputData->, then
                                              // the relative weight of the
                                              // strokes of the input image
