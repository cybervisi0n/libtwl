...
static void
PRCi_CalcStrokeDistance_Fine(fx32 *score, fx32 *weight,
                             const PRCiPatternData_Common *inputData,
                             const PRCiPatternData_Common *protoData,
                             int normalizeSize, int lengthFilterThreshold,
                             int lengthFilterRatio, int iStroke, void *buffer);

u32 PRC_GetRecognitionBufferSizeEx_Fine(int maxPointCount, int maxStrokeCount,
...

  {
    ...
    for (iPattern = 0; iPattern < protoDB->patternCount; iPattern++, proto++) {
      ...
      for (iStroke = 0; iStroke < inputData->strokeCount; iStroke++) {
        ...
        PRCi_CalcStrokeDistance_Fine(&score, &weight, inputData, protoData,
      ...
      wholeScore =
          FX_Div(wholeScore * 4,
                 wholeWeight * normalizeSize); // 4 == (1<<FX32_SHIFT)/(512*2) ,
                                               // about 512 and 2, see below.
...
#define PRCi_ANGLE_SCORE(input, proto)                                         \
  ((32768 - PRCi_ABS((s16)(protoAngles[(proto)] - inputAngles[(input)]))) / 128)
...
#ifdef SDK_DEBUG
  {
    ...
    if (width > normalizeSize) {
      OS_TWarning("too small normalizeSize. PRCPrototypeList.normalizeSize "
                  "seems smaller than actual data.");
    }
  }
#endif
...
#elif // beam search version. // Commenting out because doesn't speed up
  {
    ...
    for (iProto = 1; iProto < protoSize; iProto++) {
      ...
      for (iInput = beamLow; iInput < beamHigh; iInput++) {
        ...
        localScore = (doubleWidth - CityBlockDistance(&inputPoints[iInput],
                                                      &protoPoints[iProto])) *
                     (angleScores_(iInput, iProto) +
                      angleScores_(iInput + 1, iProto + 1));
    ...
    if (beamHigh < inputSize) {
      for (iInput = beamHigh; iInput < inputSize; iInput++) {
        sumScore_(iInput, protoSize - 1) =
            sumScore_(iInput - 1, protoSize - 1) + 1;
        nMatches_(iInput, protoSize - 1) =
            nMatches_(iInput - 1, protoSize - 1) + 1;
