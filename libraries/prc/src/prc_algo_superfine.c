...
static void PRCi_CalcStrokeDistance_Superfine(
    fx32 *score, fx32 *weight, const PRCiPatternData_Common *inputData,
    const PRCiPatternData_Common *protoData, int normalizeSize,
    int lengthFilterThreshold, int lengthFilterRatio, int iStroke,
    void *buffer);

u32 PRC_GetRecognitionBufferSizeEx_Superfine(
...
int PRC_GetRecognizedEntriesEx_Superfine(
...

  {
    ...
    for (iPattern = 0; iPattern < protoDB->patternCount; iPattern++, proto++) {
      ...
      wholeScore = FX_Div(wholeScore, wholeWeight * normalizeSize *
                                          2); // normalizeSize*2 == doubleWidth
...
static inline void GetMixedPoint(PRCPoint *p, const PRCPoint *p1, int w1,
                                 const PRCPoint *p2, int w2) {
  int a, w;
  a = (p1->x * w1 + p2->x * w2);
  SDK_TASSERTMSG(a >= 0, "a < 0: (%d, %d)*%d-(%d, %d)*%d", p1->x, p1->y, w1,
                 p2->x, p2->y, w2);
  w = w1 + w2;
  SDK_TASSERTMSG(w > 0, "w <= 0: (%d, %d)*%d-(%d, %d)*%d", p1->x, p1->y, w1,
                 p2->x, p2->y, w2);
  {
    OSIntrMode enabled;
...
#define PRCi_ANGLE_SCORE(input, proto)                                         \
  ((FX_CosIdx((u16)(protoAngles[(proto)] - inputAngles[(input)])) +            \
    FX32_ONE) /                                                                \
   (FX32_ONE * 2 / 256))
...
#define angleScores_(x, y) (*(angleScores + (x) * (maxPointCount + 1) + (y)))
...
#ifdef SDK_DEBUG
  {
    ...
    if (width > normalizeSize) {
      OS_TWarning("too small normalizeSize. PRCPrototypeList.normalizeSize "
                  "seems too smaller than actual data.");
    }
  }
#endif
...

  {
    ...
    PRCi_ALLOC_ARRAY(direction, int, maxPointCount *maxPointCount, buffer,
                     addr);
    PRCi_ALLOC_ARRAY(angleScores, int,
                     ((maxPointCount + 1) * (maxPointCount + 1)), buffer, addr);
  ...
  *score = 0;
  ...
  if (inputStrokeLength > lengthFilterThreshold ||
      protoStrokeLength > lengthFilterThreshold) {
    if (inputStrokeLength * lengthFilterRatio < protoStrokeLength ||
      ...
      OS_TPrintf("Skipped because of length filter %d <=> %d\n",
                 FX_Whole(inputStrokeLength), FX_Whole(protoStrokeLength));
#endif
      return;
    }
  }
  ...
  if (protoSize == 1 || inputSize == 1) {
    ...
    angleScores_(inputSize, protoSize) =
        PRCi_ANGLE_SCORE(inputSize - 1, protoSize - 1);
  ...
  sumScore_(0, 0) =
      (doubleWidth - CityBlockDistance(&inputPoints[0], &protoPoints[0])) *
      angleScores_(0, 0) * 2;
  nMatches_(0, 0) = 1;
  for (iInput = 1; iInput < inputSize; iInput++) {
    sumScore_(iInput, 0) =
        (doubleWidth -
         CityBlockDistance(&inputPoints[iInput], &protoPoints[0])) *
            (angleScores_(iInput, 0) + angleScores_(iInput + 1, 1)) +
  ...
  for (iProto = 1; iProto < protoSize; iProto++) {
    sumScore_(0, iProto) =
        (doubleWidth -
         CityBlockDistance(&inputPoints[0], &protoPoints[iProto])) *
            (angleScores_(0, iProto) + angleScores_(1, iProto + 1)) +
...
#ifdef PRC_DEBUG_RECOGNIZE
  {
    ...
    while (!(iInput == 0 && iProto == 0)) {
      ...
      localScore =
          sumScore_(iInput, iProto) - sumScore_(iInput + dx, iProto + dy);
      angleScore =
          angleScores_(iInput, iProto) + angleScores_(iInput + 1, iProto + 1);

      OS_TPrintf(" %2d <-> %2d : 0.%03d = 0.%03d * 0.%03d, average from begin: "
                 "0.%03d\n",
                 iInput, iProto, localScore / normalizeSize,
                 localScore * 512 / angleScore / normalizeSize, angleScore * 2,
                 sumScore_(iInput, iProto) / nMatches_(iInput, iProto) /
                     normalizeSize);
...

  {
    ...
    for (iMatch = 0; iMatch < nMatches - 1; iMatch++) {
...
#ifdef PRC_DEBUG_RECOGNIZE_DEEPLY
      OS_TPrintf(" [%d, %d] - [%d, %d]\n", inputMatch[iMatch],
                 protoMatch[iMatch], inputMatch[iMatch + 1],
                 protoMatch[iMatch + 1]);
#endif
      inputLocalLength = 0;
      protoLocalLength = 0;
      for (iInput = inputMatch[iMatch] + 1; iInput <= inputMatch[iMatch + 1];
           iInput++) {
        inputLocalLength += inputLengths[iInput];
      }
      for (iProto = protoMatch[iMatch] + 1; iProto <= protoMatch[iMatch + 1];
           iProto++) {
      ...
      localScore = 0;
      loopEnd = (inputMatch[iMatch + 1] - iInput) +
                (protoMatch[iMatch + 1] - iProto) + 1;
#ifdef PRC_DEBUG_RECOGNIZE_DEEPLY
        ...
        if (inputNextRatio <= protoNextRatio) {
          ...
          if (iProto > 0) {
            GetMixedPoint(&protoPoint, &protoPoints[iProto - 1], protoNextRatio,
                          &protoPoints[iProto],
                          protoOrigNextRatio - protoNextRatio);
          } else {
...
#ifdef PRC_DEBUG_CHECK_OVERFLOW
          {
            ...
            localScore += inputNextRatio * (angleScores_(iInput, iProto)) *
                          (doubleWidth - CityBlockDistance(&inputPoints[iInput],
                                                           &protoPoint));
#ifdef PRC_DEBUG_CHECK_OVERFLOW
          ...
          if (iInput > 0) {
            GetMixedPoint(&inputPoint, &inputPoints[iInput - 1], inputNextRatio,
                          &inputPoints[iInput],
                          inputOrigNextRatio - inputNextRatio);
          } else {
...
#ifdef PRC_DEBUG_CHECK_OVERFLOW
          {
            ...
            localScore += protoNextRatio * (angleScores_(iInput, iProto)) *
                          (doubleWidth - CityBlockDistance(&protoPoints[iProto],
                                                           &inputPoint));
#ifdef PRC_DEBUG_CHECK_OVERFLOW
            if (prevScore > localScore)
              OS_TWarning("Internal Error: score overflow\n");
      ...
      SDK_TASSERTMSG(weight != 0,
                     "inputStrokeLength: %d, inputLocalLength: %d, "
                     "protoStrokeLength: %d, protoLocalLength: %d\n",
                     inputStrokeLength, inputLocalLength, protoStrokeLength,
                     protoLocalLength);
      totalWeight += weight;
    }
    ...
    {
      fx32 ratio;
      ratio = (inputStrokeRatio >= protoStrokeRatio) ? inputStrokeRatio
                                                     : protoStrokeRatio;
