...
static BOOL PRCi_TerminateStrokes(u16 *selectedPoints, int *pSelectedPointNum,
                                  int maxPointCount,
  ...
  if (!PRC_IsPenUpMarker(&inputPoints[selectedPoints[selectedPointNum - 1]])) {
    ...
    if (!PRC_IsPenUpMarker(
            &inputPoints[selectedPoints[selectedPointNum - 2]])) {
      ...
      if (selectedPointNum < maxPointCount) {
        ...
        if (selectedPointNum >= 3 &&
            !PRC_IsPenUpMarker(
                &inputPoints[selectedPoints[selectedPointNum - 3]])) {
...
BOOL PRC_ResampleStrokes_None(u16 *selectedPoints, int *pSelectedPointNum,
                              int maxPointCount, int maxStrokeCount,
                              const PRCStrokes *strokes, int threshold,
                              void *buffer) {
  ...
  if (size < 2) {
    ...
    *pSelectedPointNum = iPoint;
    ...
    if (!PRC_IsPenUpMarker(&strokes->points[size - 1])) {
      ...
      (void)PRCi_TerminateStrokes(selectedPoints, pSelectedPointNum,
                                  maxPointCount, strokes);
    }
  }
...
BOOL PRC_ResampleStrokes_Distance(u16 *selectedPoints, int *pSelectedPointNum,
                                  int maxPointCount, int maxStrokeCount,
                                  const PRCStrokes *strokes, int threshold,
                                  void *buffer) {
  ...
  newFlag = TRUE;
  for (iPoint = 0; iPoint < size && selectedPointNum < maxPointCount;
       iPoint++, point++) {
    if (!PRC_IsPenUpMarker(point)) {
      if (newFlag) {
        ...
        length +=
            PRCi_ABS(point->x - prevPoint.x) + PRCi_ABS(point->y - prevPoint.y);
        if (length >= threshold) {
          selectedPoints[selectedPointNum] = (u16)iPoint;
          selectedPointNum++;
          length = 0;
      ...
      if (newFlag) {
        ...
        if (selectedPoints[selectedPointNum - 1] !=
            iPoint - 1) // When execution comes here, [selectedPointNum>0] is
                        // always true
        {
        ...
        if (strokeCount >= maxStrokeCount) {
          ...
          iPoint++; // For the processing after the loop. // At the present
                    // time, has no meaning because newFlag == TRUE
          break;
        }
      }
    }
  ...
  *pSelectedPointNum = selectedPointNum;
  ...
  if (!newFlag) {
    ...
    if (selectedPointNum > 0 &&
        selectedPoints[selectedPointNum - 1] != iPoint - 1 &&
...
BOOL PRC_ResampleStrokes_Angle(u16 *selectedPoints, int *pSelectedPointNum,
                               int maxPointCount, int maxStrokeCount,
                               const PRCStrokes *strokes, int threshold,
                               void *buffer) {
#define PRC_RESAMPLE_ANGLE_LENGTH_THRESHOLD                                    \
  6 // A valid angle cannot be obtained unless one moves about 6 city block
    // lengths away
  int selectedPointNum;
  int strokeCount;
  int iPoint;
  ...
  for (iPoint = 0; iPoint < size && selectedPointNum < maxPointCount;
    ...
    if (!PRC_IsPenUpMarker(point)) {
      if (newFlag) {
        ...
        prevPoint = *point;
        if (iPoint + 1 < size) {
          prevAngle = FX_Atan2Idx(((point + 1)->y - point->y) << FX32_SHIFT,
          ...
          if (firstFlag) {
...

            {
              ...
              if (PRCi_ABS((s16)(prevAngle - nextAngle)) >= threshold) {
                currAngle = FX_Atan2Idx((point->y - prevPoint.y) << FX32_SHIFT,
      ...
      if (newFlag) {
        ...
        if (selectedPoints[selectedPointNum - 1] !=
            iPoint - 1) // Always selectedPointNum>0 when comes to here
        {
...
BOOL PRC_ResampleStrokes_Recursive(u16 *selectedPoints, int *pSelectedPointNum,
                                   int maxPointCount, int maxStrokeCount,
                                   const PRCStrokes *strokes, int threshold,
                                   void *buffer) {
  ...
  while (1) {
    ...
    if (endIndex < size) {
      ...
      selectedPoints[selectedPointNum] = (u16)-1;
      selectedPointNum++; // -1 is specially reserved to indicate the final
                          // PenUpMarker
    }
    ...
    strokeCount++; // strokeCount is only counted for the sake of limiting it
                   // with maxStrokeCount
    ...
    while (stackSize > 0) {
      ...
      yDir = y2 - y1;
      offs = -(x1 * y2 - x2 * y1); // Items bundled together in order to reduce
