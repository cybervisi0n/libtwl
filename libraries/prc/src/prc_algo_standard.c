...
static inline void GetMiddlePoint(PRCPoint *p, const PRCPoint *p1,
                                  const PRCPoint *p2) {
  p->x = (s16)((p1->x + p2->x) / 2);
  p->y = (s16)((p1->y + p2->y) / 2);
}

u32 PRC_GetRecognitionBufferSizeEx_Standard(
...

  {
    ...
    for (iPattern = 0; iPattern < protoDB->patternCount; iPattern++, proto++) {
      ...
      for (iStroke = 0; iStroke < inputData->strokeCount; iStroke++) {
        ...
        protoRatio =
            &protoData->lineSegmentRatioToStrokeArray[protoStrokeIndex];
        inputRatio =
            &inputData->lineSegmentRatioToStrokeArray[inputStrokeIndex];
        ...
        for (i = 0; i < protoSize + inputSize - 3; i++) {
          ...
          if (protoNextRatio <= inputNextRatio) {
            ...
            score *=
                (inputNextRatio <
                 inputRatio[iInput] /
                     2) // Remainder of less than half = Next point is closer
                    ? (doubleWidth - CityBlockDistance(&inputPoints[iInput],
                                                       &protoPoints[iProto]))
                    : (doubleWidth - CityBlockDistance(&inputPoints[iInput - 1],
                                                       &protoPoints[iProto]));
