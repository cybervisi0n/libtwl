...
asm void OS_SetExceptionVectorUpper(void) {
  mrc p15, 0, r0, c1, c0, 0 orr r0, r0, #HW_C1_EXCEPT_VEC_UPPER mcr p15, 0, r0,
      c1, c0, 0 bx lr
}

asm void OS_SetExceptionVectorLower(void) {
  mrc p15, 0, r0, c1, c0, 0 bic r0, r0, #HW_C1_EXCEPT_VEC_UPPER mcr p15, 0, r0,
      c1, c0, 0 bx lr
}
...
#ifdef SDK_BUILD_ARM
  OSi_DebuggerHandler = (void *)((0x2600000 <= OSi_OriginalHandler &&
                                  OSi_OriginalHandler < 0x2800000)
                                     ? OSi_OriginalHandler
                                     : NULL);
#endif
...
asm static void OSi_DebuggerExceptionHook(void) {
  ...
  ldr r12, = OSi_ExceptionHookStack stmia r12, {r0 - r4, sp, lr} mrs r4,
      CPSR

          mrs r0,
      CPSR and r0, r0, #0x1f teq r0, #0x17 beq user_exception teq r0,
      #0x1b bne user_exception_end is_und :
      ...
      [ r0, #4 ] ldr r0,
      [ r0, #12 ] tst r1,
      #0x20 beq is_und_arm is_und_thumb : bic r0,
      r0,
      #1 ldrh r0,
      [ r0, # - 2 ] ldr r1,
      = 0x0000EFFF cmp r0,
      r1 beq user_exception_end ldr r1,
      = 0x0000DEFE cmp r0,
      r1 beq user_exception_end ldr r1,
      = 0x0000BE00 cmp r0,
      r1 beq user_exception_end b user_exception is_und_arm : bic r0,
      r0,
      #3 ldr r0,
      [ r0, # - 4 ] ldr r1,
      = 0xE7FFFFFF cmp r0,
      ...
      r1 beq user_exception_end ldr r1,
      = 0xE6000010 cmp r0,
      r1 beq user_exception_end b user_exception user_exception :

      ldmia r12,
      {r0 - r1} ldr r12,
      = HW_ITCM_END stmfd r12 !,
      {r0 - r3, sp, lr} and r0,
      sp,
      #1 mov sp,
      r12 bl OSi_GetAndDisplayContext ldmfd sp !,
      {r0 - r3, r12, lr} mov sp,
      r12 user_exception_end :

      msr CPSR_cxsf,
      r4 ldr r12,
      = OSi_ExceptionHookStack ldmia r12,
      {r0 - r4, sp, lr} ldr r12,
      = OSi_DebuggerHandler ldr r12,
      [r12] cmp r12,
      #0 bxne r12 bx lr
}
#include <nitro/codereset.h>
...
void OSi_GetAndDisplayContext(void) {}
void OSi_SetExContext(void) {}
#else
#include <nitro/code32.h>
asm void OSi_ExceptionHandler(void) {
  ...
  and r0,
      ...
      teq r1,
      #0x17 bne @10 bl OSi_GetAndDisplayContext b usr_return

      @10 :

      teq r1,
      #0x1b bne usr_return bl OSi_GetAndDisplayContext
...
static asm void OSi_GetAndDisplayContext(void) {
  stmfd sp !,
      {r0, lr} /* 8-byte alignment of the call stack */
...
static asm void OSi_SetExContext(void) {
  ...
  mrs r2, CPSR str r2,
      [ r1, #OSiExContext.debug[1] ]
#endif

      str r0,
      [ r1, #OSiExContext.exinfo ]

      ldr r0,
      [ r12, #0 ] str r0, [ r1, #OS_CONTEXT_R0 ] ldr r0, [ r12, #4 ] str r0,
      [ r1, #OS_CONTEXT_R1 ] ldr r0, [ r12, #8 ] str r0,
      [ r1, #OS_CONTEXT_R2 ] ldr r0, [ r12, #12 ] str r0,
      [ r1, #OS_CONTEXT_R3 ] ldr r2, [ r12, #16 ] bic r2, r2,
      #1

      add r0,
      r1, #OS_CONTEXT_R4 stmia r0,
      {r4 - r11}

#ifdef OSiDEBUG

  str r12,
      [ r1, #OSiExContext.debug[0] ]
#endif

#ifdef SDK_ARM9

      ldr r0,
      [ r2, #0 ] str r0, [ r1, #OSiExContext.cp15 ] ldr r3, [ r2, #4 ] str r3,
      [ r1, #OS_CONTEXT_CPSR ] ldr r0, [ r2, #8 ] str r0,
      [ r1, #OS_CONTEXT_R12 ] ldr r0, [ r2, #12 ] str r0,
      [ r1, #OS_CONTEXT_PC_PLUS4 ]
#else  // ifdef SDK_ARM9

  mov r0,
      #0 str r0, [ r1, #OSiExContext.cp15 ] ldr r3, [ r2, #0 ] str r3,
      [ r1, #OS_CONTEXT_CPSR ] ldr r0, [ r2, #4 ] str r0,
      [ r1, #OS_CONTEXT_R12 ] ldr r0, [ r2, #8 ] str r0,
      [ r1, #OS_CONTEXT_PC_PLUS4 ]
#endif // ifdef SDK_ARM9

      mrs r0,
      CPSR orr r3, r3, #0x80 bic r3, r3, #0x20 msr CPSR_cxsf,
      r3

          str sp,
      [ r1, #OS_CONTEXT_R13 ] str lr, [ r1, #OS_CONTEXT_R14 ] mrs r2,
      SPSR

#ifdef OSiDEBUG

          str r2,
      [ r1, #OSiExContext.debug[3] ]
#endif
...
static void OSi_DisplayExContext() {
  ...
  for (i = 0; i < 13; i++) {
    OSi_ExPrintf("R%02d=%08X  %c", i, OSi_ExContext.context.r[i],
                 ((i & 3) == 3) ? '\n' : ' ');
  }
  ...
  if (OSi_UserExceptionHandler) {
    ...
    bl OS_EnableProtectionUnit
        ...
        = OSi_ExContext
            ...
            ldr r1,
        = OSi_UserExceptionHandlerArg ldr r1,
        [r1]

        ldr r12,
        = OSi_UserExceptionHandler ldr r12, [r12] ldr lr, = @1 bx r12 @1 :
#ifdef SDK_ARM9
