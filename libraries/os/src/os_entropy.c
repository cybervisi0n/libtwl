...
void OS_GetLowEntropyData(u32 buffer[OS_LOW_ENTROPY_DATA_SIZE / sizeof(u32)]) {
  ...
  const u8 *macAddress = (u8 *)((u32)(work->nvramUserInfo) +
  ...
  buffer[6] = (u32)((*(u16 *)(&work->touch_panel[0]) << 16) |
                    *(u16 *)(&work->touch_panel[2]));
  buffer[7] = (u32)((work->wm_rssi_pool << 16) |
                    (reg_PAD_KEYINPUT | *(vu16 *)HW_BUTTON_XY_BUF));
}
