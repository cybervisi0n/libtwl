...
static u8
    only_forChina_charData[OSi_ONLY_FORCHINA_CHAR_SIZE] ATTRIBUTE_ALIGN(4) = {
        };
...
static u8 *LoadImage(ImageIndex index, u32 *p_size) {
  ...
  static const char *(path_table[IMAGE_MAX]) = {
      "notes_forChina_chrData.bin", "notes_forChina_scrData.bin",
      "digit_chrData.bin",          "logo_forChina_chrData.bin",
      "logo_forChina_scrData.bin",  "logo_forChina_palData.bin",
  };
  ...
  static u8 *(ptr_table[]) = {
      _binary_notes_forChina_chrData_bin, _binary_notes_forChina_scrData_bin,
      _binary_digit_chrData_bin,          _binary_logo_forChina_chrData_bin,
      _binary_logo_forChina_scrData_bin,  _binary_logo_forChina_palData_bin,
  };
...
static void VBlankIntr(void) { OS_SetIrqCheckFlag(OS_IE_V_BLANK); }
...
static void SetISBNString(const char **isbn) {
  ...
  const struct {
    ...
    u8 length; /* Display character count */
      ...
      {80, 136, 17},  /* ISBN */
      {125, 151, 12}, /* Joint registration number */
      {106, 167, 4},  /* (China-specific information displayed on left side) */
      {139, 167, 3},  /* (China-specific information displayed on right side) */
  };
  const int count = sizeof(pos) / sizeof(*pos); /* Total number of lines */
  const int digit_num =
      12; /* Number of characters for the digit display ("0123456789- ") */
  u16 pos_x;
  u16 pos_y;
  u16 index;
  u16 width;
...
static void DispExclusiveMessage(void) {
  ...
  {
    reg_GX_VRAMCNT_A = (u8)((1 << REG_GX_VRAMCNT_A_MST_SHIFT) |
    ...
    reg_G2_BG0CNT =
        (u16)((GX_BG_SCRSIZE_TEXT_256x256 << REG_G2_BG0CNT_SCREENSIZE_SHIFT) |
              ...
              (GX_BG_EXTPLTT_01 << REG_G2_BG0CNT_BGPLTTSLOT_SHIFT) |
...

    {
      ...
      SVC_UnpackBits(only_forChina_charData, (u32 *)(HW_BG_VRAM + 0x20),
                     &param);
    }
  ...
  {
    reg_GX_VRAMCNT_C = (u8)((4 << REG_GX_VRAMCNT_C_MST_SHIFT) |
                            (1 << REG_GX_VRAMCNT_C_E_SHIFT));
    reg_G2S_DB_BG0CNT =
        (u16)((GX_BG_SCRSIZE_TEXT_256x256
              ...
              (GX_BG_EXTPLTT_01 << REG_G2S_DB_BG0CNT_BGPLTTSLOT_SHIFT) |
              (0 << REG_G2S_DB_BG0CNT_PRIORITY_SHIFT));
    reg_G2S_DB_BG0OFS = 0;
    reg_GXS_DB_DISPCNT |=
        ((GX_BGMODE_0 << REG_GXS_DB_DISPCNT_BGMODE_SHIFT) |
...
void OS_ShowAttentionChina(const char **isbn, OSChinaIsbn param) {
...

  {
...

    {
      ...
      reg_G2_BG0CNT =
          (u16)((GX_BG_SCRSIZE_TEXT_256x256 << REG_G2_BG0CNT_SCREENSIZE_SHIFT) |
                (GX_BG_COLORMODE_256 << REG_G2_BG0CNT_COLORMODE_SHIFT) |
                (GX_BG_SCRBASE_0xf000 << REG_G2_BG0CNT_SCREENBASE_SHIFT) |
                (GX_BG_CHARBASE_0x00000 << REG_G2_BG0CNT_CHARBASE_SHIFT) |
                (GX_BG_EXTPLTT_01 << REG_G2_BG0CNT_BGPLTTSLOT_SHIFT) |
                (0 << REG_G2_BG0CNT_PRIORITY_SHIFT));
...

    {
      reg_GX_VRAMCNT_C = (u8)((4 << REG_GX_VRAMCNT_C_MST_SHIFT) |
                              (1 << REG_GX_VRAMCNT_C_E_SHIFT));
      reg_GX_VRAMCNT_D = (u8)((4 << REG_GX_VRAMCNT_D_MST_SHIFT) |
...

  {
    ...
    for (i = 0; i < OSi_FORCHINA_FADE_SPAN; i++) {
      reg_GX_MASTER_BRIGHT =
          (u16)((1 << REG_GX_MASTER_BRIGHT_E_MOD_SHIFT) |
    ...
    for (i = 0; i < OSi_FORCHINA_DISP_SPAN; i++) {
      ...
      if (param == OS_CHINA_ISBN_CHECK_ROM)
        i = 0;
    }
