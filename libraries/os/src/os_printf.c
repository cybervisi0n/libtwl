...
SDK_WEAK_SYMBOL void OS_TVFPrintfEx(int console, const char *fmt,
                                    va_list vlist) {
...
#ifndef SDK_FINALROM
SDK_WEAK_SYMBOL void OSi_FWarning(int console, const char *file, int line,
                                  const char *fmt, ...) {
  va_list vlist;
...
SDK_WEAK_SYMBOL void OSi_TFWarning(int console, const char *file, int line,
                                   const char *fmt, ...) {
  va_list vlist;
...
#ifndef SDK_FINALROM
SDK_WEAK_SYMBOL void OSi_FPanic(int console, const char *file, int line,
                                const char *fmt, ...) {
  va_list vlist;
...
SDK_WEAK_SYMBOL void OSi_TFPanic(int console, const char *file, int line,
                                 const char *fmt, ...) {
  va_list vlist;
...
SDK_WEAK_SYMBOL int OS_VSNPrintf(char *dst, size_t len, const char *fmt,
                                 va_list vlist) {
  return STD_TVSNPrintf(dst, len, fmt, vlist);
}

SDK_WEAK_SYMBOL int OS_VSNPrintfEx(char *dst, size_t len, const char *fmt,
                                   va_list vlist) {
  char buf[24];
  int n_buf;
  char prefix[2];
    ...
    if ((unsigned int)(((unsigned char)*s ^ 0x20) - 0xA1) < 0x3C) {
      ...
      case 'b': /* binary */
      {
        ...
        while (1) {
          ...
          right_char =
              (char)((right < 10) ? (right + '0') : (right + hex_char));
...
void OS_SetPrintOutput(OSPrintOutputProc proc, int num) {
  SDK_ASSERT((0 <= num && num <= OS_PRINT_OUTPUT_WINDOW_MAX) ||
             num == OS_PRINT_OUTPUT_NONE);
...
OSFPutStringHookType OS_GetFPutStringHook(void) { return OSi_FPutStringHook; }
#endif
