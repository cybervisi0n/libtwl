...
static void OSi_SetStatistics(void *statBuf, OSFunctionCost *entry,
                              OSFunctionCost *exit);
static void OSi_CalcThreadInterval(register OSThread *saveThread,
                                   register OSThread *loadThread);
...
asm BOOL OS_EnableFunctionCost(void) {
  ...
  stmfd sp !, {lr} bl OSi_GetFunctionCostInfo ldmfd sp !, {lr} cmp r0,
      #0 bxeq lr

          mov r2,
      r0 ldrh r0, [ r2, #OSFunctionCostInfo.enable ] mov r1, #1 strh r1,
      [ r2, #OSFunctionCostInfo.enable ]
...
asm BOOL OS_DisableFunctionCost(void) {
  ...
  stmfd sp !, {lr} bl OSi_GetFunctionCostInfo ldmfd sp !, {lr} cmp r0,
      #0 bxeq lr
          ...
          mov r2,
      r0 ldrh r0, [ r2, #OSFunctionCostInfo.enable ] mov r1, #0 strh r1,
      [ r2, #OSFunctionCostInfo.enable ]
...
asm BOOL OS_RestoreFunctionCost(BOOL) {
  ...
  stmfd sp !, {r0, lr} bl OSi_GetFunctionCostInfo mov r1, r0 ldmfd sp !,
      {r0, lr} cmp r1,
      #0 bxeq lr

          ldrh r2,
      [ r1, #OSFunctionCostInfo.enable ] strh r0,
      [ r1, #OSFunctionCostInfo.enable ] mov r0,
      r2
...
void OS_InitFunctionCost(void *buf, u32 size) {
  ...
  SDK_TASSERTMSG(((u32)buf & 3) == 0,
                 "FunctionCost buffer must be aligned by 4");
  SDK_TASSERTMSG(
      (size >= OSi_COSTINFO_SIZEOF_HEADERPART + sizeof(OSFunctionCost)),
  ...
  SDK_TASSERTMSG(OS_IsThreadAvailable(),
                 "OS_InitFunctionCost: thread system not initialized.");
  ...
  *infoPtr = (OSFunctionCostInfo *)buf;
  ...
  (*infoPtr)->limit =
      (OSFunctionCost *)((u32)buf + OSi_COSTINFO_SIZEOF_HEADERPART +
                         OS_CalcFunctionCostLines(size) *
                             sizeof(OSFunctionCost));
...
int OS_CalcFunctionCostLines(u32 size) {
  int n =
      (int)((size - OSi_COSTINFO_SIZEOF_HEADERPART) / sizeof(OSFunctionCost));
  return (n < 0) ? 0 : n;
}
...
#ifdef SDK_NO_THREAD
    ldr r0, = OSi_DefaultFunctionCostInfo ldr r0, [r0, #0] bx lr
#else
    ldr r0, = OSi_ThreadInfo ldr r0,
    [r0, #OSThreadInfo.current] // r0 = currentThread. 0 if no currentThread
    cmp r0,
    #0 ldrne r0,
    [r0, #OSThread.profiler] // r0 = functionCostInfo
    bx lr
#endif
}
#include <nitro/codereset.h>
...
asm void __PROFILE_ENTRY(void) {
  ...
  stmfd sp !, {r0} ldr r0,
      = OSi_IsThreadInitialized ldr r0, [ r0, #0 ] cmp r0, #0 ldmfd sp !,
      {r0} bxeq lr
#endif
  ...
  mrs r4,
      cpsr orr r1, r4, #HW_PSR_IRQ_DISABLE msr cpsr_c,
      r1

          stmfd sp !,
      {r0} bl OSi_GetFunctionCostInfo mov r1, r0 ldmfd sp !, {r0} cmp r1,
      #0 beq _exit

          ldrh r2,
      [ r1, #OSFunctionCostInfo.enable ] cmp r2,
      #0 beq _exit

          ldr r2,
      [ r1, #OSFunctionCostInfo.current ] cmp r2,
      #0 beq _exit

          ldr r3,
      [ r1, #OSFunctionCostInfo.limit ] cmp r2,
      r3 bpl _exit

          str r0,
      [ r2, #OSFunctionCost.entry.name ]

      stmfd sp !,
      {r1 - r2} bl OSi_GetTick_noProfile ldmfd sp !, {r1 - r2} str r0,
      [ r2, #OSFunctionCost.entry.time ]

#ifdef OSi_FUNCTIONCOST_THREAD_INTERVAL

      mov r3,
      #0 str r3,
      [ r2, #OSFunctionCost.exit.interval ]
#endif

      add r2,
      r2, #OSi_SIZEOF_FUNCTIONCOST str r2,
      [ r1, #OSFunctionCostInfo.current ]
...
asm void __PROFILE_EXIT(void) {
  ...
  ldr r0,
      = OSi_IsThreadInitialized ldr r0, [ r0, #0 ] cmp r0,
      #0 beq _exit
#endif

          mrs r3,
      cpsr orr r1, r3, #HW_PSR_IRQ_DISABLE msr cpsr_c,
      r1

          bl OSi_GetFunctionCostInfo mov r1,
      r0 cmp r1,
      #0 beq _exit_ri
      ...
      #0 beq _exit_ri
          ...
          ldr r2,
      [ r1, #OSFunctionCostInfo.current ] // r2 = currentPtr of trace
      cmp r2,
      #0 beq _exit_ri
      ...
      r0 bpl _exit_ri
          ...
          ldr r0,
      = OSi_FUNCTIONCOST_EXIT_TAG str r0,
      [ r2, #OSFunctionCost.exit.tag ]

      stmfd sp !,
      {r1 - r3} bl OSi_GetTick_noProfile ldmfd sp !, {r1 - r3} str r0,
      [ r2, #OSFunctionCost.exit.time ]
...
void OS_InitStatistics(void *statBuf, u32 size) {
  ...
  infoPtr->size = ((size - OSi_STATISTICS_SIZEOF_HEADERPART) /
                   sizeof(OSFunctionCostStatistics)) *
                      sizeof(OSFunctionCostStatistics) +
                  OSi_STATISTICS_SIZEOF_HEADERPART;
  ...
  *(u32 *)((OSFunctionCostStatistics *)p - 1) =
      OSi_FUNCTIONCOST_CHECKNUM_BUFFER;
}
...
static void OSi_SetStatistics(void *statBuf, OSFunctionCost *entry,
  ...
  time =
      (time > entry->entry.interval) ? (u32)(time - entry->entry.interval) : 0;
#endif
...
#include <nitro/code32.h>
#pragma profile off
static asm void OSi_CalcThreadInterval(register OSThread *saveThread,
                                       register OSThread *loadThread) {
#pragma unused(saveThread, loadThread)
  ...
  cmp r0,
      #0 // r0 = saving thread
      beq _skip_saveThread ldr r2,
      [ r0, #OSThread.profiler ] // r2 = functionCostInfo
      cmp r2,
      #0 beq _skip_saveThread

          ldrh r3,
      [ r2, #OSFunctionCostInfo.enable ] // r3 = master enable
      cmp r3,
      #0 beq _skip_saveThread

          stmfd sp !,
      {r1 - r3} bl OSi_GetTick_noProfile // r0 = currentTime
          ldmfd sp !,
      {r1 - r3} str r0,
      [ r2, #OSFunctionCostInfo.breakTime ]

      _skip_saveThread :

      cmp r1,
      #0 // r1 = loadingThread
      beq _skip_loadThread ldr r2,
      [ r1, #OSThread.profiler ] // r2 = functionCostInfo
      cmp r2,
      #0 beq _skip_loadThread
      ...
      #0 beq _skip_loadThread
          ...
          ldr r3,
      [ r2, #OSFunctionCostInfo.current ] // r3 = currentPtr of trace
      cmp r3,
      #0 beq _skip_loadThread

          sub r1,
      r3,
      #OSi_SIZEOF_FUNCTIONCOST // r1 = previous Ptr

      add r0,
      r2,
      #OSFunctionCostInfo.array[0] // r0 = array Ptr
      cmp r1,
      r0 bmi _skip_loadThread

          stmfd sp !,
      {r1 - r3} bl OSi_GetTick_noProfile // r0 = currentTime
          ldmfd sp !,
      {r1 - r3} ldr r3,
      [ r2, #OSFunctionCostInfo.breakTime ] // r3 = breakTime
      sub r3,
      r0,
      r3 // r3 = currentTime - breakTime = interval

          ldr r0,
      [ r1, #OSFunctionCost.entry.interval ] // add interval
      add r0,
      r0,
      r3 str r0,
      [ r1, #OSFunctionCost.entry.interval ]
