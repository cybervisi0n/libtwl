...
static int OSi_CompareVCount(OSVAlarm *alarm, s32 currentVCount,
                             s32 currentVFrame);
...
#define OSi_IsNearVCount(a, b, c)                                              \
  ((OSi_VCountSub((a), (c)) < OSi_VCountSub((b), (c))) ? TRUE : FALSE)
...
BOOL OS_IsVAlarmAvailable(void) { return OSi_UseVAlarm; }
...
void OS_SetVAlarm(OSVAlarm *alarm, s16 count, s16 delay,
  ...
  alarm->fire = count;
  alarm->frame =
      (u32)((count > currentVCount) ? currentVFrame : (currentVFrame + 1));
...
void OS_SetPeriodicVAlarm(OSVAlarm *alarm, s16 count, s16 delay,
                          OSVAlarmHandler handler, void *arg) {
  OSIntrMode enabled = OS_DisableInterrupts();
  s32 currentVCount;
  s32 currentVFrame;
  ...
  SDK_TASSERTMSG(0 <= delay && delay <= OSi_VHIGHT,
...
void OS_CancelAllVAlarms(void) {
  ...
  for (alarm = OSi_VAlarmQueue.head, next = alarm ? alarm->next : NULL; alarm;
       alarm = next, next = alarm ? alarm->next : NULL) {
...
#ifdef DEBUGPRINT
    OS_TPrintf("[%d:%d %d] %d %d check=%d \n", alarm->arg, alarm->frame,
               alarm->fire, currentVFrame, currentVCount, check);
#endif
...
static int OSi_CompareVCount(OSVAlarm *alarm, s32 currentVFrame,
                             s32 currentVCount) {
  s32 delayVFrame;
  s32 delayVCount;

  delayVFrame =
      currentVFrame - (s32)alarm->frame; // get diff for safety against overflow
...
void OS_DumpVAlarm(void) {
  ...
  for (alarm = OSi_VAlarmQueue.head; alarm; alarm = next) {
    ...
    OS_TPrintf("%08x fire=%d:%d delay=%d arg=%2d prev=%08x next=%08x\n", alarm,
               alarm->frame, alarm->fire, alarm->delay, alarm->arg, alarm->prev,
               alarm->next);
  }
