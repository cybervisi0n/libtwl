...
asm BOOL OS_EnableCallTrace(void) {
  ...
  stmfd sp !, {lr} bl OSi_GetCallTraceInfo ldmfd sp !, {lr} cmp r0,
      #0 bxeq lr

          mov r2,
      r0 ldrh r0, [ r2, #OSCallTraceInfo.enable ] mov r1, #1 strh r1,
      [ r2, #OSCallTraceInfo.enable ]
...
asm BOOL OS_DisableCallTrace(void) {
  ...
  stmfd sp !, {lr} bl OSi_GetCallTraceInfo ldmfd sp !, {lr} cmp r0,
      #0 bxeq lr
          ...
          mov r2,
      r0 ldrh r0, [ r2, #OSCallTraceInfo.enable ] mov r1, #0 strh r1,
      [ r2, #OSCallTraceInfo.enable ]
...
asm BOOL OS_RestoreCallTrace(BOOL) {
  ...
  stmfd sp !, {r0, lr} bl OSi_GetCallTraceInfo mov r1, r0 ldmfd sp !,
      {r0, lr} cmp r1,
      #0 bxeq lr

          ldrh r2,
      [ r1, #OSCallTraceInfo.enable ] strh r0,
      [ r1, #OSCallTraceInfo.enable ] mov r0,
      r2
...
void OS_InitCallTrace(void *buf, u32 size, OSCallTraceMode mode) {
  ...
  SDK_TASSERTMSG(
      (size >= OSi_TRACEINFO_SIZEOF_HEADERPART + sizeof(OSCallTrace)),
  ...
  *infoPtr = (OSCallTraceInfo *)buf;
  ...
  (*infoPtr)->limit =
      (OSCallTrace *)((u32)buf + OSi_TRACEINFO_SIZEOF_HEADERPART +
...
asm void __PROFILE_ENTRY(void) {
  ...
  stmfd sp !,
      {r0, lr} bl OSi_GetCallTraceInfo mov r1,
      r0 // r1 = callTraceInfo
          ldmfd sp !,
      {r0, lr} cmp r1,
      #0 beq _exit
          ...
          ldrh r2,
      [ r1, #OSCallTraceInfo.enable ] cmp r2,
      #0 beq _exit

          ldr r2,
      [ r1, #OSCallTraceInfo.current ] // r2 = currentPtr of trace
      cmp r2,
      #0 beq _exit

          ldrh r3,
      [ r1, #OSCallTraceInfo.circular ] cmp r3,
      #0 bne _circular_skip_overcheck

#ifdef OS_CALLTRACE_CHECK_OVERSTACK
          ldr r3,
      [ r1, #OSCallTraceInfo.limit ] cmp r2, r3 ldrpl r1,
      [ sp, #16 ] // lr
      ldrpl r2,
      [ sp, #12 ] // r0
      movpl r3,
      sp // sp ( r1,r2,r3 stored )
      ...
      ldr r0,
      [ r1, #OSCallTraceInfo.level ] str r0,
      [ r2, #OSCallTrace.level ] add r0,
      r0,
      #1 str r0,
      [ r1, #OSCallTraceInfo.level ]
#endif

      add r2,
      r2,
      #OSi_SIZEOF_CALLTRACE

      ldrh r3,
      [ r1, #OSCallTraceInfo.circular ] cmp r3,
      #0 beq _notcircular_skip_ring ldr r3,
      [ r1, #OSCallTraceInfo.limit ] cmp r2,
      r3 bmi _store_current add r2,
      r1,
      #OSCallTraceInfo.array

          _notcircular_skip_ring : _store_current : str r2,
      [ r1, #OSCallTraceInfo.current ]
...
asm void __PROFILE_EXIT(void) {
  ...
  ldr r0,
      ...
      #0 beq _exit
          ...
          bl OSi_GetCallTraceInfo cmp r0,
      #0 beq _exit

          ldrh r1,
      [ r0, #OSCallTraceInfo.enable ] cmp r1,
      #0 beq _exit

#ifdef OS_CALLTRACE_LEVEL_AVAILABLE

          ldr r1,
      [ r0, #OSCallTraceInfo.level ] sub r1, r1, #1 movmi r1, #0 str r1,
      [ r0, #OSCallTraceInfo.level ]
#endif

      ldrh r1,
      [ r0, #OSCallTraceInfo.circular ] cmp r1,
      #0 bne _exit

          ldr r1,
      [ r0, #OSCallTraceInfo.current ] // r1 = current
      sub r1,
      r1,
      #OSi_SIZEOF_CALLTRACE
...
void OSi_DumpFullInfo(OSCallTraceInfo *info) {
  if (info && info->current) {
    ...
    while (1) {
      ...
      if ((u32)p < (u32)info->array || 0x2000000 > (u32)(p->name) ||
          0x2400000 < (u32)(p->name)) {
        break;
      }
...
#include <nitro/code32.h>
asm void OS_DumpCallTrace(void) { mov r0, lr b OSi_DumpCallTrace_core }
#include <nitro/codereset.h>
...
asm void OS_DumpThreadCallTrace(const OSThread *thread) {
  mov r1, lr b OSi_DumpThreadCallTrace_core
}
#include <nitro/codereset.h>
