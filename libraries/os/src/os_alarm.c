...
#define OSi_ALARM_TIMERCONTROL                                                 \
  (REG_OS_TM0CNT_H_E_MASK | REG_OS_TM0CNT_H_I_MASK | OS_TIMER_PRESCALER_64)
...
void OS_InitAlarm(void) {
  if (!OSi_UseAlarm) {
    ...
    SDK_TASSERTMSG(OS_IsTickAvailable(),
                   "OS_InitAlarm: alarm system needs of tick system.");
...
BOOL OS_IsAlarmAvailable(void) { return OSi_UseAlarm; }
...
void OS_SetAlarm(OSAlarm *alarm, OSTick tick, OSAlarmHandler handler,
                 void *arg) {
  OSIntrMode enabled;
...
void OS_SetPeriodicAlarm(OSAlarm *alarm, OSTick start, OSTick period,
...
void OS_CancelAlarms(u32 tag) {
  ...
  for (alarm = OSi_AlarmQueue.head, next = alarm ? alarm->next : NULL; alarm;
...
struct OSiAlarmQueue *OSi_GetAlarmQueue(void) { return &OSi_AlarmQueue; }
