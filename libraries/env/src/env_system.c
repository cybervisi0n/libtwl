...
static ENVResource *
ENVi_SearchByMemberNameFromResourceSet(ENVResource *resSet,
...
void ENV_SetClass(const char *className) {
  SDK_WARNING(STD_GetStringLength(className) <= ENV_CLASS_NAME_MAX,
...
char *ENV_GetClass(void) { return sCurrentClass; }
...
ENVResource *ENVi_SearchByMemberName(const char *memberName,
  ...
  if (link && memberName && *memberName != '\0') {
    while (link) {
      ENVResource *p =
          ENVi_SearchByMemberNameFromResourceSet(link->array, memberName);
      if (p) {
        if (resSetPtr) {
...
ENVResource *ENVi_SearchByFullName(const char *fullName,
                                   ENVResource **resSetPtr) {
  ENVResourceSetLink *link = sResourceSetLinkHead;
  ...
  if (link && fullName && *fullName != '\0') {
    while (link) {
      ENVResource *p =
          ENVi_SearchByFullNameFromResourceSet(link->array, fullName);
      if (p) {
        if (resSetPtr) {
          *resSetPtr = link->array;
        }
...
BOOL ENV_GetU64(const char *name, u64 *retVal) {
  ...
  if (p->type & ENV_RESTYPE_OFFSET_MASK) {
    *retVal = *(u64 *)((u32)resSetPtr + (u32)(p->ptr));
    ...
    SDK_WARNING((p + 1)->name &&
                    !STD_CompareString((p + 1)->name, ENVi_DUMMY_FOR_U64),
...
ENVResource *ENV_SearchByPartialName(ENVResourceIter *iter,
